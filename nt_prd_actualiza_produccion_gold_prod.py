...
LEFT JOIN (SELECT complexentityno, MIN(descripfecha) fecha, MIN(avesgranjaacum)avesgranjaacum, MIN(edadgranja)edadgranja,MIN(edadgalpon)edadgalpon, MIN(edadlote) edadlote, MIN(edadiniciosaca) edadiniciosaca,COUNT(distinct descripfecha) DiasSacaEfectivo FROM {database_name_gl}.ft_ventas_CD GROUP BY complexentityno) VE ON PD.complexentityno = VE.complexentityno  \
...
LEFT JOIN {database_name_tmp}.STDConsDiaNuevo STDCN on substring(DTA.complexentityno,1,(LENGTH(DTA.complexentityno)-3)) = STDCN.ComplexEntityNoGalpon and Upper(DTA.DescripTipoAlimentoXTipoProducto) = Upper(STDCN.DescripTipoAlimentoXTipoProducto) \
...
AND PD.pk_plantel NOT IN (21,180)  \
...
df_STDPorcConsDiaHorizontal = spark.sql("""
    SELECT
        A.pk_plantel,
        A.pk_lote,
        A.pk_galpon,
...
LEFT JOIN {database_name_gl}.lk_tiempo LT ON LT.fecha =MO.EventDate
LEFT JOIN {database_name_gl}.ft_consolidado_galpon CG ON MO.ComplexEntityNo = CG.ComplexEntityNo
LEFT JOIN {database_name_tmp}.RegistrosTotalCeros RTC ON RTC.ComplexEntityNo = MO.ComplexEntityNo AND RTC.Eventdate = MO.Eventdate
