...
df_Ingresobb1 = spark.sql(f"SELECT \
                          PE.ComplexEntityNo \
                          ,(select pk_empresa from {database_name}.lk_empresa where cempresa=1) as pk_empresa  \
                          ...
                          ,(select nempresa from {database_name}.lk_empresa where cempresa=1) descripempresa \
...
if datos_existentes:
    ...
    final_data2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name}.{table_name4}")
    ...
    print(f"Tabla {table_name4}Temporal eliminada correctamente de la base de datos '{database_name}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}{table_name4}"
    }
...
if datos_existentes5:
    ...
    final_data5.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name}.{table_name5}Temporal")
    ...
    final_data5_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name}.{table_name5}")
    ...
    print(f"Tabla {table_name5}Temporal eliminada correctamente de la base de datos '{database_name}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}{table_name5}"
    }
