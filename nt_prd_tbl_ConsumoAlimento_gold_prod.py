...
df_ConsumoAlimento = spark.sql(f"SELECT nvl(MO.pk_tiempo,(select pk_tiempo from {database_name_gl}.lk_tiempo where fecha=cast('1899-11-30' as date))) as pk_tiempo, \
nvl(MO.descripfecha,cast('1899-11-30' as date)) as fecha, \
MO.pk_empresa, \
nvl(MO.pk_division,(select pk_division from {database_name_gl}.lk_division where cdivision=0)) pk_division, \
nvl(MO.pk_zona,(select pk_zona from {database_name_gl}.lk_zona where czona='0')) pk_zona, \
...
nvl(LPC.pk_productoconsumo,(select pk_productoconsumo from {database_name_gl}.lk_productoconsumo where cproductoconsumo='0')) pk_productoconsumo, \
nvl(MO.pk_grupoconsumo,(select pk_grupoconsumo from {database_name_gl}.lk_grupoconsumo where cgrupoconsumo='0')) pk_grupoconsumo, \
nvl(LSG.pk_subgrupoconsumo,(select pk_subgrupoconsumo from {database_name_gl}.lk_subgrupoconsumo where csubgrupoconsumo=0)) pk_subgrupoconsumo, \
...
nvl(MO.pk_especie,(select pk_especie from {database_name_gl}.lk_especie where cespecie='0')) pk_especie, \
...
MO.Edad AS Edad, \
MO.ConsDia, \
nvl(MO.pk_alimento,(select pk_alimento from {database_name_gl}.lk_alimento where calimento='0')) pk_alimento, \
...
LEFT JOIN {database_name_gl}.lk_formula LFR ON LFR.ProteinProductsIRN = cast(MO.ProteinProductsIRN as varchar(50)) \
...
WHERE MO.Consdia <> 0 AND (date_format(MO.descripfecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-9),'yyyyMM'))")
...
df_ConsumoAlimento.write \
    ...
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.consumoalimento")
...
pk_administrador ,
pk_proveedor 
,CASE WHEN (Edad = 0 or Edad = -1) and (date_format(fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-9),'yyyyMM')) then 1 
WHEN Edad <= 0 and (date_format(fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-9),'yyyyMM')) AND SUBSTRING(ComplexEntityNo,1,1) = 'V' then 1 
...
,Valor 
,FechaCierre 
from {database_name_tmp}.consumoalimento""")
...
df_ConsumoAlimento_upd.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.consumoalimentoupd")
...
df_ConsumoAlimento0 = spark.sql(f"""SELECT 
...
FROM {database_name_tmp}.consumoalimentoupd
...
df_ConsumoAlimento0.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.consumoalimento0")
print('carga consumoalimento0')
df_ConsumoAlimento = spark.sql(f"SELECT * FROM {database_name_tmp}.consumoalimento0 WHERE consdia <> 0 ORDER BY fecha DESC")
...
df_ConsumoAlimento.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.consumoalimento1")
print('carga consumoalimento1')
df_estadoTemp = spark.sql(f"select complexentityno,pk_estado,pk_empresa from {database_name_gl}.stg_ProduccionDetalle \
...
df_estadoTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.estadoTemp")
print('carga estadoTemp')
df_ft_consumoalimento_upd = spark.sql(f"""
SELECT 
pk_tiempo,
A.pk_empresa ,
pk_division ,
...
pk_grupoconsumo ,
pk_subgrupoconsumo ,
pk_tipoproducto,pk_especie , 
CASE WHEN isnotnull(B.PK_estado) = true THEN B.pk_estado ELSE A.pk_estado END AS pk_estado,
pk_administrador ,
pk_proveedor ,
pk_diasvida ,
pk_semanavida ,
...
'' as descripstandard,
'' as descripproducto,
'' as descripproductoconsumo,
'' as descripgrupoconsumo,
'' as descripsubgrupoconsumo,
...
'' as descripalimento,
'' as descripformula
FROM {database_name_tmp}.consumoalimento1 A left join {database_name_tmp}.estadoTemp B on A.ComplexEntityNo = B.ComplexEntityNo""")

df_ft_consumoalimento = df_ft_consumoalimento_upd

print('inicio de carga de la tabla df_ft_consumoalimento')
additional_options = {
"path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temp/ft_consumoalimento_Temp"
}
df_ft_consumoalimento.write \
    .format("parquet") \
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ft_consumoalimento_Temp")
print('carga ft_consumoalimento_Temp')
...
if datos_existentes:
    ...
    filtered_new_data = df_ft_consumoalimento.filter((date_format(F.col("descripfecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_consumoalimentoTemporal")
    ...
    final_data2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_consumoalimento")
    ...
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_consumoalimentoTemporal')
    print(f"Tabla ft_onsumoalimentoTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
...
df_FormulaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.FormulaTemp")
...
FROM {database_name_tmp}.FormulaTemp F group by  F.ComplexEntityNo, F.pk_diasvida" )
...
df_ListaFormulaNoNameTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ListaFormulaNoNameTemp")
...
from {database_name_tmp}.FormulaTemp A \
left join {database_name_tmp}.ListaFormulaNoNameTemp F on F.ComplexEntityNo = A.ComplexEntityNo and F.pk_diasvida = A.pk_diasvida \
...
df_TablaConsumoAlimentoXFormulaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.TablaConsumoAlimentoXFormula")
...
WHERE ConsDia <>0.00 AND (date_format(descripFecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-9),'yyyyMM')) \
...
df_consumoTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.consumoTemp")
...
df_consumoSTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.consumoSTemp")
...
df_TablaPivotConsumoXTipoAlimentoTemp = spark.sql(f""" 
WITH PivotConsumo AS (
    SELECT
        ComplexEntityNo,
        pk_diasvida,
        MAX(CASE WHEN pk_alimento = 56 THEN ConsDia END) AS PreInicio,
        MAX(CASE WHEN pk_alimento = 25 THEN ConsDia END) AS Inicio,
        MAX(CASE WHEN pk_alimento = 5  THEN ConsDia END) AS Acabado,
        MAX(CASE WHEN pk_alimento = 75 THEN ConsDia END) AS Terminado,
        MAX(CASE WHEN pk_alimento = 17 THEN ConsDia END) AS Finalizador,
        MAX(CASE WHEN pk_alimento = 28 THEN ConsDia END) AS PavoIni,
        MAX(CASE WHEN pk_alimento = 39 THEN ConsDia END) AS Pavo1,
        MAX(CASE WHEN pk_alimento = 40 THEN ConsDia END) AS Pavo2,
        MAX(CASE WHEN pk_alimento = 41 THEN ConsDia END) AS Pavo3,
        MAX(CASE WHEN pk_alimento = 42 THEN ConsDia END) AS Pavo4,
        MAX(CASE WHEN pk_alimento = 43 THEN ConsDia END) AS Pavo5,
        MAX(CASE WHEN pk_alimento = 44 THEN ConsDia END) AS Pavo6,
        MAX(CASE WHEN pk_alimento = 45 THEN ConsDia END) AS Pavo7
    FROM {database_name_tmp}.consumoTemp
    GROUP BY ComplexEntityNo, pk_diasvida
)
SELECT * FROM PivotConsumo
""")
...
df_TablaPivotConsumoXTipoAlimentoTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.TablaPivotConsumoXTipoAlimento")
print('carga TablaPivotConsumoXTipoAlimento')
...
#                                            
df_TablaPivotConsumoXSemanaTemp = spark.sql(f""" 
WITH PivotConsumoS AS (
    SELECT
        ComplexEntityNo,
        MAX(CASE WHEN pk_semanavida = 2  THEN ConsDia END) AS ConsSem1,
        MAX(CASE WHEN pk_semanavida = 3  THEN ConsDia END) AS ConsSem2,
        MAX(CASE WHEN pk_semanavida = 4  THEN ConsDia END) AS ConsSem3,
        MAX(CASE WHEN pk_semanavida = 5  THEN ConsDia END) AS ConsSem4,
        MAX(CASE WHEN pk_semanavida = 6  THEN ConsDia END) AS ConsSem5,
        MAX(CASE WHEN pk_semanavida = 7  THEN ConsDia END) AS ConsSem6,
        MAX(CASE WHEN pk_semanavida = 8  THEN ConsDia END) AS ConsSem7,
        MAX(CASE WHEN pk_semanavida = 9  THEN ConsDia END) AS ConsSem8,
        MAX(CASE WHEN pk_semanavida = 10 THEN ConsDia END) AS ConsSem9,
        MAX(CASE WHEN pk_semanavida = 11 THEN ConsDia END) AS ConsSem10,
        MAX(CASE WHEN pk_semanavida = 12 THEN ConsDia END) AS ConsSem11,
        MAX(CASE WHEN pk_semanavida = 13 THEN ConsDia END) AS ConsSem12,
        MAX(CASE WHEN pk_semanavida = 14 THEN ConsDia END) AS ConsSem13,
        MAX(CASE WHEN pk_semanavida = 15 THEN ConsDia END) AS ConsSem14,
        MAX(CASE WHEN pk_semanavida = 16 THEN ConsDia END) AS ConsSem15,
        MAX(CASE WHEN pk_semanavida = 17 THEN ConsDia END) AS ConsSem16,
        MAX(CASE WHEN pk_semanavida = 18 THEN ConsDia END) AS ConsSem17,
        MAX(CASE WHEN pk_semanavida = 19 THEN ConsDia END) AS ConsSem18,
        MAX(CASE WHEN pk_semanavida = 20 THEN ConsDia END) AS ConsSem19,
        MAX(CASE WHEN pk_semanavida = 21 THEN ConsDia END) AS ConsSem20
    FROM {database_name_tmp}.consumoSTemp
    GROUP BY ComplexEntityNo
)
SELECT * FROM PivotConsumoS
""")
...
df_TablaPivotConsumoXSemanaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.TablaPivotConsumoXSemana")
print('carga TablaPivotConsumoXSemana')
...
from {database_name_tmp}.consumoTemp A \
left join {database_name_tmp}.TablaPivotConsumoXTipoAlimento B on A.ComplexEntityNo = B.ComplexEntityNo and A.pk_diasvida = B.pk_diasvida \
left join {database_name_tmp}.TablaPivotConsumoXSemana C on A.ComplexEntityNo = C.ComplexEntityNo \
...
df_ConsumosXTipoAlimentoTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ConsumosXTipoAlimento")
