...
df_mvHimChickTrans= spark.sql(f"select CONCAT(rtrim(upper(farmno)),'-',rtrim(upper(entityno))) as ComplexEntityNo, sum(RelativeHeadPlaced) as Total \
                              from {database_name_si}.si_mvhimchicktrans \
...
df_mvHimChickTrans.write \
    ...
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.mvHimChickTrans")
...
df_mvHimChickTransGalpon.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.mvHimChickTransGalpon")
...
from {database_name_si}.si_mvproteinproductwhusage \
...
df_GasPropano.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.GasPropano")
...
from {database_name_si}.si_mvproteinproductwhusage x \
...
df_GasGranel.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.GasGranel")
...
df_Cama1.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Cama1")
print('carga Cama1')
...
df_Cama2.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Cama2")
print('carga Cama2')
...
df_Cama3.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Cama3")
print('carga Cama3')
...
df_Cama4.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Cama4")
print('carga Cama4')
...
df_Cama5.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Cama5")
print('carga Cama5')
...
df_Cama6.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Cama6")
print('carga Cama6')
...
df_Agua.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Agua")
print('carga Agua')
...
df_mvPmtsProcRecvTrans.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.mvPmtsProcRecvTrans")
...
df_mvPmtsProcRecvTransGalpon.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.mvPmtsProcRecvTransGalpon")
...
from \
{database_name_tmp}.mvHimChickTrans A \
left join {database_name_tmp}.GasPropano B on upper(A.ComplexEntityNo) = upper(B.TransactionEntityID) \
left join {database_name_tmp}.GasGranel C on upper(A.ComplexEntityNo) = upper(c.TransactionEntityID) \
left join {database_name_tmp}.Cama1 D on upper(A.ComplexEntityNo) = upper(D.TransactionEntityID) \
left join {database_name_tmp}.Cama2 E on upper(A.ComplexEntityNo) = upper(E.TransactionEntityID) \
left join {database_name_tmp}.Cama3 F on upper(A.ComplexEntityNo) = upper(F.TransactionEntityID) \
left join {database_name_tmp}.Cama4 G on upper(A.ComplexEntityNo) = upper(G.TransactionEntityID) \
left join {database_name_tmp}.Cama5 H on upper(A.ComplexEntityNo) = upper(H.TransactionEntityID) \
left join {database_name_tmp}.Cama6 k on upper(A.ComplexEntityNo) = upper(K.TransactionEntityID) \
left join {database_name_tmp}.mvPmtsProcRecvTrans I on upper(A.ComplexEntityNo) = upper(i.ComplexEntityNo) \
left join {database_name_tmp}.Agua J on upper(A.ComplexEntityNo) = upper(J.TransactionEntityID) \
union \
select \
...
J.ProductName AS ProductNameA \
from \
{database_name_tmp}.mvHimChickTransGalpon A \
left join {database_name_tmp}.GasPropano B on upper(A.ComplexEntityNo) = upper(B.TransactionEntityID) \
left join {database_name_tmp}.GasGranel C on upper(A.ComplexEntityNo) = upper(c.TransactionEntityID) \
left join {database_name_tmp}.Cama1 D on upper(A.ComplexEntityNo) = upper(D.TransactionEntityID) \
left join {database_name_tmp}.Cama2 E on upper(A.ComplexEntityNo) = upper(E.TransactionEntityID) \
...
left join {database_name_tmp}.Cama5 H on upper(A.ComplexEntityNo) = upper(H.TransactionEntityID) \
left join {database_name_tmp}.Cama6 k on upper(A.ComplexEntityNo) = upper(K.TransactionEntityID) \
left join {database_name_tmp}.mvPmtsProcRecvTransGalpon I on upper(A.ComplexEntityNo) = upper(i.ComplexEntityNo) \
left join {database_name_tmp}.Agua J on upper(A.ComplexEntityNo) = upper(J.TransactionEntityID))A ")
...
df_ConsumoGL.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ConsumoGL")
...
from {database_name_tmp}.mvHimChickTransGalpon A \
left join {database_name_tmp}.GasPropano B on upper(A.ComplexEntityNo) = upper(B.TransactionEntityID) \
left join {database_name_tmp}.GasGranel C on upper(A.ComplexEntityNo) = upper(c.TransactionEntityID) \
left join {database_name_tmp}.Cama1 D on upper(A.ComplexEntityNo) = upper(D.TransactionEntityID) \
left join {database_name_tmp}.Cama2 E on upper(A.ComplexEntityNo) = upper(E.TransactionEntityID) \
...
from {database_name_tmp}.mvHimChickTrans A \
left join {database_name_tmp}.GasPropano B on upper(A.ComplexEntityNo) = upper(B.TransactionEntityID) \
left join {database_name_tmp}.GasGranel C on upper(A.ComplexEntityNo) = upper(c.TransactionEntityID) \
left join {database_name_tmp}.Cama1 D on upper(A.ComplexEntityNo) = upper(D.TransactionEntityID) \
left join {database_name_tmp}.Cama2 E on upper(A.ComplexEntityNo) = upper(E.TransactionEntityID) \
...
left join {database_name_tmp}.Cama4 G on upper(A.ComplexEntityNo) = upper(G.TransactionEntityID) \
left join {database_name_tmp}.Cama5 H on upper(A.ComplexEntityNo) = upper(H.TransactionEntityID) \
left join {database_name_tmp}.Cama6 L on upper(A.ComplexEntityNo) = upper(L.TransactionEntityID) \
left join {database_name_tmp}.mvPmtsProcRecvTrans I on upper(A.ComplexEntityNo) = upper(i.ComplexEntityNo) \
left join {database_name_tmp}.Agua J on upper(A.ComplexEntityNo) = upper(J.TransactionEntityID) \
left join {database_name_tmp}.ConsumoGL K ON upper(K.ComplexEntityNoLT) = upper(A.ComplexEntityNo) \
...
if datos_existentes:
    ...
    final_data2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.stg_gasaguacama")
