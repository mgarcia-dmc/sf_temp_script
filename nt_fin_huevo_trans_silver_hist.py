...
JOB_NAME = "nt_fin_huevo_trans_silver_hist"
...
# Variables YBWT_MDOC
table_name_source_huevo = "br_ybwt_mdoc"
table_name_target_huevo = "si_ybwt_mdoc"
path_source_huevo = f"s3://{bronze_bucket_name}/UE1STG{p_ambU}AS3FIN001/SAP/HUEVO/{table_name_source_huevo}"
path_target_huevo = f"s3://{silver_bucket_name}/UE1STG{p_ambU}AS3FIN001/SAP/HUEVO/{table_name_target_huevo}"
    
# Leer archivos de origen
try:
    df_source_huevo = spark.read.parquet(path_source_huevo)
    # Contar registros y finalizar
    record_count_huevo = df_source_huevo.count()
    print(f"Registros procesados: {record_count_huevo}")
    # Guardar datos en formato Parquet y registrar en Data Catalog
    df_source_huevo.write \
        .format("parquet") \
        .mode("overwrite") \
    ...
    print(f"No se encontraron archivos para la tabla {table_name_source_huevo}. Error: {e}")
