...
file_name_target31 = f"{bucket_name_prdmtech}{name_table31}/"
path_target31 = f"s3://{bucket_name_target}/{file_name_target31}"
...
FROM {database_name_si}.si_pmtsprocrecvtrans PPT \
LEFT JOIN {database_name_si}.si_mvpmtsprocrecvtranshousedetail PPTD ON PPT.IRN = PPTD.PmtsProcRecvTransIRN \
LEFT JOIN {database_name_si}.si_mvpmtstransferdestdetails AS PTS ON PPTD.ComplexEntityNo = PTS.SourceComplexEntityNo \
LEFT JOIN {database_name_si}.si_mvpmtstransferdestdetails AS PTD ON PPTD.ComplexEntityNo = PTD.DestinationComplexEntityNo \
...
df_SacaPavo0Temp.write \
    ...
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SacaPavo0")
...
from {database_name_tmp}.SacaPavo0 \
where ComplexEntityNo in (select DestinationComplexEntityNo from {database_name_si}.si_mvpmtstransferdestdetails where DEstinationComplexEntityNo like 'v%') \
...
df_DestinationComplexEntityNoTemp1.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.DestinationComplexEntityNoTemp1")
...
FROM
    {database_name_tmp}.SacaPavo0 A
LEFT JOIN
    {database_name_tmp}.DestinationComplexEntityNoTemp1 B ON A.DestinationComplexEntityNo = B.ComplexEntityNo
LEFT JOIN
    (SELECT DISTINCT SourceComplexEntityNo FROM {database_name_si}.si_mvpmtstransferdestdetails WHERE SourceComplexEntityNo LIKE 'v%') AS EXISTS_V
...
df_SacaPavo0_upd.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SacaPavo0_upd")
...
WITH FarmHdCountTotals AS (
    ...
    FROM {database_name_si}.si_mvpmtsprocrecvtranshousedetail
...
FarmHdCountAccumulated AS (select ComplexEntityNo,EventDate, max(FarmHdCountDAcum) FarmHdCountDAcum from (
    ...
    FROM
        {database_name_si}.si_mvpmtsprocrecvtranshousedetail
    WHERE
...
SELECT
    ...
    FWNA.FarmWtNetDAcum
FROM
    {database_name_si}.si_pmtsprocrecvtrans PPT
LEFT JOIN
    {database_name_si}.si_mvpmtsprocrecvtranshousedetail PPTD ON PPT.IRN = PPTD.PmtsProcRecvTransIRN
LEFT JOIN
    (SELECT * FROM (
        SELECT
            ComplexEntityNo,
        ...
        FROM
            {database_name_gl}.ft_alojamiento A
        WHERE
            CantAlojXRaza = (
                SELECT MAX(CantAlojXRaza)
                FROM {database_name_gl}.ft_alojamiento B
...
SELECT
    ...
    FWNA.FarmWtNetDAcum
FROM
    {database_name_tmp}.SacaPavo0 SP
LEFT JOIN
    (SELECT * FROM(
        SELECT
            ComplexEntityNo,
...
df_Saca1Temp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Saca1")
...
WITH SacaMortData AS (
    ...
    FROM
        {database_name_si}.si_brimentityinventory A
    INNER JOIN {database_name_si}.si_mvbrimentities B ON A.ProteinEntitiesIRN = B.IRN AND (xDate between B.FirstDateSold AND B.LastDateSold)
    WHERE
...
SELECT
    ...
    (SELECT SUM(FarmHdCountd) AS FarmHdCountd
     FROM {database_name_tmp}.Saca1 X
...
FROM
    {database_name_si}.si_brimentityinventory A
LEFT JOIN
    {database_name_si}.si_mvbrimentities B ON A.ProteinEntitiesIRN = B.IRN
LEFT JOIN
    {database_name_tmp}.Saca1 C ON A.ProteinEntitiesIRN = C.ProteinEntitiesIRND AND A.TransactionIRN = C.irnd
LEFT JOIN
    {database_name_si}.si_mvbrimentities D ON D.IRN = C.ProteinEntitiesIRN
LEFT JOIN
    {database_name_si}.si_mvbrimentities E ON C.ComplexEntityNoGalpon = E.ComplexEntityNo AND E.GRN = 'H'
...
CASE WHEN isnull(PTS.SourceComplexEntityNo) = true THEN PTD.SourceComplexEntityNo ELSE PTS.SourceComplexEntityNo END SourceComplexEntityNo, \
CASE WHEN isnull(PTD.DestinationComplexEntityNo) = true THEN PTS.DestinationComplexEntityNo ELSE PTD.DestinationComplexEntityNo END DestinationComplexEntityNo \
FROM {database_name_si}.si_brimentityinventory A \
LEFT JOIN {database_name_si}.si_mvbrimentities B ON A.ProteinEntitiesIRN  = B.IRN \
LEFT JOIN {database_name_si}.si_mvpmtstransferdestdetails AS PTS ON B.ComplexEntityNo = PTS.SourceComplexEntityNo \
LEFT JOIN {database_name_si}.si_mvpmtstransferdestdetails AS PTD ON B.ComplexEntityNo = PTD.DestinationComplexEntityNo \
...
df_SACAPAVO01Temp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SACAPAVO01")
...
from {database_name_tmp}.SACAPAVO01 \
where ComplexEntityNo in (select DestinationComplexEntityNo from {database_name_si}.si_mvpmtstransferdestdetails where DEstinationComplexEntityNo like 'v%') \
...
df_DestinationComplexEntityNo01Temp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.DestinationComplexEntityNo01")
...
SELECT
    ...
    A.DestinationComplexEntityNo
FROM
    {database_name_tmp}.SACAPAVO01 A
LEFT JOIN
    {database_name_tmp}.DestinationComplexEntityNo01 B ON A.DestinationComplexEntityNo = B.ComplexEntityNo
LEFT JOIN
    (SELECT DISTINCT SourceComplexEntityNo FROM {database_name_si}.si_mvpmtstransferdestdetails WHERE SourceComplexEntityNo LIKE 'v%') AS EXISTS_V
    ON A.ComplexEntityNo = EXISTS_V.SourceComplexEntityNo
""")
...
df_SACAPAVO01_upd.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SACAPAVO01_upd")
...
SELECT  
        ...
        SUM(abs(Quantity)) AS SacaMort
    FROM
        (select FirstDateSold,LastDateSold,ProteinEntitiesIRNS from {database_name_tmp}.SACAPAVO01_upd group by FirstDateSold,LastDateSold,ProteinEntitiesIRNS)  A
    INNER JOIN {database_name_si}.si_brimentityinventory B ON B.ProteinEntitiesIRN = A.ProteinEntitiesIRNS AND (B.xDate between A.FirstDateSold AND A.LastDateSold) and b.SourceCode = 'BrimFieldTrans' 
    GROUP BY
        a.ProteinEntitiesIRNS
)
SELECT
    ...
    SMD.SacaMort
FROM
    {database_name_tmp}.SACAPAVO01_upd A
LEFT JOIN
    {database_name_tmp}.Saca1 C ON A.ProteinEntitiesIRNS = C.ProteinEntitiesIRND AND A.TransactionIRN = C.irnd
LEFT JOIN
    {database_name_si}.si_mvbrimentities D ON D.IRN = C.ProteinEntitiesIRN
LEFT JOIN
    {database_name_si}.si_mvbrimentities E ON C.ComplexEntityNoGalpon = E.ComplexEntityNo AND E.GRN = 'H'
...
df_Saca2Temp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Saca2")
...
df_EdadTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.EdadTemp")
...
SELECT 
    ...
    G.EdadNumGalpon,
    L.EdadNumLote
FROM {database_name_tmp}.Saca2 A
...
df_EdadxDateTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.EdadxDateTemp")
...
df_alojamiento2Temp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.alojamiento2Temp")
...
from {database_name_tmp}.EdadxdateTemp A \
left join (select ComplexEntityNo, MAX(AlojNumCorral) AlojNumCorral, MAX(AlojNumGalpon) AlojNumGalpon, MAX(AlojNumLote)AlojNumLote from {database_name_tmp}.alojamiento2Temp group by ComplexEntityNo ) B on A.ComplexEntityNo = B.ComplexEntityNo \
...
df_EdadLiquidacionTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.EdadLiquidacionTemp")
...
,nvl(LPRS.pk_producto,(select pk_producto from {database_name_gl}.lk_producto where cproducto='0')) pk_producto \
...
,nvl(SA.pk_especie,(select pk_especie from {database_name_gl}.lk_especie where cespecie='0')) pk_especie \
...
,nvl(PRO.pk_proveedor,(select pk_proveedor from {database_name_gl}.lk_proveedor where cproveedor=0)) pk_proveedor \
...
,(select count(distinct x.EventDate) from {database_name_tmp}.Saca2 x where x.ComplexEntityNoGalpon = SA.ComplexEntityNoGalpon) DiasSacaEfectivoGalpon \
from {database_name_tmp}.Saca2 SA \
LEFT JOIN {database_name_si}.si_proteincostcenters PCC ON SA.ProteinCostCentersIRN = PCC.IRN \
LEFT JOIN {database_name_si}.si_proteinentities PE ON PE.IRN = SA.ProteinEntitiesIRND \
LEFT JOIN {database_name_si}.si_proteinstandardversions PSV ON PSV.IRN = PE.ProteinStandardVersionsIRN \
left join {database_name_si}.si_proteinproductsanimals PPA ON SA.ProteinProductsAnimalsIRN_Ovr = PPA.IRN \
left join {database_name_si}.si_proteinproducts PP on PPA.ProteinProductsIRN = PP.IRN \
LEFT JOIN {database_name_gl}.lk_tiempo LT ON LT.fecha = cast(cast(SA.EventDate as timestamp) as date) \
LEFT JOIN {database_name_gl}.lk_plantel LP ON LP.IRN = CAST(SA.ProteinFarmsIRN AS VARCHAR (50)) \
LEFT JOIN {database_name_gl}.lk_lote LL ON LL.pk_plantel=LP.pk_plantel AND LL.nlote = RTRIM(SA.EntityNo) and ll.activeflag in (0,1) \
...
LEFT JOIN {database_name_gl}.lk_sexo LS ON LS.csexo = RTRIM(SA.PenNo) \
...
LEFT JOIN {database_name_gl}.lk_producto LPRS ON LPRS.cproducto = PP.ProductNo \
LEFT JOIN {database_name_gl}.lk_tipoproducto TPS ON TPS.ntipoproducto=LPRS.grupoproducto \
LEFT JOIN {database_name_gl}.lk_estado LES ON LES.cestado=PE.Status \
LEFT JOIN {database_name_gl}.lk_planta PLA ON CAST(SA.ProteinCostCentersIRN AS varchar(50)) = PLA.ProteinCostCentersIRN \
LEFT JOIN {database_name_gl}.lk_vehiculo VEH ON CAST(SA.ProteinVehiclesIRN AS VARCHAR(50)) = CAST(VEH.IRN as VARCHAR(50)) \
...
from {database_name_tmp}.edadTemp \
...
LEFT JOIN {database_name_tmp}.EdadLiquidacionTemp E ON E.ComplexEntityNo = SA.ComplexEntityNo \
...
df_SacaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Saca")
...
from {database_name_gl}.ft_mortalidad_Diario 
...
if datos_existentes:
    ...
    final_data.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmr}.ft_ventas_CDTemporal")
    ...
    final_data2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_ventas_cd")
    ...
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_ventas_CDTemporal')
    print(f"Tabla ft_ventas_CDTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_ventas_cd"
    }
...
left join {database_name_gl}.lk_ft_Excel_categoria B on A.pk_lote = B.pk_lote 
where pk_empresa =1 """)
...
if datos_existentes:
    ...
    final_data.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_ventas_CDTemporal")
...
from {database_name_gl}.ft_ventas_CD A
left join 
(select pk_plantel,pk_lote,fechanacimientolote from {database_name_gl}.ft_ventas_cd 
