...
file_name_target1 = f"{bucket_name_prdmtech}ft_ProduccionDetalle/"
...
df_CategoriaGalponTemp = spark.sql(f"SELECT ComplexEntityNo, RTRIM(code) Categoria,e.pk_plantel,f.pk_lote,g.pk_galpon \
                                    FROM {database_name}.si_brimentities a LEFT JOIN {database_name}.si_proteinentities b on a.ProteinEntitiesIRN=b.IRN \
                                    LEFT JOIN {database_name}.si_mvbrimentities c ON c.ProteinEntitiesIRN=a.ProteinEntitiesIRN \
                                    LEFT JOIN {database_name}.si_mtsysuserfieldcodes d ON a.U_Categoria=d.IRN \
                                    LEFT JOIN {database_name}.lk_plantel e ON e.IRN=CAST(b.ProteinFarmsIRN AS VARCHAR(50)) \
                                    LEFT JOIN {database_name}.lk_lote f ON f.PK_Plantel=e.PK_Plantel and f.nlote=rtrim(b.EntityNo) and f.ActiveFlag in (0,1) \
                                    LEFT JOIN {database_name}.lk_galpon g ON g.IRN= cast(b.ProteinHousesIRN as varchar(50)) and f.ActiveFlag in (0,1) \
                                    WHERE a.U_Categoria is not null and GRN = 'H'")
print('carga df_CategoriaGalponTemp')
...
df_CategoriaGalponTemp.write \
    ...
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.CategoriaGalponTemp")
print('carga CategoriaGalponTemp')
df_CategoriaLoteTemp = spark.sql(f"SELECT clote, \
                                    (SELECT  concat_ws(',' , collect_list( DISTINCT Categoria)) categoria \
                                     FROM {database_name}.CategoriaGalponTemp WHERE pk_lote = f.pk_lote) categoria, \
...
df_CategoriaLoteTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.CategoriaLoteTemp")
print('carga CategoriaLoteTemp')



df_Categoria = spark.sql(f"select * from {database_name}.CategoriaGalponTemp \
                          UNION ALL \
                          select * from {database_name}.CategoriaLoteTemp") 
#df_CategoriaGalponTemp.union(df_CategoriaLoteTemp)
...
df_Categoria.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.categoria")
print('carga categoria')
df_Atipicos = spark.sql(f"SELECT ComplexEntityNo,PlantNo, 2 AS FlagAtipico FROM {database_name}.si_mvpmtsprocrecvtranshousedetail \
                         WHERE PlantNo like '%SIN_ELIM%' \
                         GROUP BY ComplexEntityNo,PlantNo")
#df_Atipicos.createOrReplaceTempView("atipicos")
...
df_U_MortAcumPercent=spark.sql(f"""WITH BSD AS (
    ...
    FROM {database_name}.si_brimstandardsdata
)
SELECT * FROM BSD
...
df_U_MortAcumPercent.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.mortacumpercent")
...
WHERE c.FarmType = 1 and b.SpeciesType = 1 and a.EntityHistoryFlag in (false) ")
print('CARGA df_mortdia1')
#se mejora la consulta colocando el filtro de fecha Eventdate
#print(f'CARGA {df_mortdia1.count()}')
...
WHERE c.FarmType = 7 and b.SpeciesType = 2 and a.EntityHistoryFlag in (false) and a.PostStatus <> 1 \
order by  a.xdate")

df_Mortdia = df_mortdia1.union(df_mortdia2)
#df_Mortdia.createOrReplaceTempView("mortdia")
...
from {database_name}.mortdia a inner join {database_name}.mortdia b \
...
df_AccionPesoBajo.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.AccionPesoBajo")
...
df_producciondetalle1 = spark.sql(f"SELECT \
                   ...
                   ,nvl(FAL.pk_alimento,(select pk_alimento from {database_name}.lk_alimento where calimento='0')) as pk_alimento \
                   ...
                   ,nvl(MO.FeedConsumed,0) AS ConsDia \
                   ...
                   ,MO.uniformity \
                   ,MO.CV \
                   FROM  {database_name}.mortdia MO \
                   LEFT JOIN {database_name}.si_proteincostcenters PCC ON MO.ProteinCostCentersIRN = PCC.IRN \
                   ...
                   LEFT JOIN {database_name}.mortacumpercent BSD ON BSD.ProteinStandardVersionsIRN = PSV.IRN and BSD.age = MO.FirstHatchDateAge \
                   LEFT JOIN {database_name}.si_brimentities BE ON BE.ProteinEntitiesIRN= MO.ProteinEntitiesIRN \
                   LEFT JOIN {database_name}.si_mvbrimentities MVB ON MVB.ProteinEntitiesIRN = MO.ProteinEntitiesIRN \
                   LEFT JOIN {database_name}.si_mvbrimfarms MVBF ON MVBF.ProteinFarmsIRN = MO.ProteinFarmsIRN \
                   LEFT JOIN {database_name}.lk_tiempo LT ON date_format(LT.fecha,'yyyyMMdd')= date_format(MO.EventDate, 'yyyyMMdd') \
                   ...
                   LEFT JOIN {database_name}.lk_diasvida  LDV ON LDV.firsthatchdateage = MO.FirstHatchDateAge \
                   LEFT JOIN {database_name}.lk_semanavida LSV ON LSV.pk_semanavida = LDV.pk_semanavida \
                   ...
                   LEFT JOIN {database_name}.lk_alimento FAL ON FAL.IRN = CAST(MO.FmimFeedTypesIRN AS varchar (50)) \
                   ...
                   LEFT JOIN {database_name}.categoria CAT ON CAT.pk_plantel = LP.pk_plantel and CAT.pk_lote = LL.pk_lote and CAT.pk_galpon = LG.pk_galpon \
                   LEFT JOIN {database_name}.atipicos AT ON MO.ComplexEntityNo = AT.ComplexEntityNo \
                   LEFT JOIN {database_name}.CausaPesoBajo CPB ON CPB.complexentityno = MO.complexentityno and CPB.FirstHatchDateAge= MO.FirstHatchDateAge \
                   LEFT JOIN {database_name}.AccionPesoBajo APB ON APB.complexentityno = MO.complexentityno and APB.FirstHatchDateAge= MO.FirstHatchDateAge \
                   WHERE INT(MO.FirstHatchDateAge) >= 0 AND LD.cdivision = '1' AND MO.VoidFlag <> 'true'" )
...
df_producciondetalle2 = spark.sql(f"SELECT MO.IRN as BrimFieldTransIRN, MO.ProteinEntitiesIRN, MO.ProteinGrowoutCodesIRN,MO.ProteinFarmsIRN, MO.ProteinCostCentersIRN,MO.FmimFeedTypesIRN, PCC.ProteinDivisionsIRN, \
                                  ...
                                  nvl(LT.pk_tiempo,(select pk_tiempo from {database_name}.lk_tiempo where fecha=cast('1899-11-30' as date))) pk_tiempo, \
                                  nvl(LT.fecha,cast('1899-11-30' as date)) fecha, \
                                  (select pk_empresa from {database_name}.lk_empresa where cempresa=1) pk_empresa, \
                                  nvl(LD.pk_division,(select pk_division from {database_name}.lk_division where cdivision=0)) as pk_division, \
                                  nvl(LP.pk_zona,(select pk_zona from {database_name}.lk_zona where czona='0')) as pk_zona, \
                                  nvl(LP.pk_subzona,(select pk_subzona from {database_name}.lk_subzona where csubzona='0')) as pk_subzona, \
                                  nvl(LP.pk_plantel,(select pk_plantel from {database_name}.lk_plantel where cplantel='0')) as pk_plantel, \
...
df_producciondetalle.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.producciondetalle")
...
df_producciondetalle_upd = spark.sql(f"select \
                                      ...
                                      ,pk_alimento \
                                      ,pk_proveedor \
                                      ,CASE WHEN (Edad = 0 or Edad = -1) and pk_division = 2 then 1 \
                                              WHEN Edad <= 0 and pk_division = 4 then 1 \
                                      ...
                                      ,uniformity \
                                      ,CV \
                                       from {database_name}.producciondetalle")
...
df_producciondetalle_upd.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.producciondetalle_upd")
...
df_STDMAS42DIAS = spark.sql(f"SELECT pk_tiempo,pk_diasvida,ComplexEntityNo,STDConsDia,STDConsAcum,U_WeightGainDay,U_FeedConversionBC \
                             FROM {database_name}.producciondetalle_upd MD \
                             WHERE pk_diasvida = (SELECT MAX(pk_diasvida) \
                                                  FROM {database_name}.producciondetalle_upd MD1 \
                             ...
                             AND pk_empresa = 1 AND pk_division = 4 \
...
df_STDMAS42DIAS.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.STDMAS42DIAS")
...
df_producciondetalle_upd2 = spark.sql(f"select \
                                         ...
                                         ,A.STDMortSem \
                                         ,A.STDPeso \
                                         ,CASE WHEN A.pk_diasvida > 43 AND A.STDConsdia = 0 AND A.STDConsAcum = 0 AND A.U_WeightGainDay = 0 AND A.U_FeedConversionBC = 0 THEN  B.STDConsDia ELSE  A.STDConsDia END AS STDConsDia \
                                         ,CASE WHEN A.pk_diasvida > 43 AND A.STDConsdia = 0 AND A.STDConsAcum = 0 AND A.U_WeightGainDay = 0 AND A.U_FeedConversionBC = 0 THEN  B.STDConsAcum ELSE  A.STDConsAcum END AS STDConsAcum \
                                         ,A.STD_IEP \
                                         ,CASE WHEN A.pk_diasvida > 43 AND A.STDConsdia = 0 AND A.STDConsAcum = 0 AND A.U_WeightGainDay = 0 AND A.U_FeedConversionBC = 0 THEN  B.U_FeedConversionBC ELSE  A.U_FeedConversionBC END AS U_FeedConversionBC \
                                         ,CASE WHEN A.pk_diasvida > 43 AND A.STDConsdia = 0 AND A.STDConsAcum = 0 AND A.U_WeightGainDay = 0 AND A.U_FeedConversionBC = 0 THEN  B.U_WeightGainDay ELSE  A.U_WeightGainDay END AS U_WeightGainDay \
                                         ,A.U_MortAcumPercent \
                                         ,A.EdadInicioSaca \
                                         ,A.U_PEAccidentados \
                                         ,A.U_PEHigadoGraso \
                                         ...
                                         ,A.uniformity \
                                         ,A.CV \
                                          from {database_name}.producciondetalle_upd A \
                                          LEFT JOIN {database_name}.STDMAS42DIAS B ON A.ComplexEntityNo = B.ComplexEntityNo")
...
df_producciondetalle_upd2.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.producciondetalle_upd2")
...
df_DestinationComplexEntityNo = spark.sql(f"select ProteinEntitiesIRN \
                                                  ...
                                                  ,U_categoria \
                                                  ,FlagAtipico \
                                                  from {database_name}.producciondetalle_upd2 \
...
df_DestinationComplexEntityNo.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.DestinationComplexEntityNo")
...
df_filter.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.filter")
print('carga filter')
...
df_producciondetalle_upd3 = spark.sql(f"select DISTINCT \
                                      ...
                                      from {database_name}.producciondetalle_upd2 A \
                                      LEFT JOIN {database_name}.DestinationComplexEntityNo B ON A.DestinationComplexEntityNo = B.ComplexEntityNo \
                                      LEFT JOIN {database_name}.filter C ON C.SourceComplexEntityNo=A.ComplexEntityNo " )
...
df_producciondetalle_upd3.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.producciondetalle_upd3")
...
try:
    ...
    logger.info(f"Datos existentes de ft_ProduccionDetalle cargados: {df_existentes.count()} registros")
except:
    datos_existentes = False
    logger.info("No se encontraron datos existentes en ft_ProduccionDetalle")
...
if datos_existentes:
    ...
    filtered_new_data = df_producciondetalle_upd3.filter((date_format(F.col("fecha"),'yyyy-MM-dd') >= fecha_str))
    ...
    # Escribir los resultados en ruta temporal
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/ft_ProduccionDetalleTemporal"
    }
    final_data.write \
        .format("parquet") \
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name}.ft_ProduccionDetalleTemporal")
    
    
    #schema = existing_data.schema
    final_data2 = spark.read.format("parquet").load(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/ft_ProduccionDetalleTemporal")
            
            
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_ProduccionDetalle"
    }
    final_data2.write \
        .format("parquet") \
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name}.ft_ProduccionDetalle")
            
    print(f"agrega registros nuevos a la tabla ft_ProduccionDetalle : {cant_ingresonuevo}")
    print(f"Total de registros en la tabla ft_ProduccionDetalle : {cant_total}")
    ...
    glue_client.delete_table(DatabaseName=database_name, Name='ft_ProduccionDetalleTemporal')
    print(f"Tabla ft_ProduccionDetalleTemporal eliminada correctamente de la base de datos '{database_name}'.")
    ...
    df_producciondetalle_upd3.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name}.ft_ProduccionDetalle")
#df_ft_ProduccionDetalle = spark.sql(f"SELECT * FROM {database_name}.producciondetalle_upd3")
#
#additional_options = {
#    "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_ProduccionDetalle"
#}
#df_ft_ProduccionDetalle.write \
#    .format("parquet") \
#    .options(**additional_options) \
#    .mode("overwrite") \
#    .saveAsTable(f"{database_name}.ft_ProduccionDetalle")
#
#print('carga ft_ProduccionDetalle')
