...
JOB_NAME = "nt_prd_tbl_Reproductora_gold"
...
from pyspark.sql.functions import current_date, current_timestamp,datediff, to_date
...
,cast(FLOOR((CASE WHEN VB.FirstHatchDate  IS NULL THEN 0 WHEN date_format(VB.FirstHatchDate,'yyyy-MM-dd')  = '1899-11-30' THEN 0 ELSE (datediff(cast(BFT.eventdate as timestamp),cast(VB.FirstHatchDate  as timestamp)))+6 END)/7 ) as int) AS FirstHatchDateAge \
...
FROM {database_name_si}.si_bimfieldtrans AS BFT \
LEFT OUTER JOIN {database_name_si}.si_mvbimentities AS VB ON BFT.ProteinEntitiesIRN = VB.ProteinEntitiesIRN \
LEFT OUTER JOIN {database_name_si}.si_proteinfarms AS PF ON VB.ProteinFarmsIRN = PF.IRN \
LEFT OUTER JOIN {database_name_si}.si_mvfmimfeedformulas AS VFFF ON BFT.FmimFeedFormulasIRN_F = VFFF.IRN \
LEFT OUTER JOIN {database_name_si}.si_mvfmimfeedformulas AS VFFM ON BFT.FmimFeedFormulasIRN_M = VFFM.IRN \
LEFT OUTER JOIN {database_name_si}.si_fmimfeedtypes AS FTF ON BFT.FmimFeedTypesIRN_Female = FTF.IRN \
LEFT OUTER JOIN {database_name_si}.si_fmimfeedtypes AS FTM ON BFT.FmimFeedTypesIRN_Male = FTM.IRN \
LEFT OUTER JOIN (select * from {database_name_si}.si_fmimfeedtrans \
                where (date_format(CAST(EventDate AS timestamp),'yyyyMM') >= \
                date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) \
                ) as FFT ON BFT.U_TrazSAP = FFT.RefNo \
LEFT OUTER JOIN (select * from {database_name_si}.si_fmimfeedtransdetails \
                where (date_format(CAST(CreationDate AS timestamp),'yyyyMM') >= \
                date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) \
                ) FFTD on FFT.IRN = FFTD.FmimFeedTransIRN \
WHERE BFT.Transfermode <> 3 and (date_format(CAST(BFT.EventDate AS timestamp),'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) \
AND VB.SpeciesType in (1,2)")
print('df_Reprod_Trans1', df_Reprod_Trans1.count())
...
,cast(FLOOR((CASE WHEN FirstHatchDate  IS NULL THEN 0 WHEN date_format(cast(VB.FirstHatchDate as timestamp) ,'yyyy-MM-dd') = '1899-11-30' THEN 0 ELSE (datediff(cast(BFT.eventdate as timestamp),cast(VB.FirstHatchDate  as timestamp)))+6 END)/7 ) as int) AS FirstHatchDateAge \
...
WHERE BFT.Transfermode = 3 and (date_format(CAST(BFT.EventDate AS timestamp),'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) AND VB.SpeciesType in (1,2)")
print('carga df_Reprod_Trans2', df_Reprod_Trans2.count())
...
df_Reprod_Trans.write \
    ...
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Trans")
print('inicia Reprod_Trans', df_Reprod_Trans.count())
...
FROM (select * from {database_name_si}.si_mvhimchicktranshouses \
      where (date_format(CAST(EventDate AS timestamp),'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-7),'yyyyMM'))) A \
LEFT JOIN {database_name_si}.si_mvhimchicktransparentdetail C ON A.ProteinEntitiesIRN = C.ProteinEntitiesIRN and C.HimChickTransHousesIRN = A.HimChickTransHousesIRN \
LEFT JOIN {database_name_si}.si_mvproteinentities D ON C.ProteinEntitiesIRN_Parent = D.IRN \
...
df_ReproductoraPadreTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ReproductoraPadre")
print('inicia ReproductoraPadre', df_ReproductoraPadreTemp.count())
df_ListaTemp = spark.sql(f"SELECT ComplexEntityNoGalpon, concat_ws(',',sort_array(collect_list(DISTINCT ComplexEntityNoPadre))) ComplexEntityNoPadre \
FROM {database_name_tmp}.ReproductoraPadre group by ComplexEntityNoGalpon")
...
df_ListaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ListaTemp")
print('inicia ListaTemp', df_ListaTemp.count())
df_ListaPadreTemp = spark.sql(f"SELECT A.ComplexEntityNo, A.ComplexEntityNoGalpon, \
case when length(B.ComplexEntityNoPadre) = 0 then REPLACE(REPLACE(REPLACE(B.ComplexEntityNoPadre, '\t', cast(null as varchar(100))), '\n', cast(null as varchar(100))), '\r',  cast(null as varchar(100))) else B.ComplexEntityNoPadre end ListaPadre \
FROM {database_name_tmp}.ReproductoraPadre A \
LEFT JOIN {database_name_tmp}.ListaTemp B ON A.ComplexEntityNoGalpon = B.ComplexEntityNoGalpon \
...
df_ListaPadreTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ListaPadre")
print('inicia ListaPadre', df_ListaPadreTemp.count())
...
,nvl(LIC.pk_incubadora,(select pk_incubadora from {database_name_gl}.lk_incubadora where cincubadora='0')) pk_incubadora \
,nvl(PRO.pk_proveedor,(select pk_proveedor from {database_name_gl}.lk_proveedor where cproveedor=0)) AS pk_proveedor \
,LP.listapadre ListaPadre \
FROM (select * from {database_name_si}.si_bimentityinventory \
      where (date_format(CAST(EventDate AS timestamp),'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM'))) BIN \
...
LEFT JOIN {database_name_gl}.lk_tiempo LT ON date_format(LT.fecha,'yyyyMMdd') =date_format(BIN.xDate,'yyyyMMdd') \
...
LEFT JOIN {database_name_si}.si_mvbimfarms MB ON CAST(MB.ProteinFarmsIRN AS VARCHAR(50)) = LPL.IRN \
LEFT JOIN {database_name_gl}.lk_proveedor PRO ON PRO.cproveedor = MB.VendorNo \
LEFT JOIN {database_name_tmp}.ListaPadre LP ON PE.ComplexEntityNo = LP.ComplexEntityNo \
WHERE BIN.EntityHistoryFlag = true AND BIN.SourceCode in ('CHICK','BimEntities') AND BIN.TransferMode <> 3 \
AND (date_format(LT.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) ")
...
df_IngresoReproductora.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.IngresoReproductora")
print('carga IngresoReproductora',df_IngresoReproductora.count())
...
,MIN(fecha) descripfecha \
FROM {database_name_tmp}.IngresoReproductora \
WHERE pk_empresa = 1 and date_format(fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-10),'yyyyMM') \
...
print('carga df_ft_Reprod_Ingreso_Total',df_ft_Reprod_Ingreso_Total.count())
...
if datos_existentes6:
    ...
    data_after_delete6 = existing_data6.filter(~((date_format(col("DescripFecha"),"yyyy-MM-dd") >= fecha_str)))
    filtered_new_data6 = df_ft_Reprod_Ingreso_Total.filter((date_format(col("DescripFecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data6.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_Ingreso_TotalTemporal")
    ...
    final_data6_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_ingreso_total")
    ...
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_Ingreso_TotalTemporal')
    print(f"Tabla ft_Reprod_Ingreso_TotalTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_reprod_ingreso_total"
    }
...
df_Consumo = spark.sql(f"SELECT ComplexEntityNo,IRN_BFT,pk_diasvida,pk_semanavida,pk_alimentoF,pk_alimentoM,FmimFeedTypesIRN_VFFF,FmimFeedTypesIRN_VFFM,sum(FeedConsumed) Consumo \
                        FROM ( \
                              ...
                              FROM {database_name_tmp}.Reprod_Trans M \
                              LEFT JOIN {database_name_gl}.lk_alimento FALF ON CAST(FALF.IRN as varchar(50)) = CAST(M.FmimFeedTypesIRN_VFFF AS varchar (50)) \
                              LEFT JOIN {database_name_gl}.lk_alimento FALM ON CAST(FALM.IRN as varchar(50)) = CAST(M.FmimFeedTypesIRN_VFFM AS varchar (50)) \
                              LEFT JOIN {database_name_gl}.lk_diasvida D ON D.FirstHatchDateAge = M.FirstHatchDateAge \
                              LEFT JOIN {database_name_gl}.lk_semanavida S ON S.pk_semanavida = D.pk_semanavida \
                              WHERE (FeedConsumedF + FeedConsumedM) <> 0.00 \
                              AND (date_format(CAST(M.EventDate AS timestamp),'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) \
                              )A \
...
df_Consumo.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.consumo")
print('carga consumo', df_Consumo.count())
df_FechaMaximaTemp = spark.sql(f"select max(cast(EventDate as timestamp)) FechaMax, ComplexEntityNo from {database_name_tmp}.Reprod_Trans group by ComplexEntityNo")
df_FechaMaximaTemp.createOrReplaceTempView("FechaMaxima")
print('carga view FechaMaxima', df_FechaMaximaTemp.count())
df_FechaCapitalizacionMaximaTemp = spark.sql(f"select max(cast(DateCap as timestamp)) FechaCapitalizacionMax, ComplexEntityNo from {database_name_tmp}.Reprod_Trans \
                                               group by ComplexEntityNo")
df_FechaCapitalizacionMaximaTemp.createOrReplaceTempView("FechaCapitalizacionMaxima")
print('carga view FechaCapitalizacionMaxima',df_FechaCapitalizacionMaximaTemp.count())
df_Reprod_Reproductora = spark.sql(f"SELECT MO.IRN_BFT as BrimFieldTransIRN,MO.ProteinEntitiesIRN_BFT as ProteinEntitiesIRN,MO.ProteinGrowoutCodesIRN_VB as ProteinGrowoutCodesIRN,MO.ProteinFarmsIRN, \
                                    ...
                                    nvl(LAD.pk_administrador,(select pk_administrador from {database_name_gl}.lk_administrador where cadministrador='0'))pk_administrador, \
                                    nvl(FALF.pk_alimento,(select pk_alimento from {database_name_gl}.lk_alimento where calimento='0')) as pk_alimentof, \
                                    nvl(FALM.pk_alimento,(select pk_alimento from {database_name_gl}.lk_alimento where calimento='0')) as pk_alimentom, \
                                    nvl(PRO.pk_proveedor,(select pk_proveedor from {database_name_gl}.lk_proveedor where cproveedor=0)) as pk_proveedor, \
                                    nvl(LGE.pk_generacion,(select pk_generacion from {database_name_gl}.lk_generacion where cgeneracion=0)) as pk_generacion, \
                                    ...
                                    floor((FirstHatchDateAgesin7*1.0)/7) + ((FirstHatchDateAgesin7*1.0)%7)/10 as Edad, \
                                    ...
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 30 OR A.pk_alimentoM = 30)),0) AS AlimInicio \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 15 OR A.pk_alimentoM = 15)),0) AS AlimCrecimiento \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 64 OR A.pk_alimentoM = 64)),0) AS AlimPreReprod \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 72 OR A.pk_alimentoM = 72)),0) AS AlimReprodI \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 73 OR A.pk_alimentoM = 73)),0) AS AlimReprodII \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 71 OR A.pk_alimentoM = 71)),0) AS AlimReprodMacho \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 10 OR A.pk_alimentoM = 10)),0) AS PosturaCrecIRP \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 63 OR A.pk_alimentoM = 63)),0) AS PosturaPreInicioRP \
                                    ,MO.U_Ruptura_Aortica,MO.U_Ruptura_AorticaMale,MO.U_Bazo_Moteado,MO.U_Bazo_MoteadoMale \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 58 OR A.pk_alimentoM = 58)),0) AS PreInicialPavas \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 28 OR A.pk_alimentoM = 28)),0) AS InicialPavosRP \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF =  8 OR A.pk_alimentoM =  8)),0) AS CreIPavas \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 11 OR A.pk_alimentoM = 11)),0) AS CrecIIPavas \
                                    ,nvl((SELECT SUM(Consumo) FROM {database_name_tmp}.consumo A WHERE A.ComplexEntityNo = MO.ComplexEntityNo AND A.pk_diasvida = MO.FirstHatchDateAgesin7 AND A.IRN_BFT = MO.IRN_BFT AND (A.pk_alimentoF = 38 OR A.pk_alimentoM = 38)),0) AS MantPavas \
                                    ,MO.ProteinProductsIRN_VFFF,MO.ProteinProductsIRN_VFFM,MO.Price,FM.FechaMax,FCM.FechaCapitalizacionMax \
                                    from {database_name_tmp}.Reprod_Trans MO \
                                    LEFT JOIN {database_name_gl}.lk_tiempo LT ON date_format(LT.fecha,'yyyyMMdd') = date_format(cast(MO.EventDate as timestamp),'yyyyMMdd') \
                                    LEFT JOIN {database_name_si}.si_proteincostcenters PCC ON MO.ProteinCostCentersIRN_VB = PCC.IRN \
                                    ...
                                    LEFT JOIN {database_name_si}.si_proteinentities PE ON PE.IRN = MO.ProteinEntitiesIRN_BFT \
                                    ...
                                    LEFT JOIN {database_name_gl}.lk_tipoproducto TP ON tp.ntipoproducto=lpr.grupoproducto \
                                    ...
                                    LEFT JOIN {database_name_gl}.lk_administrador LAD ON LAD.IRN = CAST(PE.ProteinTechSupervisorsIRN AS VARCHAR(50)) \
                                    LEFT JOIN {database_name_gl}.lk_alimento FALF ON FALF.IRN = CAST(MO.FmimFeedTypesIRN_VFFF AS varchar (50)) \
                                    LEFT JOIN {database_name_gl}.lk_alimento FALM ON FALM.IRN = CAST(MO.FmimFeedTypesIRN_VFFM AS varchar (50)) \
                                    LEFT JOIN {database_name_si}.si_mvbimfarms MVBF ON MVBF.ProteinFarmsIRN = MO.ProteinFarmsIRN \
                                    LEFT JOIN {database_name_gl}.lk_proveedor PRO ON PRO.cproveedor = MVBF.VendorNo \
                                    LEFT JOIN {database_name_si}.si_bimstandardsdata BSD ON BSD.ProteinStandardVersionsIRN = PSV.IRN and BSD.age = MO.FirstHatchDateAge \
                                    LEFT JOIN {database_name_gl}.lk_generacion LGE ON MO.GenerationCode = LGE.cgeneracion \
...
df_Reprod_Reproductora.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Reproductora")
print('carga Reprod_Reproductora', df_Reprod_Reproductora.count())
...
df_stdcobbedad64Temp = spark.sql(f"select distinct ComplexEntityNo,Edad,U_FeedPerHenWK,U_FeedPerMaleWK,U_PercentProdTEWK,U_PercentProdHEWK,U_HenWeight,U_MaleWeight,U_TEPerHHACM,U_HEPerHHACM, \
                                  ...
                                  from {database_name_tmp}.Reprod_Reproductora \
...
df_stdcobbedad64Temp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.stdcobbedad64")
print('carga stdcobbedad64', df_stdcobbedad64Temp.count())
#Se actualiza los STD mayores de 64 con los datos de la tabla anterior
df_UPDReprod_Reproductora = spark.sql(f"SELECT BrimFieldTransIRN,ProteinEntitiesIRN,ProteinGrowoutCodesIRN,ProteinFarmsIRN, \
...
from {database_name_tmp}.Reprod_Reproductora A \
LEFT JOIN \
{database_name_tmp}.stdcobbedad64 B ON A.complexentityno = B.complexentityno")

additional_options = {
"path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temp/Reprod_Reproductora_new"
}
# 1️⃣ Crear DataFrame intermedio
df_UPDReprod_Reproductora.write \
    .mode("overwrite") \
    .format("parquet") \
    .options(**additional_options) \
    .saveAsTable(f"{database_name_tmp}.Reprod_Reproductora_new")

df_Reprod_Reproductora_nueva = spark.sql(f"""SELECT * from {database_name_tmp}.Reprod_Reproductora_new """)

# 2️⃣ Eliminar la tabla original (opcional, si se permite)
spark.sql(f"DROP TABLE IF EXISTS {database_name_tmp}.Reprod_Reproductora")

# 4️⃣ Volver a crear la tabla original con los datos actualizados
additional_options2 = {
"path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temp/Reprod_Reproductora"
}
df_Reprod_Reproductora_nueva.write \
    .format("parquet") \
...
spark.sql(f"DROP TABLE IF EXISTS {database_name_tmp}.Reprod_Reproductora_new")

print('carga UPD Reprod_Reproductora', df_Reprod_Reproductora_nueva.count())
...
df_stdrossedad65Temp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.stdrossedad65")
print('carga stdrossedad65', df_stdrossedad65Temp.count())
#Se actualiza los STD mayores de 65 con los datos de la tabla anterior
df_UPDReprod_Reproductora2 = spark.sql(f"SELECT A.BrimFieldTransIRN,A.ProteinEntitiesIRN,A.ProteinGrowoutCodesIRN,A.ProteinFarmsIRN, \
...
from {database_name_tmp}.Reprod_Reproductora A \
LEFT JOIN \
{database_name_tmp}.stdrossedad65 B ON A.complexentityno = B.complexentityno")

additional_options = {
"path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temp/Reprod_Reproductora_new2"
}
# 1️⃣ Crear DataFrame intermedio
df_UPDReprod_Reproductora2.write \
    .mode("overwrite") \
    .format("parquet") \
    .options(**additional_options) \
    .saveAsTable(f"{database_name_tmp}.Reprod_Reproductora_new2")

df_Reprod_Reproductora_nueva2 = spark.sql(f"""SELECT * from {database_name_tmp}.Reprod_Reproductora_new2 """)
...
df_Reprod_Reproductora_nueva2.write \
    .format("parquet") \
    .options(**additional_options2) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Reproductora")  
...
# 5️⃣ (Opcional) Eliminar la tabla temporal si ya no es neces
spark.sql(f"DROP TABLE IF EXISTS {database_name_tmp}.Reprod_Reproductora_new2")
print('carga UPD Reprod_Reproductora 2', df_Reprod_Reproductora_nueva2.count())
#Saca
...
FROM ( \
      ...
      FROM {database_name_si}.si_bimentityinventory BIN \
      LEFT JOIN {database_name_si}.si_mvproteinentities PE ON PE.IRN = BIN.ProteinEntitiesIRN \
      LEFT JOIN {database_name_gl}.lk_tiempo LT ON date_format(LT.fecha,'yyyy-MM-dd') = date_format(cast(EventDate as timestamp),'yyyy-MM-dd') \
      WHERE SourceCode in ('PmtsProcRecvTrans','Pmts Transfers') AND (date_format(CAST(EventDate AS timestamp),'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) \
...
df_SacaReproductoraTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SacaReproductora")
print('carga SacaReproductora', df_SacaReproductoraTemp.count())
...
FROM {database_name_tmp}.Reprod_Reproductora RE \
left join {database_name_tmp}.SacaReproductora SR on RE.ComplexEntityNo = SR.ComplexEntityNo and RE.pk_tiempo = SR.pk_tiempo \
...
df_ValoresParaAcumularTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ValoresParaAcumular")

print('carga ValoresParaAcumular', df_ValoresParaAcumularTemp.count())
...
df_AcumuladosTemp = spark.sql(f" WITH BSD AS ( \
    ...
    FROM {database_name_tmp}.ValoresParaAcumular \
) \
SELECT * FROM BSD \
...
df_AcumuladosTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Acumulados")
print('carga Acumulados', df_AcumuladosTemp.count())
...
df_ValoresParaAcumularGalponTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ValoresParaAcumularGalpon")

print('carga ValoresParaAcumularGalpon', df_ValoresParaAcumularGalponTemp.count())
...
FROM {database_name_tmp}.ValoresParaAcumularGalpon \
) \
SELECT * FROM BSD \
...
df_AcumuladosGalponTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.AcumuladosGalpon")

print('carga AcumuladosGalpon', df_AcumuladosGalponTemp.count())
...
df_ValoresParaAcumularLoteTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ValoresParaAcumularLote")

print('carga ValoresParaAcumularLote', df_ValoresParaAcumularLoteTemp.count())
...
SUM(TransSalM) OVER ( \
    ...
    FROM {database_name_tmp}.ValoresParaAcumularLote \
) \
SELECT * FROM BSD \
...
df_AcumuladosLoteTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.AcumuladosLote")

print('carga AcumuladosLote', df_AcumuladosLoteTemp.count())
...
from {database_name_tmp}.Acumulados A \
left join {database_name_tmp}.AcumuladosGalpon B ON substring(A.complexentityno,1,(length(A.complexentityno)-3)) = B.ComplexEntityNoGalpon AND A.pk_tiempo = B.pk_tiempo \
left join {database_name_tmp}.AcumuladosLote   C ON substring(A.complexentityno,1,(length(A.complexentityno)-6)) = C.ComplexEntityNoLote   AND A.pk_tiempo = C.pk_tiempo")
...
df_Reprod_Acumulados.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Acumulados")

print('carga Reprod_Acumulados', df_Reprod_Acumulados.count())
...
FROM {database_name_tmp}.Reprod_Reproductora FR \
LEFT JOIN {database_name_gl}.ft_reprod_ingreso_total FIR on FR.ComplexEntityNo = FIR.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.SacaReproductora TSR on FR.ComplexEntityNo = TSR.ComplexEntityNo and FR.pk_tiempo = TSR.pk_tiempo \
LEFT JOIN {database_name_tmp}.Reprod_Acumulados TA on FR.ComplexEntityNo = TA.ComplexEntityNo and FR.pk_tiempo = TA.pk_tiempo \
WHERE FR.GRN = 'P' \
AND FR.pk_division in (4,5,2)  \
and (date_format(FR.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) \
...
df_Reprod_Mort_DiarioTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Mort_Diario")

print('carga Reprod_Mort_Diario', df_Reprod_Mort_DiarioTemp.count())
#Inserta los últimos 4 meses de la tabla
df_ft_Reprod_Mort_Diario =spark.sql(f"SELECT MRD.pk_tiempo,MRD.pk_empresa,MRD.pk_division,MRD.pk_zona,MRD.pk_subzona,MRD.pk_plantel,MRD.pk_lote,MRD.pk_galpon, \
...
MRD.U_PEDespatarradosH,MRD.U_PEDespatarradosM,MRD.ListaPadre,MRD.U_RupturaAorticaH,MRD.U_RupturaAorticaM,MRD.U_BazoMoteadoH,MRD.U_BazoMoteadoM,MRD.fecha DescripFecha \
FROM {database_name_tmp}.Reprod_Mort_Diario MRD \
WHERE (date_format(fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-10),'yyyyMM')) \
order by pk_tiempo")

print('carga df_ft_Reprod_Mort_Diario', df_ft_Reprod_Mort_Diario.count())
fechaactual = datetime.now().replace(day=1)
fecha_menos = fechaactual - relativedelta(months=4)
fecha_str = fecha_menos.strftime("%Y-%m-%d")
...
if datos_existentes7:
    ...
    data_after_delete7 = existing_data7.filter(~((date_format(col("DescripFecha"),"yyyy-MM-dd") >= fecha_str) ))
    filtered_new_data7 = df_ft_Reprod_Mort_Diario.filter((date_format(col("DescripFecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data7.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_Mort_DiarioTemporal")
    #schema = existing_data.schema
    ...
    final_data7_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_mort_diario")
    ...
    print(f"Total de registros en la tabla ft_Reprod_Mort_Diario : {cant_total7}")
     #Limpia la ubicación temporal
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})    
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_Mort_DiarioTemporal')
    print(f"Tabla ft_Reprod_Mort_DiarioTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
...
print('carga ft_reprod_mort_diario', df_ft_Reprod_Mort_Diario.count())
# Reproductora
...
FROM {database_name_si}.si_mvproteinentities PE \
LEFT JOIN {database_name_gl}.lk_especie LES ON LES.IRN = CAST(PE.ProteinBreedCodesIRN AS VARCHAR(50)) \
...
df_LineaGeneticaXNivelTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.LineaGeneticaXNivel")

print('carga LineaGeneticaXNivel', df_LineaGeneticaXNivelTemp.count())
...
df_ValoresParaAcumularProdTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ValoresParaAcumularProd")

print('carga ValoresParaAcumularProd', df_ValoresParaAcumularProdTemp.count())
#Se genera los valores acumulados por corral
...
FROM {database_name_tmp}.ValoresParaAcumularProd \
) \
SELECT * FROM BSD \
ORDER BY ComplexEntityNo, pk_tiempo")
...
df_AcumuladosProdTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.AcumuladosProd")

print('carga AcumuladosProd', df_AcumuladosProdTemp.count())
...
df_ValoresParaAcumularGalponProdTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ValoresParaAcumularGalponProd")

print('carga ValoresParaAcumularGalponProd', df_ValoresParaAcumularGalponProdTemp.count())
#Se genera los valores acumulados por galpon
...
FROM {database_name_tmp}.ValoresParaAcumularGalponProd \
) \
SELECT * FROM BSD \
ORDER BY ComplexEntityNoGalpon, pk_tiempo")
...
df_AcumuladosGalponProdTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.AcumuladosGalponProd")

print('carga AcumuladosGalponProd', df_AcumuladosGalponProdTemp.count())
...
df_ValoresParaAcumularLoteProdTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ValoresParaAcumularLoteProd")

print('carga ValoresParaAcumularLoteProd', df_ValoresParaAcumularLoteProdTemp.count())
#Se genera los valores acumulados por lote
...
FROM {database_name_tmp}.ValoresParaAcumularLoteProd \
) \
SELECT * FROM BSD \
ORDER BY ComplexEntityNoLote, pk_tiempo")
...
df_AcumuladosLoteProdTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.AcumuladosLoteProd")

print('carga AcumuladosLoteProd', df_AcumuladosLoteProdTemp.count())
#Se crea e inserta los valores acumulados por corral, galpon y lote
...
from {database_name_tmp}.AcumuladosProd A \
left join {database_name_tmp}.AcumuladosGalponProd B ON substring(A.complexentityno,1,(length(A.complexentityno)-3)) = B.ComplexEntityNoGalpon AND A.pk_tiempo = B.pk_tiempo \
left join {database_name_tmp}.AcumuladosLoteProd C ON substring(A.complexentityno,1,(length(A.complexentityno)-6)) = C.ComplexEntityNoLote AND A.pk_tiempo = C.pk_tiempo")
...
df_AcumuladosReproductoraProdTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.AcumuladosReproductoraProd")

print('carga AcumuladosReproductoraProd', df_AcumuladosReproductoraProdTemp.count())
#Convierta las fechas en valores númericos
df_TablaFNTemp = spark.sql(f"""
SELECT 
pk_tiempo, 
fecha, 
substring(complexentityno, 1, (length(complexentityno) - 6)) AS ComplexEntityNo, 
DATEDIFF(CAST(EventDate AS TIMESTAMP), DATE '1900-01-01') AS NumIdfecha, 
FechaNacimiento, 
DATEDIFF(CAST(fechanacimiento AS TIMESTAMP), DATE '1900-01-01') AS NumFechaNacimiento
FROM {database_name_tmp}.Reprod_Reproductora
ORDER BY pk_tiempo
""")
...
df_TablaFNTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.TablaFN")

print('carga TablaFN', df_TablaFNTemp.count())
#Saca un promedio de todas las fechas de nacimiento por lote 
df_FNPromLoteTemp = spark.sql(f"""
    SELECT 
        ComplexEntityNo, 
        CAST(DATEDIFF(
            DATE_ADD(DATE '1900-01-01', CAST(FechaNacimientoPromLote AS INTEGER) ), 
            DATE '1900-01-01'
        ) AS BIGINT) AS FechaNacimientoPromLote
    FROM ( 
        SELECT ComplexEntityNo, AVG(NumFechaNacimiento * 1.0) AS FechaNacimientoPromLote 
        FROM {database_name_tmp}.TablaFN 
        GROUP BY ComplexEntityNo 
    ) A
""")
...
df_FNPromLoteTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.FNPromLote")

print('carga FNPromLote', df_FNPromLoteTemp.count())
...
FLOOR(CASE 
    WHEN AVG(B.FechaNacimientoPromLote) IS NULL THEN 0 
    WHEN AVG(B.FechaNacimientoPromLote) = -32 THEN 0 
    ELSE (DATEDIFF(
            DATE_ADD('1900-01-01', CAST(AVG(A.NumIdfecha) AS INT)), 
            DATE_ADD('1900-01-01', CAST(AVG(B.FechaNacimientoPromLote) AS INT))
        ) + 6) / 7 
END) AS pk_diavidalote
from {database_name_tmp}.TablaFN A 
left join {database_name_tmp}.FNPromLote B on A.complexentityno = B.complexentityno 
...
df_Edad1Temp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Edad1")

print('carga Edad1', df_Edad1Temp.count())
...
df_STDLoteTemp = spark.sql(f"SELECT DISTINCT pk_semanavida,ComplexEntityNo,U_FeedPerHenWK ,U_FeedPerMaleWK ,U_PercentProdTEWK ,U_PercentProdHEWK , \
                            ...
                            ,U_HenMortWK,U_MaleMortWK,U_HenMortACM,U_MaleMortACM,U_EggWeight \
                            FROM {database_name_tmp}.Reprod_Reproductora PE ")
...
df_STDLoteTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.STDLote")

print('carga STDLote', df_STDLoteTemp.count())
...
from {database_name_tmp}.Reprod_Reproductora FR \
LEFT JOIN {database_name_gl}.ft_reprod_mort_diario MR on FR.ComplexEntityNo = MR.ComplexEntityNo and FR.pk_diasvida = MR.pk_diasvida and FR.pk_tiempo = MR.pk_tiempo \
LEFT JOIN {database_name_tmp}.AcumuladosReproductoraProd TA on FR.ComplexEntityNo = TA.ComplexEntityNo and FR.pk_tiempo = TA.pk_tiempo \
LEFT JOIN {database_name_tmp}.Edad1 E on substring(FR.complexentityno,1,(length(FR.complexentityno)-6)) = E.complexentityno and Fr.pk_tiempo = E.pk_tiempo \
LEFT JOIN {database_name_tmp}.STDLote ST on FR.ComplexEntityNo = ST.ComplexEntityNo and cast(E.pk_diavidalote as int) = cast(ST.pk_semanavida as int) \
WHERE FR.GRN = 'P' \
...
df_Reprod_DiarioTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Diario")

print('carga Reprod_Diario', df_Reprod_DiarioTemp.count())
df_maxStdGADiaH = spark.sql(f"SELECT Complexentityno,pk_semanavida, max(StdGADiaH) StdGADiaH \
FROM {database_name_tmp}.Reprod_Diario group by Complexentityno,pk_semanavida")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_maxStdGADiaH.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.MaxStdGADiaH")
print('carga MaxStdGADiaH', df_maxStdGADiaH.count())
#df_rounded_filtered = spark.sql(f"""
#    SELECT complexentityno,floor(NULLIF(((ConsAlimH/NULLIF(SaldoH,0)))*1000,0) * 100) / 100 AS rounded_value
#    FROM default.Reprod_Diario
#    WHERE complexentityno = 'C015-0779-04-01' AND date_format(fecha,'yyyyMM') = '202406' order by 2 """)
#df_rounded_filtered.show()
df_ft_Reprod_Diario = spark.sql(f"select RE.pk_tiempo,RE.pk_empresa,RE.pk_division,RE.pk_zona,RE.pk_subzona,RE.pk_plantel,RE.pk_lote, \
...
,RE.SelecH \
,RE.SelecM \
,ROUND(cast(NULLIF(((RE.ConsAlimH/NULLIF(RE.SaldoH,0)))*1000,0) as decimal(10,2)) ,2) GADH \
,RE.StdGADiaH \
...
,RE.CrecIIPavas \
,RE.MantPavas \
,cast(null as decimal(38,3)) u_coevarhimix \
,cast(null as decimal(38,3)) u_coevarhilay \
,RE.fecha descripfecha \
from {database_name_tmp}.Reprod_Diario RE  left join \
{database_name_tmp}.Reprod_Diario RE2 on RE2.Complexentityno = RE.Complexentityno AND RE2.pk_diasvida = RE.pk_diasvida -1 \
left join {database_name_tmp}.MaxStdGADiaH A ON A.Complexentityno = RE.Complexentityno  AND A.pk_semanavida = RE.pk_semanavida -1 \
where (date_format(RE.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-10),'yyyyMM')) and RE.CondicionH < 5.00000")

print('carga df_ft_Reprod_Diario', df_ft_Reprod_Diario.count())
fechaactual = datetime.now().replace(day=1)
...
if datos_existentes8:
    ...
    data_after_delete8 = existing_data8.filter(~((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str)))
    filtered_new_data8 = df_ft_Reprod_Diario.filter((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data8.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_DiarioTemporal")
    ...
    final_data8_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_diario")
    ...
    print(f"Total de registros en la tabla ft_Reprod_Diario : {cant_total8}")
     #Limpia la ubicación temporal
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_DiarioTemporal')
    print(f"Tabla ft_Reprod_DiarioTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_reprod_diario"
    }
...
print('carga ft_Reprod_Diario', df_ft_Reprod_Diario.count())
...
df_Reprod_Galpon_Diario_CriaTemp = spark.sql(f"select \
                                              ...
                                              max(nvl(FIR.pk_especie,(select pk_especie from {database_name_gl}.lk_especie where cespecie='0'))) pk_especieg, \
                                              ...
                                              from {database_name_tmp}.Reprod_Diario RD \
                                              LEFT JOIN {database_name_tmp}.LineaGeneticaXNivel FIR on substring(RD.complexentityno,1,(length(RD.complexentityno)-3)) = FIR.ComplexEntityNo \
                                              where pk_etapa = 1 \
...
df_Reprod_Galpon_Diario_CriaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Galpon_Diario_Cria")
print('carga Reprod_Galpon_Diario_Cria', df_Reprod_Galpon_Diario_CriaTemp.count())
...
df_Reprod_Galpon_Diario_LevanteTemp = spark.sql(f"select \
                                                 ...
                                                 MAX(RD.FechaMax) FechaMax \
                                                 from {database_name_tmp}.Reprod_Diario RD \
                                                 LEFT JOIN {database_name_tmp}.LineaGeneticaXNivel FIR on substring(rtrim(ltrim(RD.complexentityno)),1,(length(RD.complexentityno)-3)) = rtrim(ltrim(FIR.complexentityno)) \
                                                 where pk_etapa = 3 \
...
df_Reprod_Galpon_Diario_LevanteTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Galpon_Diario_Levante")
print('carga Reprod_Galpon_Diario_Levante', df_Reprod_Galpon_Diario_LevanteTemp.count())
#Inserta los últimos 4 meses de la tabla
...
df_ft_Reprod_Galpon_Diario1 = spark.sql(f"WITH Reprod_Galpon_Diario_Cria_Filtrado AS ( \
    ...
    FROM \
    {database_name_tmp}.Reprod_Galpon_Diario_Cria RPG \
    WHERE \
...
RPG.CrecIIPavas, \
RPG.MantPavas, \
RPG.fecha descripfecha \
FROM \
CalculosAdicionales RPG")
print('carga df_ft_Reprod_Galpon_Diario1', df_ft_Reprod_Galpon_Diario1.count())
...
END AS PorcViabProdM \
FROM \
{database_name_tmp}.Reprod_Galpon_Diario_Levante RPG \
WHERE \
(date_format(RPG.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')) \
...
print('carga df_ft_Reprod_Galpon_Diario2', df_ft_Reprod_Galpon_Diario1.count())
print('carga df_ft_Reprod_Galpon_Diario', df_ft_Reprod_Galpon_Diario.count())
fechaactual = datetime.now().replace(day=1)
fecha_menos = fechaactual - relativedelta(months=4)
fecha_str = fecha_menos.strftime("%Y-%m-%d")
...
if datos_existentes9:
    ...
    data_after_delete9 = existing_data9.filter(~((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str) ))
    filtered_new_data9 = df_ft_Reprod_Galpon_Diario.filter((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data9.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_Galpon_DiarioTemporal")
    ...
    final_data9_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_galpon_diario")
    ...
    print(f"Total de registros en la tabla ft_Reprod_Galpon_Diario : {cant_total9}")
     #Limpia la ubicación temporal
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_Galpon_DiarioTemporal')
    print(f"Tabla ft_Reprod_Galpon_DiarioTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_reprod_galpon_diario"
    }
...
print('carga ft_Reprod_Galpon_Diario', df_ft_Reprod_Galpon_Diario.count())
...
df_Reprod_Galpon_Edad_CriaTemp = spark.sql(f"select \
                                            ...
                                            from {database_name_tmp}.Reprod_Diario A \
                                            LEFT JOIN {database_name_tmp}.LineaGeneticaXNivel FIR on substring(A.complexentityno,1,(length(A.complexentityno)-3)) = FIR.ComplexEntityNo \
                                            left join {database_name_gl}.lk_tiempo t on t.pk_tiempo = A.pk_tiempo \
                                            where pk_etapa = 1 \
...
df_Reprod_Galpon_Edad_CriaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Galpon_Edad_Cria")
print('carga Reprod_Galpon_Edad_Cria', df_Reprod_Galpon_Edad_CriaTemp.count())
...
df_Reprod_Galpon_Edad_LevanteTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Galpon_Edad_Levante")
print('carga Reprod_Galpon_Edad_Levante', df_Reprod_Galpon_Edad_LevanteTemp.count())
#Inserta los últimos 4 meses de la tabla
...
df_ft_Reprod_Galpon_Semana1 = spark.sql(f"select \
                                        ...
                                        ,RPG.TransIngM \
                                        ,RPG.TransSalM \
                                        ,RPG.TransIngAcumHGalpon transingacumh \
                                        ,RPG.TransSalAcumHGalpon transsalacumh \
                                        ,RPG.TransIngAcumMGalpon transingacumm \
                                        ,RPG.TransSalAcumMGalpon transsalacumm \
                                        ...
                                        ,RPG.fecha descripfecha\
                                        from {database_name_tmp}.Reprod_Galpon_Edad_Cria RPG \
                                        LEFT JOIN {database_name_tmp}.Reprod_Galpon_Edad_Cria A on  A.Complexentityno = RPG.Complexentityno  AND A.pk_semanavida = RPG.pk_semanavida -1 \
                                        where (date_format(RPG.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM'))")

print('carga df_ft_Reprod_Galpon_Semana1', df_ft_Reprod_Galpon_Semana1.count())
df_ft_Reprod_Galpon_Semana2 = spark.sql(f"select \
                                        ...
                                        ,ROW_NUMBER()  OVER(PARTITION BY RPG.ComplexEntityNo ORDER BY RPG.pk_semanavida ASC) RowNumber \
                                        ,RPG.fecha descripfecha\
                                        from {database_name_tmp}.Reprod_Galpon_Edad_Levante RPG \
                                        LEFT JOIN {database_name_tmp}.Reprod_Galpon_Edad_Levante A on  A.Complexentityno = RPG.Complexentityno  AND A.pk_semanavida = RPG.pk_semanavida -1 \
                                        where (date_format(RPG.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')) and RPG.CondicionH < 5.000000")

df_ft_Reprod_Galpon_Semana =df_ft_Reprod_Galpon_Semana1.union(df_ft_Reprod_Galpon_Semana2)
  
print('carga df_ft_Reprod_Galpon_Semana2', df_ft_Reprod_Galpon_Semana2.count())
print('carga df_ft_Reprod_Galpon_Semana', df_ft_Reprod_Galpon_Semana.count())
fechaactual = datetime.now().replace(day=1)
fecha_menos = fechaactual - relativedelta(months=4)
fecha_str = fecha_menos.strftime("%Y-%m-%d")
...
if datos_existentes10:
    ...
    data_after_delete10 = existing_data10.filter(~((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str)))
    filtered_new_data10 = df_ft_Reprod_Galpon_Semana.filter((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data10.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_Galpon_SemanaTemporal")
    ...
    final_data10_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_galpon_semana")
    ...
    print(f"Total de registros en la tabla ft_Reprod_Galpon_Semana : {cant_total10}")
     #Limpia la ubicación temporal
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_Galpon_SemanaTemporal')
    print(f"Tabla ft_Reprod_Galpon_SemanaTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_reprod_galpon_semana"
    }
    df_ft_Reprod_Galpon_Semana.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_galpon_semana")
print('carga ft_Reprod_Galpon_Semana', df_ft_Reprod_Galpon_Semana.count())    
df_PonderadosTemp = spark.sql(f"select \
                               ...
                               ,nvl(sum(PorcHvoPisoXSaldoH) / nullif(sum(SaldoHDePorcHvoPiso),0),0) PorcHvoPisoPond \
                                   from ( \
                                         ...
                                         from {database_name_tmp}.Reprod_Diario \
                                         ) A \
...
df_PonderadosTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Ponderados")
print('carga Ponderados', df_PonderadosTemp.count())
...
df_CapitalizacionXLoteTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.CapitalizacionXLote")
print('carga CapitalizacionXLote', df_CapitalizacionXLoteTemp.count())
...
df_Reprod_Lote_Diario_CriaTemp = spark.sql(f"select \
                                            ...
                                            max(nvl(FIR.pk_especie,(select pk_especie from {database_name_gl}.lk_especie where cespecie='0'))) pk_especiel, \
                                            min(pk_estado) pk_estado, \
                                            RP.pk_administrador, \
                                            RP.pk_proveedor, \
                                            pk_generacion, \
                                            ...
                                            from {database_name_tmp}.Reprod_Diario RP \
                                            left join {database_name_tmp}.Ponderados P on RP.pk_tiempo = P.pk_tiempo and cast(RP.pk_lote as integer) = cast(P.pk_lote as integer) \
                                            LEFT JOIN {database_name_tmp}.LineaGeneticaXNivel FIR on substring(RP.complexentityno,1,(length(RP.complexentityno)-6)) = FIR.ComplexEntityNo \
                                            left join {database_name_tmp}.CapitalizacionXLote CL on substring(RP.complexentityno,1,(length(RP.complexentityno)-6)) = CL.Lote and RP.pk_tiempo = CL.FechaCap\
                                            left join {database_name_tmp}.CapitalizacionXLote CL2 on substring(RP.complexentityno,1,(length(RP.complexentityno)-6)) = CL2.Lote \
...
df_Reprod_Lote_Diario_CriaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Lote_Diario_Cria")
print('carga Reprod_Lote_Diario_Cria', df_Reprod_Lote_Diario_CriaTemp.count())
df_maxStdGADiaHReprodLoteDiario = spark.sql(f"SELECT Complexentityno,pk_diavidalote, max(StdGADiaH) StdGADiaH \
FROM {database_name_tmp}.Reprod_Lote_Diario_Cria group by Complexentityno,pk_diavidalote")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_maxStdGADiaHReprodLoteDiario.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.MaxStdGADiaHReprodLoteDiario")
print('carga MaxStdGADiaHReprodLoteDiario', df_maxStdGADiaHReprodLoteDiario.count())
#Inserta los últimos 4 meses de la tabla
...
df_ft_Reprod_Lote_Diario = spark.sql(f"select \
                                    ...
                                    ,RPG.IngresoMAcum \
                                    ,RPG.fecha descripfecha \
                                    from {database_name_tmp}.Reprod_Lote_Diario_Cria RPG  \
                                    left join {database_name_tmp}.Reprod_Lote_Diario_Cria RPG2 on  RPG2.Complexentityno = RPG.Complexentityno AND  RPG2.pk_tiempo =  RPG.pk_tiempo - 1 and RPG2.pk_administrador = RPG.pk_administrador \
                                    left join {database_name_tmp}.MaxStdGADiaHReprodLoteDiario A ON A.Complexentityno = RPG.Complexentityno  AND A.pk_diavidalote = RPG.pk_diavidalote -1 \
                                    where (date_format(RPG.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')) \
                                    and RPG.CondicionH < 5.000000")

print('df_ft_Reprod_Lote_Diario', df_ft_Reprod_Lote_Diario.count())
fechaactual = datetime.now().replace(day=1)
fecha_menos = fechaactual - relativedelta(months=4)
fecha_str = fecha_menos.strftime("%Y-%m-%d")
...
if datos_existentes11:
    ...
    data_after_delete11 = existing_data11.filter(~((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str)))
    filtered_new_data11 = df_ft_Reprod_Lote_Diario.filter((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data11.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_Lote_DiarioTemporal")
    ...
    final_data11_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_lote_diario")
    ...
    print(f"Total de registros en la tabla ft_Reprod_Lote_Diario : {cant_total11}")
     #Limpia la ubicación temporal
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_Lote_DiarioTemporal')
    print(f"Tabla ft_Reprod_Lote_DiarioTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_reprod_lote_diario"
    }
    df_ft_Reprod_Lote_Diario.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_lote_diario")
print('ft_Reprod_Lote_Diario', df_ft_Reprod_Lote_Diario.count())    
#Ponderados
...
df_Ponderados1Temp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Ponderados1")
print('carga Ponderados1', df_Ponderados1Temp.count())
...
df_Reprod_Lote_Sem_CriaTemp = spark.sql(f"select \
                                        ...
                                        from {database_name_tmp}.Reprod_Lote_Diario_Cria A \
                                        left join {database_name_tmp}.Ponderados1 P on A.pk_tiempo = P.pk_tiempo and cast(A.pk_lote as integer) = cast(P.pk_lote as integer) \
                                        LEFT JOIN {database_name_tmp}.LineaGeneticaXNivel FIR on A.ComplexEntityNo = FIR.ComplexEntityNo \
                                        where pk_etapa = 1 \
...
df_Reprod_Lote_Sem_CriaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Lote_Sem_Cria")
print('carga Reprod_Lote_Sem_Cria', df_Reprod_Lote_Sem_CriaTemp.count())
...
max(A.pk_especie) pk_especie,nvl(FIR.pk_especie,(select pk_especie from {database_name_gl}.lk_especie where cespecie='0')) pk_especiel, \
...
df_Reprod_Lote_Sem_LevanteTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Reprod_Lote_Sem_Levante")
print('carga Reprod_Lote_Sem_Levante' , df_Reprod_Lote_Sem_LevanteTemp.count())
df_maxStdGADiaHReprodLoteSemana1 = spark.sql(f"SELECT Complexentityno,pk_semanavida, max(StdGADiaH) StdGADiaH FROM \
{database_name_tmp}.Reprod_Lote_Sem_Cria group by Complexentityno,pk_semanavida")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_maxStdGADiaHReprodLoteSemana1.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.MaxStdGADiaHReprodLoteSemana1")
print('carga MaxStdGADiaHReprodLoteSemana1' , df_maxStdGADiaHReprodLoteSemana1.count())
df_ft_Reprod_Lote_Semana1 = spark.sql(f"select \
                                        ...
                                        ,nvl((RPG.ConsAlimH + RPG.ConsAlimM) / nullif(RPG.ProdHI,0),0) AlimProdHI \
                                        ,RPG.fecha descripfecha \
                                        from {database_name_tmp}.Reprod_Lote_Sem_Cria RPG \
                                        left join {database_name_tmp}.Reprod_Lote_Sem_Cria RPG2 on RPG2.Complexentityno = RPG.Complexentityno AND RPG2.pk_semanavida = RPG.pk_semanavida -1 and RPG2.pk_administrador = RPG.pk_administrador \
                                        left join {database_name_tmp}.MaxStdGADiaHReprodLoteSemana1 A on A.Complexentityno = RPG.Complexentityno  AND A.pk_semanavida = RPG.pk_semanavida -1 \
                                        where (date_format(RPG.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM'))")
print('df_ft_Reprod_Lote_Semana1', df_ft_Reprod_Lote_Semana1.count())
df_maxStdGADiaHReprodLoteSemana2 = spark.sql(f"SELECT Complexentityno,pk_semanavida, max(StdGADiaH) StdGADiaH \
FROM {database_name_tmp}.Reprod_Lote_Sem_Cria group by Complexentityno,pk_semanavida")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_maxStdGADiaHReprodLoteSemana2.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.MaxStdGADiaHReprodLoteSemana2")
print('carga MaxStdGADiaHReprodLoteSemana2', df_maxStdGADiaHReprodLoteSemana2.count())
df_ft_Reprod_Lote_Semana2 =spark.sql(f"select \
                                    ...
                                    ,nvl((RPG.ConsAlimH + RPG.ConsAlimM) / nullif(RPG.ProdHI,0.000000),0.000000) AlimProdHI \
                                    ,RPG.fecha descripfecha\
                                    from {database_name_tmp}.Reprod_Lote_Sem_Levante RPG \
                                    left join {database_name_tmp}.Reprod_Lote_Sem_Levante RPG2 on RPG2.Complexentityno = RPG.Complexentityno AND RPG2.pk_semanavida = RPG.pk_semanavida -1 and RPG2.pk_administrador = RPG.pk_administrador \
                                    left join {database_name_tmp}.MaxStdGADiaHReprodLoteSemana2 A on A.Complexentityno = RPG.Complexentityno  AND A.pk_semanavida = RPG.pk_semanavida -1 \
                                    where (date_format(RPG.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')) \
                                    and RPG.CondicionH <5.000000")

df_ft_Reprod_Lote_Semana = df_ft_Reprod_Lote_Semana1.union(df_ft_Reprod_Lote_Semana2)
print('carga df_ft_Reprod_Lote_Semana2', df_ft_Reprod_Lote_Semana2.count())
print('carga df_ft_Reprod_Lote_Semana', df_ft_Reprod_Lote_Semana.count())
fechaactual = datetime.now().replace(day=1)
fecha_menos = fechaactual - relativedelta(months=4)
fecha_str = fecha_menos.strftime("%Y-%m-%d")
...
if datos_existentes12:
    ...
    data_after_delete12 = existing_data12.filter(~((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str)))
    filtered_new_data12 = df_ft_Reprod_Lote_Semana.filter((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data12.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_Lote_SemanaTemporal")
    ...
    final_data12_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_lote_semana")
    ...
    print(f"Total de registros en la tabla ft_Reprod_Lote_Semana : {cant_total12}")
     #Limpia la ubicación temporal
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_Lote_SemanaTemporal')
    print(f"Tabla ft_Reprod_Lote_SemanaTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")  
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_reprod_lote_semana"
    }
    df_ft_Reprod_Lote_Semana.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_lote_semana")
print('carga ft_Reprod_Lote_Semana', df_ft_Reprod_Lote_Semana.count())    
# Ponderados Galpon
df_PonderadoGalponMensualTemp = spark.sql(f"select \
                                           ...
                                           from ( \
                                                ...
                                                from {database_name_gl}.ft_reprod_galpon_diario a\
...
df_PonderadoGalponMensualTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.PonderadoGalponMensual")
print('carga PonderadoGalponMensual', df_PonderadoGalponMensualTemp.count())
#Inserta los últimos 4 meses de la tabla
...
df_ft_Reprod_Galpon_Mensual = spark.sql(f"select \
                                        ...
                                        ,max(a.descripfecha) descripfecha \
                                        from {database_name_gl}.ft_reprod_galpon_diario A \
                                        left join {database_name_gl}.lk_tiempo B on A.pk_tiempo = B.pk_tiempo \
                                        left join {database_name_gl}.lk_mes C on B.idmes = C.idmes \
                                        left join {database_name_tmp}.PonderadoGalponMensual D on C.idmes = D.fecha and A.pk_lote = D.pk_lote and A.pk_galpon = D.pk_galpon \
                                        where (date_format(a.descripfecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')) \
                                        group by C.idmes,A.pk_empresa,A.pk_division,A.pk_zona,A.pk_subzona,A.pk_plantel,A.pk_lote,A.pk_galpon,A.pk_etapa \
                                                ,A.pk_standard,A.pk_tipoproducto,A.pk_especie,A.pk_estado,A.pk_administrador,A.pk_proveedor,A.pk_generacion \
                                                ,A.ComplexEntityNo,A.FechaNacimiento,A.FechaCap \
                                                order by MAX(A.pk_tiempo)")
print('carga df_ft_Reprod_Galpon_Mensual', df_ft_Reprod_Galpon_Mensual.count())
fechaactual = datetime.now().replace(day=1)
fecha_menos = fechaactual - relativedelta(months=4)
...
if datos_existentes13:
    ...
    data_after_delete13 = existing_data13.filter(~((col("idmes") >= fecha_str)))
    filtered_new_data13 = df_ft_Reprod_Galpon_Mensual.filter((col("idmes") >= fecha_str))
    ...
    final_data13.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_Galpon_MensualTemporal")
    ...
    final_data13_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_galpon_mensual")
    ...
    print(f"Total de registros en la tabla ft_Reprod_Galpon_Mensual : {cant_total13}")
     #Limpia la ubicación temporal
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_Galpon_MensualTemporal')
    print(f"Tabla ft_Reprod_Galpon_MensualTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_reprod_galpon_mensual"
    }
    df_ft_Reprod_Galpon_Mensual.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_galpon_mensual")
print('carga ft_Reprod_Galpon_Mensual', df_ft_Reprod_Galpon_Mensual.count())    
#Ponderados Lote
df_PonderadoLoteMensualTemp = spark.sql(f"select \
                                        ...
                                        from    ( \
                                                select \
                                                    ...
                                                    ,(UnifM*SaldoM) UnifMXSaldoM \
                                                    ,case when UnifM>0 then SaldoM else 0 end SaldoMDeUnifM \
                                                    from {database_name_gl}.ft_reprod_lote_diario a\
                                                    left join {database_name_gl}.lk_tiempo b on a.pk_tiempo=b.pk_tiempo \
                                                ) A \
...
df_PonderadoLoteMensualTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.PonderadoLoteMensual")
print('carga PonderadoLoteMensual', df_PonderadoLoteMensualTemp.count())
df_ft_Reprod_Lote_Mensual= spark.sql(f"select \
                                    ...
                                    ,SUM(A.U_PEErrorSexoH) U_PEErrorSexoH \
                                    ,SUM(A.U_PEErrorSexoM) U_PEErrorSexoM \
                                    ,max(b.fecha) descripfecha \
                                    from {database_name_gl}.ft_reprod_lote_diario A \
                                    left join {database_name_gl}.lk_tiempo B on A.pk_tiempo = B.pk_tiempo \
                                    left join {database_name_gl}.lk_mes C on B.idmes = C.idmes \
                                    left join {database_name_tmp}.PonderadoLoteMensual D on C.idmes = D.fecha and A.pk_lote = D.pk_lote  \
                                    where (date_format(A.descripfecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')) \
...
print('carga df_ft_Reprod_Lote_Mensual', df_ft_Reprod_Lote_Mensual.count())
fechaactual = datetime.now().replace(day=1)
fecha_menos = fechaactual - relativedelta(months=4)
fecha_str = fecha_menos.strftime("%Y%m")
...
if datos_existentes14:
    ...
    final_data14.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_Lote_MensualTemporal")
    ...
    final_data14_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_lote_mensual")
    ...
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_Lote_MensualTemporal')
    print(f"Tabla ft_Reprod_Lote_MensualTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_reprod_lote_mensual"
    }
    df_ft_Reprod_Lote_Mensual.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_lote_mensual")
print('carga ft_Reprod_Lote_Mensual', df_ft_Reprod_Lote_Mensual.count())    
df_MaximoCorralTemp=spark.sql(f"select max(pk_tiempo) pk_tiempomax,min(pk_tiempo) pk_tiempomin,pk_empresa,pk_plantel,pk_lote,pk_galpon,pk_sexo,ComplexEntityNo,pk_etapa \
                               from {database_name_gl}.ft_reprod_diario A \
...
df_MaximoCorralTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.MaximoCorral")
print('carga MaximoCorral', df_MaximoCorralTemp.count())
df_PonderadoLiquidacionDiarioTemp=spark.sql(f"select ComplexEntityNo,pk_etapa \
                                             ...
                                             from ( \
                                                    ...
                                                    from {database_name_gl}.ft_reprod_diario \
                                                  ) A \
...
df_PonderadoLiquidacionDiarioTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.PonderadoLiquidacionDiario")
print('carga PonderadoLiquidacionDiario', df_PonderadoLiquidacionDiarioTemp.count())
#-Maxima edad entera 
...
df_MaximaEdadDiarioTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.MaximaEdadDiario")
print('carga MaximaEdadDiario', df_MaximaEdadDiarioTemp.count())
df_ft_Reprod_Liquid_DiarioF = spark.sql(f"select max(A.pk_tiempo) pk_tiempo,A.pk_empresa ,A.pk_division ,A.pk_zona ,A.pk_subzona ,A.pk_plantel \
                                              ...
                                              ,max(a.descripfecha) descripfecha \
                                              from {database_name_gl}.ft_reprod_diario A \
                                              left join {database_name_tmp}.MaximoCorral B on A.complexentityno = B.complexentityno and A.pk_etapa = B.pk_etapa \
                                              left join {database_name_tmp}.PonderadoLiquidacionDiario C on A.ComplexEntityNo = C.ComplexEntityNo and A.pk_etapa = C.pk_etapa \
                                              left join {database_name_tmp}.MaximaEdadDiario D on A.ComplexEntityNo = D.ComplexEntityNo \
                                              left join {database_name_gl}.ft_reprod_diario A1 on A1.complexentityno = b.complexentityno and A1.pk_tiempo = B.pk_tiempomax and A1.pk_etapa = B.pk_etapa \
...
# Escribir los resultados en ruta temporal
additional_options = {
"path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temp/ft_Reprod_Liquid_DiarioF"
}
df_ft_Reprod_Liquid_DiarioF.write \
    .format("parquet") \
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ft_Reprod_Liquid_DiarioF")
print('carga df_ft_Reprod_Liquid_DiarioF', df_ft_Reprod_Liquid_DiarioF.count())
df_ft_Reprod_Liquid_Diario = spark.sql(f"""SELECT * from {database_name_tmp}.ft_Reprod_Liquid_DiarioF
WHERE date_format(DescripFecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')
""")
print('carga df_ft_Reprod_Liquid_Diario', df_ft_Reprod_Liquid_Diario.count())
fechaactual = datetime.now().replace(day=1)
fecha_menos = fechaactual - relativedelta(months=4)
fecha_str = fecha_menos.strftime("%Y-%m-%d")
...
if datos_existentes15:
    ...
    data_after_delete15 = existing_data15.filter(~((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str)))
    filtered_new_data15 = df_ft_Reprod_Liquid_Diario.filter((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data15.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_Liquid_DiarioTemporal")
    ...
    final_data15_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_liquid_diario")
    ...
    print(f"Total de registros en la tabla ft_Reprod_Liquid_Diario : {cant_total15}")
     #Limpia la ubicación temporal
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_Liquid_DiarioTemporal')
    print(f"Tabla ft_Reprod_Liquid_DiarioTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")   
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_reprod_liquid_diario"
    }
    df_ft_Reprod_Liquid_Diario.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_liquid_diario")
print('carga ft_Reprod_Liquid_Diario', df_ft_Reprod_Liquid_Diario.count())    
#Maxima edad entera 
df_MaximaEdadGalponTemp = spark.sql(f"select complexentityno,max(edad) EdadMax \
                                     from {database_name_gl}.ft_reprod_galpon_diario \
                                     where cast(edad as varchar(20)) like '%.000000' \
                                     group by complexentityno")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_MaximaEdadGalponTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.MaximaEdadGalpon")
print('carga MaximaEdadGalpon', df_MaximaEdadGalponTemp.count())
...
df_MaximoGalponTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.MaximoGalpon")
print('carga MaximoGalpon', df_MaximoGalponTemp.count())
...
df_PonderadoLiquidacionGalponTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.PonderadoLiquidacionGalpon")
print('carga PonderadoLiquidacionGalpon', df_PonderadoLiquidacionGalponTemp.count())
...
df_Liquidacion2Temp1 = spark.sql(f"select \
                                ...
                                from {database_name_gl}.ft_reprod_galpon_semana X \
                                left join {database_name_gl}.ft_reprod_galpon_semana A on A.complexentityno = X.complexentityno and A.pk_etapa = X.pk_etapa and A.edad = 1.0 \
                                left join {database_name_gl}.ft_reprod_galpon_semana B on B.complexentityno = X.complexentityno and B.pk_etapa = X.pk_etapa and B.edad = 2.0 \
                                left join {database_name_gl}.ft_reprod_galpon_semana C on C.complexentityno = X.complexentityno and C.pk_etapa = X.pk_etapa and C.edad = 12.0 \
                                left join {database_name_gl}.ft_reprod_galpon_semana D on D.complexentityno = X.complexentityno and D.pk_etapa = X.pk_etapa and D.edad = 18.0 \
                                left join {database_name_gl}.ft_reprod_galpon_semana E on E.complexentityno = X.complexentityno and E.pk_etapa = X.pk_etapa and E.edad = 23.0 \
                                where X.pk_etapa = 1 \
                                group by X.pk_etapa,X.ComplexEntityNo")
print('df_Liquidacion2Temp1', df_Liquidacion2Temp1.count())
df_Liquidacion2Temp2 = spark.sql(f"select \
                                    ...
                                    ,0 Unif23Sem \
                                    from {database_name_gl}.ft_reprod_galpon_semana X \
                                    left join {database_name_gl}.ft_reprod_galpon_semana A on A.complexentityno = X.complexentityno and A.pk_etapa = X.pk_etapa and A.RowNumber = 1.0 \
                                    where X.pk_etapa = 3 \
                                    group by X.pk_etapa,X.ComplexEntityNo")
...
df_Liquidacion2.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Liquidacion2")
print('carga df_Liquidacion2Temp2', df_Liquidacion2Temp2.count())
print('carga df_Liquidacion2', df_Liquidacion2.count())
df_ft_Reprod_Liquid_GalponF =spark.sql(f"select max(A.pk_tiempo) pk_tiempo,A.pk_empresa,A.pk_division,A.pk_zona,A.pk_subzona,A.pk_plantel,A.pk_lote,A.pk_galpon,A.pk_etapa \
,A.pk_standard,A.pk_tipoproducto,A.pk_especie pk_especie,A.pk_estado,A.pk_administrador,A.pk_proveedor,A.pk_generacion,A.ComplexEntityNo,A.FechaNacimiento \
...
,max(A.descripfecha) descripfecha \
from {database_name_gl}.ft_reprod_galpon_diario A \
left join {database_name_tmp}.MaximoGalpon B on A.complexentityno = B.complexentityno and A.pk_etapa = B.pk_etapa \
left join {database_name_tmp}.PonderadoLiquidacionGalpon C on A.ComplexEntityNo = C.ComplexEntityNo and A.pk_etapa = C.pk_etapa \
left join {database_name_tmp}.Liquidacion2 D on A.ComplexEntityNo = D.ComplexEntityNo and A.pk_etapa = D.pk_etapa \
left  join {database_name_tmp}.MaximaEdadGalpon E on A.ComplexEntityNo = E.ComplexEntityNo \
left join {database_name_gl}.ft_reprod_galpon_diario F on F.complexentityno = b.complexentityno and F.pk_tiempo = B.pk_tiempomax and F.pk_etapa = B.pk_etapa \
...
,A.pk_especie,A.pk_estado,A.pk_administrador,A.pk_proveedor,A.pk_generacion,A.ComplexEntityNo,A.FechaNacimiento,A.FechaCap \
,b.complexentityno,b.pk_tiempomax,b.pk_etapa,A.pk_etapa,D.FechaIni")
# Escribir los resultados en ruta temporal
additional_options = {
"path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temp/ft_Reprod_Liquid_GalponF"
}
df_ft_Reprod_Liquid_GalponF.write \
    .format("parquet") \
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ft_Reprod_Liquid_GalponF")
print('carga df_ft_Reprod_Liquid_GalponF', df_ft_Reprod_Liquid_GalponF.count())
df_ft_Reprod_Liquid_Galpon = spark.sql(f"""SELECT * from {database_name_tmp}.ft_Reprod_Liquid_GalponF
WHERE date_format(DescripFecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')
""")
print('carga df_ft_Reprod_Liquid_Galpon', df_ft_Reprod_Liquid_Galpon.count())
fechaactual = datetime.now().replace(day=1)
fecha_menos = fechaactual - relativedelta(months=4)
fecha_str = fecha_menos.strftime("%Y-%m-%d")
...
if datos_existentes16:
    ...
    data_after_delete16 = existing_data16.filter(~((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str)))
    filtered_new_data16 = df_ft_Reprod_Liquid_Galpon.filter((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data16.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_Liquid_GalponTemporal")
    ...
    final_data16_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_liquid_galpon")
    ...
    print(f"Total de registros en la tabla ft_Reprod_Liquid_Galpon : {cant_total16}")
     #Limpia la ubicación temporal
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_Liquid_GalponTemporal')
    print(f"Tabla ft_Reprod_Liquid_GalponTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
else:
    additional_options = {
        "path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}ft_reprod_liquid_galpon"
    }
    df_ft_Reprod_Liquid_Galpon.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_liquid_galpon")
print('ft_Reprod_Liquid_Galpon', df_ft_Reprod_Liquid_Galpon.count())    
...
,nvl(MO.pk_alimentof,(select pk_alimento from {database_name_gl}.lk_alimento where calimento='0')) pk_alimento \
,nvl(LFR.pk_formula,(select pk_formula from {database_name_gl}.lk_formula where cformula='0')) pk_formula \
,MO.Price Valor \
FROM {database_name_tmp}.Reprod_Reproductora MO  \
LEFT JOIN {database_name_gl}.lk_formula LFR ON LFR.ProteinProductsIRN = cast(MO.ProteinProductsIRN_VFFF as varchar(50)) \
LEFT JOIN {database_name_gl}.lk_productoconsumo LPC ON LFR.ProteinProductsIRN = LPC.IRN \
LEFT JOIN {database_name_gl}.lk_subgrupoconsumo LSG ON LPC.pk_subgrupoconsumo = LSG.pk_subgrupoconsumo \
WHERE MO.FeedConsumedF <> 0 and (date_format(MO.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) \
union \
SELECT \
 MO.pk_tiempo  \
,MO.fecha\
...
,MO.FechaNacimiento Nacimiento \
,MO.Edad AS Edad \
,MO.FeedConsumedM FeedConsumed \
,nvl(MO.pk_alimentof,(select pk_alimento from {database_name_gl}.lk_alimento where calimento='0')) pk_alimento \
,nvl(LFR.pk_formula,(select pk_formula from {database_name_gl}.lk_formula where cformula='0')) pk_formula \
,MO.Price Valor \
FROM {database_name_tmp}.Reprod_Reproductora MO \
LEFT JOIN {database_name_gl}.lk_formula LFR ON LFR.ProteinProductsIRN = cast(MO.ProteinProductsIRN_VFFM as varchar(50)) \
LEFT JOIN {database_name_gl}.lk_productoconsumo LPC ON LFR.ProteinProductsIRN = LPC.IRN \
LEFT JOIN {database_name_gl}.lk_subgrupoconsumo LSG ON LPC.pk_subgrupoconsumo = LSG.pk_subgrupoconsumo \
WHERE MO.FeedConsumedM <> 0 and (date_format(MO.fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) \
")

# Escribir los resultados en ruta temporal
additional_options = {
"path": f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temp/consumoalimentoreprodtemp1"
}
df_ConsumoAlimentoReprodTemp1.write \
    .format("parquet") \
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.consumoalimentoreprodtemp1")
print('carga consumoalimentoreprodtemp1', df_ConsumoAlimentoReprodTemp1.count())
...
,pk_administrador \
,pk_proveedor \
,CASE WHEN Edad in (0,-1) AND (date_format(fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) THEN 2 \
      WHEN Edad <= 0 AND (date_format(fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-12),'yyyyMM')) AND SUBSTRING(ComplexEntityNo,1,1) = 'V' THEN 2 \
...
,pk_formula \
,Valor \
from {database_name_tmp}.consumoalimentoreprodtemp1")
...
df_ConsumoAlimentoReprodTemp_upd.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ConsumoAlimentoReprodTemp_upd")
print('carga ConsumoAlimentoReprodTemp_upd', df_ConsumoAlimentoReprodTemp_upd.count())
...
,nacimiento \
,MAX(edad) edad \
,SUM(FeedConsumed) cconsumo \
,pk_alimento \
,pk_formula \
,'' as categoria \
,SUM(Valor) Valor \
,MAX(fecha) descripfecha \
FROM {database_name_tmp}.ConsumoAlimentoReprodTemp_upd \
...
df_ConsumoAlimentoReprod.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ConsumoAlimentoReprod")
print('carga ConsumoAlimentoReprod', df_ConsumoAlimentoReprod.count())

df_ft_Reprod_ConsumoAlimento =spark.sql(f"SELECT * FROM {database_name_tmp}.ConsumoAlimentoReprod WHERE cconsumo <> 0 and (date_format(descripfecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-10),'yyyyMM')) \
                                            order by pk_tiempo desc")
print('carga df_ft_Reprod_ConsumoAlimento', df_ft_Reprod_ConsumoAlimento.count())
fechaactual = datetime.now().replace(day=1)
fecha_menos_doce_meses = fechaactual - relativedelta(months=10)
fecha_str = fecha_menos_doce_meses.strftime("%Y-%m-%d")
...
if datos_existentes17:
    ...
    data_after_delete17 = existing_data17.filter(~((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str)))
    filtered_new_data17 = df_ft_Reprod_ConsumoAlimento.filter((date_format(col("descripfecha"),"yyyy-MM-dd") >= fecha_str))
    ...
    final_data17.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Reprod_ConsumoAlimentoTemporal")
    ...
    final_data17_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_consumoalimento")
    ...
    print(f"Total de registros en la tabla ft_Reprod_ConsumoAlimento : {cant_total17}")
     #Limpia la ubicación temporal
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Reprod_ConsumoAlimentoTemporal')
    print(f"Tabla ft_Reprod_ConsumoAlimentoTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
    ...
    df_ft_Reprod_ConsumoAlimento.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_reprod_consumoalimento")
print('carga ft_Reprod_ConsumoAlimento', df_ft_Reprod_ConsumoAlimento.count())
