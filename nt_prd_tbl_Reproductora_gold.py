...
FROM (select * from {database_name}.si_mvhimchicktranshouses \
...
FROM (select * from {database_name}.si_bimentityinventory \
...
df_Consumo = spark.sql(f"SELECT ComplexEntityNo,IRN_BFT,pk_diasvida,pk_semanavida,pk_alimentoF,pk_alimentoM,FmimFeedTypesIRN_VFFF,FmimFeedTypesIRN_VFFM,sum(FeedConsumed) Consumo \
                        FROM ( \
                              ...
                              LEFT JOIN {database_name}.lk_diasvida D ON D.FirstHatchDateAge = M.FirstHatchDateAge \
...
df_UPDReprod_Reproductora.write \
    ...
    .format("parquet") \
    .options(**additional_options) \
    .saveAsTable("default.Reprod_Reproductora_new")

df_Reprod_Reproductora_nueva = spark.sql("""SELECT * from default.Reprod_Reproductora_new """)

# 2️⃣ Eliminar la tabla original (opcional, si se permite)
spark.sql("DROP TABLE IF EXISTS default.Reprod_Reproductora")
...
df_Reprod_Reproductora_nueva.write \
    ...
    .mode("overwrite") \
    .saveAsTable(f"default.Reprod_Reproductora")  

# 5️⃣ (Opcional) Eliminar la tabla temporal si ya no es neces
spark.sql("DROP TABLE IF EXISTS default.Reprod_Reproductora_new")
...
df_UPDReprod_Reproductora2.write \
    ...
    .format("parquet") \
    .options(**additional_options) \
    .saveAsTable("default.Reprod_Reproductora_new2")

df_Reprod_Reproductora_nueva2 = spark.sql("""SELECT * from default.Reprod_Reproductora_new2 """)
...
# 5️⃣ (Opcional) Eliminar la tabla temporal si ya no es neces
spark.sql("DROP TABLE IF EXISTS default.Reprod_Reproductora_new2")
...
FROM {database_name}.Reprod_Reproductora RE \
...
FROM {database_name}.Reprod_Reproductora FR \
...
df_STDLoteTemp = spark.sql(f"SELECT DISTINCT pk_semanavida,ComplexEntityNo,U_FeedPerHenWK ,U_FeedPerMaleWK ,U_PercentProdTEWK ,U_PercentProdHEWK , \
                            ...
                            ,U_HenMortWK,U_MaleMortWK,U_HenMortACM,U_MaleMortACM,U_EggWeight \
                            FROM {database_name}.Reprod_Reproductora PE ")
...
df_ft_Reprod_Liquid_DiarioF.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.ft_Reprod_Liquid_DiarioF")
print('carga df_ft_Reprod_Liquid_DiarioF', df_ft_Reprod_Liquid_DiarioF.count())
df_ft_Reprod_Liquid_Diario = spark.sql(f"""SELECT * from {database_name}.ft_Reprod_Liquid_DiarioF
...
df_ft_Reprod_Liquid_GalponF.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.ft_Reprod_Liquid_GalponF")
print('carga df_ft_Reprod_Liquid_GalponF', df_ft_Reprod_Liquid_GalponF.count())
df_ft_Reprod_Liquid_Galpon = spark.sql(f"""SELECT * from {database_name}.ft_Reprod_Liquid_GalponF
WHERE date_format(DescripFecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')
""")
...
,MO.Price Valor \
FROM {database_name}.Reprod_Reproductora MO  \
...
,pk_formula \
,Valor \
from consumoalimentoreprodtemp1")
...
df_ft_Reprod_ConsumoAlimento =spark.sql(f"SELECT * FROM {database_name}.ConsumoAlimentoReprod WHERE cconsumo <> 0 and (date_format(descripfecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-10),'yyyyMM')) \
