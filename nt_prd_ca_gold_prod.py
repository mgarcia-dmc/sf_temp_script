...
df_mortalidad = spark.sql(f"select distinct mortalityname nmortalidad,replace(B.ColumnName, 'U_', 'U_PE') irn ,1 idempresa \
                                    from {database_name_si}.si_BrimMortalities A left join {database_name_si}.si_MtSysUserFields B on A.MtSysUserFieldsIRN = B.IRN \
                                    where speciestype = 1 and B.ColumnName not like 'U_PE%'")
print('carga df_mortalidad')
additional_options = {
    "path": f"s3://ue1stgprodas3dtl005-gold/UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/Temp/ca_mortalidad"
}
df_mortalidad.write \
    .format("parquet") \
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ca_mortalidad")
print('crea ca_mortalidad')
df_mortalidad2 = spark.sql(f"select MortalityName, B.ColumnName irn,1 idempresa \
                             from {database_name_si}.si_BrimMortalities A left join  {database_name_si}.si_MtSysUserFields B on A.MtSysUserFieldsIRN = B.IRN \
                             where speciestype = 1 and B.ColumnName like 'U_PE%' and B.ColumnName not in (select distinct irn from {database_name_tmp}.ca_mortalidad)")
...
additional_options = {
    "path": f"s3://ue1stgprodas3dtl005-gold/UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/Temp/ca_mortalidad2"
}
df_mortalidad3.write \
    .format("parquet") \
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ca_mortalidad2")
...
from {database_name_si}.si_brimmortalities A \
left join {database_name_si}.si_mtsysuserfields B on A.MtSysUserFieldsIRN = B.IRN \
where speciestype = 2 and replace(B.ColumnName,'U_PVE', 'U_PE') not in (select distinct IRN from {database_name_tmp}.ca_mortalidad2)")
...
additional_options = {
    "path": f"s3://ue1stgprodas3dtl005-gold/UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/Temp/ca_mortalidadtemp"
}
df_mortalidad5.write \
    .format("parquet") \
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ca_mortalidadtemp")
...
df_LotePavoTemp1 = spark.sql(f"SELECT DISTINCT substring(b.ComplexEntityNo,1,length(b.ComplexEntityNo)-6) as clote,b.EntityNo as nlote,b.FarmNo as noplantel, \
            ...
            FROM {database_name_si}.si_brimfieldtrans a INNER JOIN \
            {database_name_si}.si_mvbrimentities b on a.ProteinEntitiesIRN=b.ProteinEntitiesIRN INNER JOIN \
            {database_name_si}.si_proteinfarms c on b.ProteinFarmsIRN=c.IRN INNER JOIN \
            {database_name_si}.si_proteinentities d on d.IRN = a.ProteinEntitiesIRN \
...
additional_options = {
    "path": f"s3://ue1stgprodas3dtl005-gold/UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/Temp/ca_lotepavo"
}
df_LotePavoTemp.write \
    .format("parquet") \
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ca_lotepavo")
...
path_lista = "s3://ue1stgprodas3dtl005-gold/UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/listatablas.txt"
lista_df = spark.read.option("delimiter", "|").format("csv").option("header", "true").load(path_lista)
bucket_name = "ue1stgprodas3dtl005-gold"
...
for row in lista_df.collect():
    ...
    pk_name = f"{row[2]}"
    pk_join = f"{row[1]}"
    path_target = f"s3://ue1stgprodas3dtl005-gold/UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/{table_name}"
    temp_target_path = f"s3://ue1stgprodas3dtl005-gold/UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/{table_name}_temp"
    prefix_target = f"UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/{table_name}/"
    prefix_target_temp = f"UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/{table_name}_temp/"

    print(f"➡️ Ejecutando {table_name}")
    # Intentar cargar los datos fuente
    try:
        if table_name == "lk_empresa":
            source_df = spark.sql(f"SELECT * FROM (SELECT DISTINCT  CASE WHEN CompanyNo='PONEDORAS HL SAC' THEN  2 WHEN CompanyNo='SAN FERNANDO' THEN  1 END  cempresa, \
                                    CASE WHEN CompanyName = 'PONEDORAS REPRO SAC' then 'RINCONADA' ELSE CompanyName END as nempresa \
                                    FROM {database_name_si}.si_proteincompanies \
                                    UNION \
            ...
            source_df = spark.sql(f"SELECT distinct idmes,idanio,mes,nmes,abrev FROM {database_name_si}.si_lk_mes")
        elif table_name == "lk_tiempo":
            source_df = spark.sql(f"SELECT distinct case when substring(fecha,2,1)='/' or substring(fecha,3,1)  = '/' then cast(concat(substring(cast (((((anio * 100) + mes) * 100)+dia) as varchar(8)),1,4), \
            ...
            FROM {database_name_si}.si_lk_tiempo")
        elif table_name == "lk_tipoproducto":
            source_df = spark.sql(f"SELECT 0 ctipoproducto , 'Sin Tipo Producto' ntipoproducto,'SAN FERNANDO' AS CompanyNo, 1 idempresa \
            ...
            source_df = spark.sql(f"SELECT DISTINCT GrowoutNo as csubzona, GrowoutName as nsubzona, IRN, '1' as Activeflag, 1 as idempresa FROM {database_name_si}.si_proteingrowoutcodes \
                                    UNION SELECT '0','Sin Subzona','00000000-0000-0000-0000-000000000000','1',1")
        elif table_name == "lk_zona":
            source_df = spark.sql(f"SELECT DISTINCT CountyNo as czona, CountyName as nzona,IRN,'1' as Activeflag, 1 as idempresa FROM {database_name_si}.si_proteincounties \
                                    UNION SELECT '0','Sin Zona','00000000-0000-0000-0000-000000000000','1',1")
        elif table_name == "lk_plantel":
            source_df = spark.sql(f"SELECT distinct PF.FarmNo cplantel,PF.FarmName nplantel, PF.FarmNo noplantel,PF.IRN,PF.Address1 direccion1,PF.Address2 direccion2,CASE WHEN ISNULL(LZ.PK_Zona)=true THEN 0 ELSE LZ.PK_Zona END PK_Zona, PF.ActiveFlag, PF.ProteinCostCentersIRN,LS.PK_Subzona,1 idempresa,UFC.Name Cluster,PV.VendorShortName duenho \
                             FROM {database_name_si}.si_proteinfarms PF \
                             LEFT JOIN {database_name_gl}.lk_zona LZ ON CAST(LZ.IRN AS varchar(50)) = CAST(PF.ProteinCountiesIRN AS varchar(50)) \
                             LEFT JOIN {database_name_gl}.lk_subzona LS ON LS.IRN = CAST(pf.ProteinGrowoutCodesIRN AS varchar(50)) \
                             LEFT JOIN {database_name_si}.si_brimfarms BF ON PF.IRN = BF.proteinfarmsirn \
                             LEFT JOIN {database_name_si}.si_mtsysuserfieldcodes UFC ON UFC.IRN = BF.U_Cluster \
                             LEFT JOIN {database_name_si}.si_mvproteinvendors PV ON PV.IRN = PF.ProteinVendorsIRN \
                             UNION \
                             SELECT 0,'Sin Plantel','0','00000000-0000-0000-0000-000000000000','Sin direccion 1','Sin direccion 2',(select PK_Zona from {database_name_gl}.lk_zona where czona=0),true ,'00000000-0000-0000-0000-000000000000',(select PK_Subzona from {database_name_gl}.lk_subzona where csubzona=0),1,'',''")
        elif table_name == "lk_galpon":
            source_df = spark.sql(f"SELECT distinct CONCAT(rtrim(b.cplantel),'-' ,rtrim(a.HouseNo)) as cgalpon,CONCAT(rtrim(b.cplantel),'-' ,rtrim(a.HouseNo)) as ngalpon,rtrim(a.HouseNo) as nogalpon ,rtrim(b.cplantel) as noplantel, a.HeadCapacity as capacidad, a.IRN, a.ActiveFlag, \
                                b.PK_Plantel, a.Length, a.Width, a.Area, 1 as idempresa FROM {database_name_si}.si_proteinhouses a left join {database_name_gl}.lk_plantel b on cast(a.ProteinFarmsIRN as varchar(50))=b.IRN \
                                union all \
                                select '0' cgalpon,'0' ngalpon,'0' nogalpon,'0' noplantel, 0 capacidad,'00000000-0000-0000-0000-000000000000' IRN, true ActiveFlag, \
                                (select pk_plantel from {database_name_gl}.lk_plantel where cplantel='0') PK_Plantel, 0 Length, 0 Width, 0 Area, 1 idempresa")
        
        elif table_name == "lk_division":
            source_df = spark.sql(f"SELECT DISTINCT DivisionNo cdivision, DivisionName ndivision,IRN,'1' activeflag FROM {database_name_si}.si_proteindivisions \
            ...
            FROM {database_name_si}.si_mvbrimentities a left join {database_name_gl}.lk_Plantel b ON b.IRN like a.ProteinFarmsIRN \
            LEFT JOIN {database_name_si}.si_proteincostcenters PCC ON a.ProteinCostCentersIRN = PCC.IRN \
            LEFT JOIN {database_name_gl}.lk_division LD ON LD.IRN = cast(PCC.ProteinDivisionsIRN as varchar(50)) \
            order by 1")
            
            additional_options = {
                "path": f"s3://ue1stgprodas3dtl005-gold/UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/Temp/ca_lote"
            }
            df_LoteTemp.write \
                .format("parquet") \
                .options(**additional_options) \
                .mode("overwrite") \
                .saveAsTable(f"{database_name_tmp}.ca_lote")
            ...
            from {database_name_tmp}.ca_lote a \
            left join {database_name_tmp}.ca_lotepavo b on a.clote = b.clote")
            
            additional_options = {
                "path": f"s3://ue1stgprodas3dtl005-gold/UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/Temp/ca_lotefinal"
            }
            df_LoteFinalTemp.write \
                .format("parquet") \
                .options(**additional_options) \
                .mode("overwrite") \
                .saveAsTable(f"{database_name_tmp}.ca_lotefinal")
            ...
            case when isnull(b.pk_plantel) = true then (select pk_plantel from {database_name_gl}.lk_plantel where cplantel='Sin Plantel') \
            ...
            FROM {database_name_si}.si_mvbimentities a left join {database_name_gl}.lk_Plantel b ON b.IRN = a.ProteinFarmsIRN \
            WHERE a.GRN='F' \
            union \
            SELECT '0' as clote ,'0' as nlote, '0' noplantel, '00000000-0000-0000-0000-000000000000' as IRN,'1' as ActiveFlag, \
            (select pk_plantel from {database_name_gl}.lk_plantel where cplantel='Sin Plantel') pk_plantel,1 as idempresa,null as Notes,'0' clote2,'0' nlote2,'' PlantelCampana ")
            
            additional_options = {
                "path": f"s3://ue1stgprodas3dtl005-gold/UE1STGPRODAS3PRD001/MTECH/SAN_FERNANDO/MAESTROS/Temp/ca_lotereproductora"
            }
            df_LoteReproductoraTemp.write \
                .format("parquet") \
                .options(**additional_options) \
                .mode("overwrite") \
                .saveAsTable(f"{database_name_tmp}.ca_lotereproductora")
            df_Lote = spark.sql(f"SELECT distinct * FROM {database_name_tmp}.ca_lotefinal \
            union \
            Select distinct * from {database_name_tmp}.ca_lotereproductora")
    
            source_df = df_Lote
        elif table_name == "lk_semanavida":
            source_df = spark.sql(f"SELECT DISTINCT csemanavida, nsemanavida FROM {database_name_si}.si_lk_semanavida order by 1")
        elif table_name == "lk_diasvida":
            source_df = spark.sql(f"SELECT DISTINCT CASE WHEN (CASE WHEN isnull(FirstHatchDate) = true THEN 0 \
                                ...
                                FROM {database_name_si}.si_brimfieldtrans a inner join {database_name_si}.si_mvbrimentities b on a.ProteinEntitiesIRN = b.ProteinEntitiesIRN \
            ...
            source_df = spark.sql(f"SELECT DISTINCT TechSupervisorNo as cadministrador, TechSupervisorname as nadministrador, IRN , U_Administrador as abrevadministrador,FarmType as tipo, 1 as idempresa \
                                    FROM {database_name_si}.si_proteintechsupervisors UNION SELECT 0,'Sin Administrador','00000000-0000-0000-0000-000000000000','Sin. Admin',1,1 ORDER BY 1")
        elif table_name == "lk_standard":
            source_df = spark.sql(f"SELECT DISTINCT StandardNo cstandard, StandardName nstandard, IRN, SpeciesType, '1' activeflag, 1 AS idempresa,CASE WHEN StandardNo like '%AD%' THEN 'Adulto' WHEN StandardNo like '%JO%' THEN 'Joven' ELSE '-' END AS lstandard \
                              FROM {database_name_si}.si_proteinstandards UNION SELECT '0','Sin Standard','00000000-0000-0000-0000-000000000000',0,'1',1,'-' order by 1")
...
#                                    FROM {database_name_si}.si_lk_grupoconsumo \
            ...
            CASE WHEN cproducto IN ('47665','47677','47690','47704','58089','47719','47724','PVHE (CD)') THEN 'EMPACADO' \
                 WHEN cproducto IN ('47738','47753','57571','57587','58052','58070','121774','121775','121776','121777','121778','121779','121780','121781','58105','121773','PVMA (GH1H2)','PVMA (DEF)') \
                     THEN 'TROZADO' ELSE grupoproductosalida END AS grupoproductosalida, idempresa \
                                                    ...
                                                    SELECT DISTINCT PP.ProductNo cproducto,Description nproducto, \
                                                     ...
                                                     FROM \
                                                          (select *,CASE WHEN ProductType = 6 AND Description like '%GAS%' THEN 'Gas' \
                                                                         ...
                                                                         from {database_name_si}.si_proteinproducts \
                                                          ) PP LEFT JOIN {database_name_si}.si_proteinproductsanimals PPA ON PPA.ProteinProductsIRN = PP.IRN \
                                                    LEFT JOIN {database_name_gl}.lk_grupoconsumo GC ON PP.ProductType = GC.cgrupoconsumo \
                                            ) X \
                                    UNION \
                                    ...
                                    order by 1")
#            source_df = spark.sql(f"SELECT DISTINCT PP.ProductNo cproducto,Description nproducto, \
#                             (CASE PP.ProductNo WHEN 'PO V H' THEN 'VIVO' WHEN 'PO V M'  THEN 'VIVO'ELSE 'BENEFICIO' end) grupoproducto, \
...
#                              FROM {database_name_si}.si_proteinproducts PP LEFT JOIN {database_name_si}.si_proteinproductsanimals PPA ON PPA.ProteinProductsIRN = PP.IRN \
#                              LEFT JOIN {database_name_gl}.lk_grupoconsumo GC ON PP.ProductType = GC.cgrupoconsumo \
#                              UNION \
...
#                              order by 1")
            ...
            source_df = spark.sql(f"SELECT DISTINCT status cestado, CASE WHEN status = 1 then 'Activo' WHEN status = 2 then 'Inactivo' WHEN status = 3 then 'Cerrado' else 'Sin Estado' end nestado, \
                                  ...
                                  FROM {database_name_si}.si_proteinentities union select distinct 0,'Sin Estado','Sin Estado','1' order by 1")
        elif table_name == "lk_especie":
            source_df = spark.sql(f"SELECT DISTINCT BreedNo cespecie,BreedName nespecie,SpeciesType tipo,IRN,ProteinBreedCodeTypesIRN,ActiveFlag,1 AS idempresa FROM {database_name_si}.si_proteinbreedcodes \
            ...
            source_df = spark.sql(f"SELECT DISTINCT FeedTypeNo calimento,FeedTypeName nalimento,cast(IRN as varchar(50)) IRN,CreationDate,1 AS idempresa FROM {database_name_si}.si_fmimfeedtypes \
            ...
            source_df = spark.sql(f"SELECT DISTINCT CASE WHEN isnull(VendorNo) = true THEN 0 ELSE VendorNo END  cproveedor, CASE WHEN isnull(VendorName)=true THEN 'Sin Proveedor' ELSE VendorName END  nproveedor, 1 AS idempresa \
                                    FROM {database_name_si}.si_mvbrimfarms UNION SELECT 0, 'Sin Proveedor',1 order by 1")
        elif table_name == "lk_formula":
            source_df = spark.sql(f"SELECT distinct MFF.FormulaNo cformula,MFF.FormulaName nformula,MFF.IRN,LA.pk_alimento,MFF.ProteinProductsIRN,ActiveFlag, 1 AS idempresa \
                                    FROM {database_name_si}.si_mvfmimfeedformulas MFF LEFT JOIN {database_name_gl}.lk_alimento LA ON LA.IRN = CAST(MFF.FmimFeedTypesIRN AS VARCHAR(50)) UNION \
                                SELECT 0, 'Sin Formula','00000000-0000-0000-0000-000000000000',(select distinct  pk_alimento from {database_name_gl}.lk_alimento where calimento='0'),'00000000-0000-0000-0000-000000000000',true,1 order by 1")
        elif table_name == "lk_subgrupoconsumo":
            source_df = spark.sql(f"SELECT 0 csubgrupoconsumo,'Sin Sub Grupo Consumo' nsubgrupoconsumo,'Sin Sub Grupo Consumo' irn,1 activeflag, 1 idempresa \
                                   UNION \
            ...
            source_df = spark.sql(f"SELECT distinct PP.productNo cproductoconsumo,SUBSTRING(PP.Description,1,50) nproductoconsumo,PP.IRN,PP.ActiveFlag, \
            ...
            WHEN ProductType = 6 AND Description like '%KRAFT%' THEN 'Kraft' \
            WHEN ProductType = 12 THEN 'Medicina' \
            ELSE 'Sin Sub Grupo Consumo' END SubGrupoConsumo from {database_name_si}.si_proteinproducts) PP \
            LEFT OUTER JOIN  {database_name_si}.si_proteinproductsanimals PPA ON PPA.ProteinProductsIRN =PP.IRN \
            LEFT JOIN {database_name_gl}.lk_grupoconsumo GC ON PP.ProductType = GC.cgrupoconsumo AND GC.idempresa = 1 \
            LEFT JOIN {database_name_gl}.lk_subgrupoconsumo SGC ON PP.SubGrupoConsumo = SGC.nsubgrupoconsumo \
            WHERE ProteinProductsIRN IS NULL \
            UNION \
            select 0,'Sin Productoconsumo','00000000-0000-0000-0000-000000000000',true,(select distinct pk_grupoconsumo from {database_name_gl}.lk_grupoconsumo where cgrupoconsumo=0),1, \
            (select distinct pk_subgrupoconsumo from {database_name_gl}.lk_subgrupoconsumo where csubgrupoconsumo='0') \
            order by 1")
        elif table_name == "lk_generacion":
            source_df = spark.sql(f"select 0 cgeneracion,'Sin Generación' ngeneracion \
            ...
            source_df = spark.sql(f"select distinct HatcheryNo cincubadora,HatcheryName nincubadora,HatcheryNo noincubadora,IRN,Activeflag,ProteinCostCentersIRN,1 AS idempresa from {database_name_si}.si_proteinfacilityhatcheries \
            ...
            source_df = spark.sql(f"SELECT distinct DriverNo cconductor, DriverName nconductor, IRN, activeflag,1 idempresa FROM {database_name_si}.si_proteindrivers \
                                  UNION SELECT '0', 'Sin Conductor','00000000-0000-0000-0000-000000000000',true,1")
        elif table_name == "lk_vehiculo":
            source_df = spark.sql(f"SELECT distinct rtrim(VehicleNo) cvehiculo, rtrim(VehicleName) nvehiculo,rtrim(VehicleName) marca ,IRN, activeflag,1 AS idempresa FROM {database_name_si}.si_proteinvehicles \
                                   UNION SELECT '0', 'Sin Vehiculo','Sin Vehiculo','00000000-0000-0000-0000-000000000000',true,1")
        elif table_name == "lk_maquinaincubadora":
            source_df = spark.sql(f"SELECT DISTINCT CONCAT(rtrim(hatcheryno),'-',rtrim(setterno)) cmaquinaincubadora ,setterno, settername,himsettersirn,rtrim(hatcheryno) hatcheryno,pk_incubadora, 1 as idempresa \
                                    FROM {database_name_si}.si_mvhimhatchtrans A \
                                    LEFT JOIN {database_name_si}.si_himhatchtrans B on A.IRN = B.IRN \
                                    LEFT JOIN {database_name_gl}.lk_incubadora C on rtrim(C.cincubadora) = rtrim(A.hatcheryno) \
            ...
            source_df = spark.sql(f"select distinct concat(raza,edad) crazaedad,* from {database_name_si}.si_ft_excel_incub_std_raza")
        elif table_name == "lk_ft_excel_categoria":
            source_df = spark.sql(f"select distinct b.pk_lote, a.lote,a.categoria from {database_name_si}.si_ft_excel_categoria a left join {database_name_gl}.lk_lote b on a.lote = b.clote ")
        elif table_name == "lk_causamortalidad":
            source_df = spark.sql(f"select distinct '' as cmortalidad,nmortalidad as nmortalidad,replace(irn,'í','i') as irn ,idempresa as idempresa from {database_name_tmp}.ca_mortalidadtemp")
        elif table_name == "lk_planta":
            source_df = spark.sql(f"SELECT distinct PlantNo cplanta, PlantName nplanta,ProteinCostCentersIRN, activeflag,1 AS pk_empresa,IRN FROM {database_name_si}.si_proteinfacilityplants \
            ...
            source_df = spark.sql(f"select distinct rtrim(VendorNo) cduenho, rtrim(VendorName) nduenho, rtrim(VendorShortName) nduenhocorto, 1 as idempresa \
                                    from {database_name_si}.si_mvproteinvendors UNION \
    ...
    df_temp.write.format("parquet").mode("overwrite").option("path", path_target).saveAsTable(f"{database_name_gl}.{table_name}")
    ...
    print(f"Tabla registrada en el catálogo: {database_name_gl}.{table_name}")  
