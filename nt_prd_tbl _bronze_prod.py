    ...
    try:
        ...
        if datos_existentes:
            ...
            if (campo_fecha != "None"):
                ...
                data_after_delete = existing_data.filter(~((date_format(col(f"{campo_fecha}"),"yyyyMM") >= AnioMes) & (date_format(col(f"{campo_fecha}"),"yyyyMM") <= AnioMesFin)))
                filtered_new_data = df_nuevos.filter((date_format(col(f"{campo_fecha}"),"yyyyMM") >= AnioMes) & (date_format(col(f"{campo_fecha}"),"yyyyMM") <= AnioMesFin))
                ...
                final_data.write \
                    ...
                    .mode("overwrite") \
                    .saveAsTable(f"{database_name_tmp}.{table_name3}")
                ...
                glue_client.delete_table(DatabaseName=database_name_tmp, Name=table_name3)
                print(f"Tabla {table_name3} eliminada correctamente de la base de datos '{database_name_tmp}'.")
