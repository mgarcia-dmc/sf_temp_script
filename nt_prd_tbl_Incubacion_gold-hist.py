...
,A.ComplexEntityNo \
,RTRIM(LM.settername) MaquinaIncubadora \
,RTRIM(LN.hatchername) MaquinaNacedora \
...
RGD.fecha = cast(cast(A.ProdDate as timestamp) as date) \
...
RGDC.fecha = cast(cast(A.SetDate as timestamp) as date) \
...
,RTRIM(A.RefNo) RefNo \
,1 pk_empresa \
FROM {database_name}.si_mvhimbreakouttrans A \
LEFT JOIN {database_name}.si_himbreakouttrans B on A.IRN = B.IRN")
...
,nvl(LN.pk_maquinanacedora,1) pk_maquinanacedora \
,A.ComplexEntityNo \
,RTRIM(settername) MaquinaIncubadora \
,RTRIM(hatchername) MaquinaNacedora \
,RGD.Edad EdadFechaProd \
,RGDC.Edad EdadFechaCarga \
...
LEFT JOIN {database_name}.ft_Reprod_Galpon_Diario RGD ON RGD.pk_plantel = LP.pk_plantel and RGD.pk_lote = LL.pk_lote and RGD.pk_galpon = LG.pk_galpon and RGD.fecha = cast(cast(A.ProdDate as timestamp) as date) \
LEFT JOIN {database_name}.ft_Reprod_Galpon_Diario RGDC ON RGDC.pk_plantel = LP.pk_plantel and RGDC.pk_lote = LL.pk_lote and RGDC.pk_galpon = LG.pk_galpon and RGDC.fecha = cast(cast(A.SetDate as timestamp) as date) \
LEFT JOIN {database_name}.si_proteincostcenters PC ON PC.IRN = A.ProteinCostCentersIRN \
LEFT JOIN {database_name}.lk_division LD ON LD.IRN = PC.ProteinDivisionsIRN \
LEFT JOIN {database_name}.lk_maquinaincubadora LM ON LM.himsettersirn = A.himsettersirn \
LEFT JOIN {database_name}.lk_maquinanacedora LN ON LN.HimHatchersIRN = A.HimHatchersIRN")
...
GROUP BY A.pk_tiempo,A.fecha,A.pk_zona,A.pk_division,A.pk_plantel,A.pk_lote,A.pk_galpon,A.pk_incubadora,A.pk_especie,A.pk_generacion \
...
df_ft_Incub_Tabla_NacDiario =spark.sql(f"SELECT pk_tiempo,fecha,anio,mes,semana,pk_division,pk_zona,pk_plantel,pk_lote \
...
GROUP BY anio,mes,semana,pk_division,pk_zona,pk_plantel,pk_lote,pk_galpon,pk_incubadora,pk_especie,pk_generacion \
