...
,(SELECT MIN(cast(TransDate as timestamp)) FROM {database_name_si}.si_mvhimchicktranshouses WHERE proteinentitiesirn = A.proteinentitiesirn) AS FechaIniAlojamiento \
,(SELECT MAX(cast(TransDate as timestamp)) FROM {database_name_si}.si_mvhimchicktranshouses WHERE proteinentitiesirn = A.proteinentitiesirn) AS FechaFinAlojamiento \
...
,CASE WHEN J.GrowerType = 5 THEN 'COMPRADO' \
      ...
      WHEN J.GrowerType <> 5 AND A.VendorSKUNo IS NULL THEN 'PROPIO' \
 END TipoOrigen \
FROM {database_name_si}.si_mvhimchicktranshouses A \
LEFT JOIN {database_name_si}.si_himchicktranshouses B ON A.proteinentitiesirn = B.proteinentitiesirn and A.himchicktranshousesIRN = B.irn \
...
LEFT JOIN {database_name_si}.si_brimentities E ON CAST(E.ProteinEntitiesIRN  AS VARCHAR(50))= CAST(A.ProteinEntitiesIRN AS VARCHAR(50)) \
LEFT JOIN {database_name_si}.si_mvproteinentities F ON A.ProteinEntitiesIRN = F.IRN \
LEFT JOIN {database_name_si}.si_himchicktransparentdetail G ON C.IRN = G.IRN \
LEFT JOIN {database_name_si}.si_bimcapitalizationtrans H ON C.ProteinEntitiesIRN_Parent = H.ProteinEntitiesIRN \
LEFT JOIN {database_name_si}.si_mvbimentities I ON D.IRN = I.IRN \
LEFT JOIN {database_name_si}.si_mvbimfarms J ON J.FarmNo = D.FarmNo \
...
df_mvHimChickTransHouses.write \
    ...
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.mvHimChickTransHousesTemp")
...
WHEN J.GrowerType <> 5 AND A.VendorSKUNo IS NULL THEN 'PROPIO' \
END TipoOrigen \
FROM {database_name_tmp}.mvHimChickTransHousesTemp A \
LEFT JOIN {database_name_si}.si_himchicktranshouses B ON A.proteinentitiesirn = B.proteinentitiesirn and A.himchicktranshousesIRN = B.irn \
LEFT JOIN {database_name_si}.si_mvhimchicktransparentdetail C ON A.ProteinEntitiesIRN = C.ProteinEntitiesIRN and C.HimChickTransHousesIRN = A.HimChickTransHousesIRN \
LEFT JOIN {database_name_si}.si_mvproteinentities D ON C.ProteinEntitiesIRN_Parent = D.IRN \
LEFT JOIN {database_name_si}.si_brimentities E ON CAST(E.ProteinEntitiesIRN  AS VARCHAR(50))= CAST(A.ProteinEntitiesIRN AS VARCHAR(50)) \
...
df_AlojamientoTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.AlojamientoTemp")
...
df_SumatoriaAlojamiento1 = spark.sql(f"SELECT ComplexEntityNo, EdadPadreDescrip, SUM(CantAlojamientoDet) CantAlojamientoXEdadPadre \
                                    FROM {database_name_tmp}.AlojamientoTemp  group by ComplexEntityNo, EdadPadreDescrip") 
# Escribir los resultados en ruta temporal
additional_options = {
...
df_SumatoriaAlojamiento1.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SumatoriaAlojamiento1")
...
df_SumatoriaAlojamiento2 = spark.sql(f"SELECT ComplexEntityNo, EdadPadreDescrip2,SUM(CantAlojamientoDet) CantAlojamientoXEdadPadre2 \
                                        FROM {database_name_tmp}.AlojamientoTemp group by ComplexEntityNo, EdadPadreDescrip2")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_SumatoriaAlojamiento2.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SumatoriaAlojamiento2")
print('carga SumatoriaAlojamiento2',df_SumatoriaAlojamiento2.count())

df_SumatoriaAlojamiento3 = spark.sql(f"SELECT ComplexEntityNo,SUM(CantAlojamientoDet) CantAlojamientoTotal FROM {database_name_tmp}.AlojamientoTemp group by ComplexEntityNo")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_SumatoriaAlojamiento3.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SumatoriaAlojamiento3")
...
df_SumatoriaAlojamiento4 = spark.sql(f"SELECT ComplexEntityNo,ProteinBreedCodesIRN,SUM(CantAlojamientoDet) CantAlojamientoTotal \
                                        FROM {database_name_tmp}.AlojamientoTemp group by ComplexEntityNo,ProteinBreedCodesIRN")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_SumatoriaAlojamiento4.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SumatoriaAlojamiento4")
...
df_SumatoriaAlojamiento5 = spark.sql(f"SELECT ComplexEntityNo,FarmNoPadre,SUM(CantAlojamientoDet) CantAlojamientoTotal \
                                        FROM {database_name_tmp}.AlojamientoTemp group by ComplexEntityNo,FarmNoPadre")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_SumatoriaAlojamiento5.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SumatoriaAlojamiento5")
...
df_SumatoriaAlojamiento6 = spark.sql(f"SELECT ComplexEntityNo,SUM(PesoAlojXCantAlojDet) PesoAlojXCantAlojDetTotal,SUM(CantAlojamientoDet) CantAlojamientoTotal \
                                    FROM {database_name_tmp}.AlojamientoTemp \
...
df_SumatoriaAlojamiento6.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SumatoriaAlojamiento6")
print('carga SumatoriaAlojamiento6',df_SumatoriaAlojamiento6.count())

df_SumatoriaAlojamiento7 = spark.sql(f"SELECT ComplexEntityNoGalpon,SUM(CantAlojamientoDet) CantAlojamientoTotal FROM {database_name_tmp}.AlojamientoTemp group by ComplexEntityNoGalpon")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_SumatoriaAlojamiento7.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SumatoriaAlojamiento7")
print('carga SumatoriaAlojamiento7',df_SumatoriaAlojamiento7.count())

df_SumatoriaAlojamiento8 = spark.sql(f"SELECT ComplexEntityNo,SUM(CantAlojamientoDet) CantAlojamientoTotal FROM {database_name_tmp}.AlojamientoTemp where ((upper(ProteinBreedCodesIRN) = '143F98F4-1292-4533-AB3D-674CC21D15DF' OR upper(ProteinBreedCodesIRN) = 'FA43CCAB-D1FC-46E1-BD14-6433E437F2F3' OR upper(ProteinBreedCodesIRN) = '8B742404-5DFE-49C4-A894-78FE52D095E1')) group by ComplexEntityNo")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_SumatoriaAlojamiento8.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SumatoriaAlojamiento8")
...
,nvl(LSZ.pk_subzona,(select pk_subzona from {database_name_gl}.lk_subzona where csubzona='0')) AS pk_subzona \
...
,nvl(LS.pk_sexo,(select pk_sexo from {database_name_gl}.lk_sexo where csexo=0)) AS pk_sexo \
,nvl(LST.pk_standard,(select pk_standard from {database_name_gl}.lk_standard where cstandard='0')) AS pk_standard \
,nvl(LPR.pk_producto,(select pk_producto from {database_name_gl}.lk_producto where cproducto='0')) AS pk_producto \
,nvl(TP.pk_tipoproducto,(select pk_tipoproducto from {database_name_gl}.lk_tipoproducto where ntipoproducto='Sin Tipo Producto')) AS pk_tipoproducto \
,nvl(LEP.pk_especie,(select pk_especie from {database_name_gl}.lk_especie where cespecie='0')) AS pk_especie \
,nvl(LES.pk_estado,(select pk_estado from {database_name_gl}.lk_estado where cestado=0)) AS pk_estado \
,nvl(LAD.pk_administrador,(select pk_administrador from {database_name_gl}.lk_administrador where cadministrador='0')) AS pk_administrador \
,nvl(LCO.pk_conductor,(select pk_conductor from {database_name_gl}.lk_conductor where cconductor='0')) AS pk_conductor \
,nvl(VH.pk_vehiculo,(select pk_vehiculo from {database_name_gl}.lk_vehiculo where cvehiculo='0')) AS pk_vehiculo \
,nvl(INC.pk_incubadora,(select pk_incubadora from {database_name_gl}.lk_incubadora where cincubadora='0')) AS pk_incubadora \
...
,FechaCierre \
,'BRIM' AS TipoGranja \
,(SELECT concat_ws(',' , collect_list( DISTINCT ComplexEntityNoPadre)) ComplexEntityNoPadre FROM {database_name_tmp}.AlojamientoTemp F WHERE F.ComplexEntityNo = AL.ComplexEntityNo) ListaPadre \
,(SELECT concat_ws(',' , collect_list( DISTINCT PlantaIncubacion)) PlantaIncubacion FROM {database_name_tmp}.AlojamientoTemp F WHERE F.ComplexEntityNo = AL.ComplexEntityNo) ListaIncubadora \
,PlantaIncubacion \
,PesoBebe \
...
,AL.Origen \
,(SELECT concat_ws(',' , collect_list( DISTINCT Origen)) Origen FROM {database_name_tmp}.AlojamientoTemp F WHERE F.ComplexEntityNo = AL.ComplexEntityNo) ListaOrigen \
,AL.TipoOrigen \
,(SELECT concat_ws(',' , collect_list( DISTINCT TipoOrigen)) TipoOrigen FROM {database_name_tmp}.AlojamientoTemp F WHERE F.ComplexEntityNo = AL.ComplexEntityNo) ListaTipoOrigen \
...
,'' Numplaca \
,'' DescripIncubadora \
FROM {database_name_tmp}.AlojamientoTemp AL \
LEFT JOIN {database_name_gl}.lk_tiempo LT ON date_format(LT.fecha,'yyyyMMdd') = date_format(AL.FechaAlojamiento,'yyyyMMdd') \
LEFT JOIN {database_name_gl}.lk_plantel LP ON LP.IRN = CAST(AL.ProteinFarmsIRN_FF AS VARCHAR (50)) \
LEFT JOIN {database_name_gl}.lk_lote LL ON LL.pk_plantel=LP.pk_plantel AND LL.nlote = AL.EntityNo AND LL.activeflag IN (0,1) \
LEFT JOIN {database_name_gl}.lk_galpon LG ON LG.noplantel = LP.noplantel AND LG.nogalpon=AL.HouseNo AND LG.activeflag IN (false,true) \
...
LEFT JOIN {database_name_gl}.lk_subzona LSZ ON LP.pk_subzona = LSZ.pk_subzona \
LEFT JOIN {database_name_gl}.lk_sexo LS ON LS.csexo = AL.Sexo \
LEFT JOIN {database_name_si}.si_proteinentities PE ON PE.IRN = CAST(AL.ProteinEntitiesIRN AS VARCHAR(50)) \
LEFT JOIN {database_name_gl}.lk_administrador LAD ON LAD.IRN = CAST(PE.ProteinTechSupervisorsIRN AS VARCHAR(50)) \
LEFT JOIN {database_name_si}.si_proteinstandardversions PSV ON CAST(PE.ProteinStandardVersionsIRN AS VARCHAR(50)) = PSV.IRN \
LEFT JOIN {database_name_gl}.lk_standard LST ON LST.IRN = CAST(PSV.ProteinStandardsIRN AS VARCHAR(50)) \
LEFT JOIN {database_name_gl}.lk_producto LPR ON LPR.IRNProteinProductsAnimals = CAST(AL.ProteinProductsAnimalsIRN AS varchar(50)) \
LEFT JOIN {database_name_gl}.lk_estado LES ON LES.cestado=PE.Status \
LEFT JOIN {database_name_gl}.lk_especie LEP ON LEP.IRN = CAST(AL.ProteinBreedCodesIRN AS VARCHAR(50)) \
LEFT JOIN {database_name_gl}.lk_tipoproducto TP ON TP.ntipoproducto=lpr.grupoproducto \
LEFT JOIN {database_name_gl}.LK_conductor LCO   ON CAST(LCO.IRN AS VARCHAR(50)) = CAST(AL.ProteinDriversIRN AS VARCHAR(50)) \
LEFT JOIN {database_name_gl}.lk_vehiculo VH     ON CAST(VH.IRN AS VARCHAR(50))  = CAST(AL.ProteinVehiclesIRN AS VARCHAR(50)) \
LEFT JOIN {database_name_gl}.lk_incubadora INC  ON INC.ProteinCostCentersIRN    = CAST(AL.ProteinCostCentersIRN_HatchHatchery as varchar (50)) \
LEFT JOIN {database_name_si}.si_mvbrimfarms MB  ON CAST(MB.ProteinFarmsIRN AS VARCHAR (50)) = LP.IRN \
LEFT JOIN {database_name_gl}.lk_proveedor PRO   ON MB.VendorNo = PRO.cproveedor \
LEFT JOIN {database_name_tmp}.categoria CAT ON LP.pk_plantel = CAT.pk_plantel and LL.pk_lote = CAT.pk_lote and LG.pk_galpon = CAT.pk_galpon \
LEFT JOIN {database_name_tmp}.atipicos AT ON AL.ComplexEntityNo = AT.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.SumatoriaAlojamiento1  SA1 ON SA1.ComplexEntityNo   = AL.ComplexEntityNo AND SA1.EdadPadreDescrip = AL.EdadPadreDescrip \
LEFT JOIN {database_name_tmp}.SumatoriaAlojamiento2  SA2 ON SA2.ComplexEntityNo   = AL.ComplexEntityNo AND SA2.EdadPadreDescrip2 = AL.EdadPadreDescrip2 \
LEFT JOIN {database_name_tmp}.SumatoriaAlojamiento3  SA3 ON SA3.ComplexEntityNo   = AL.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.SumatoriaAlojamiento4  SA4 ON SA4.ComplexEntityNo   = AL.ComplexEntityNo AND SA4.ProteinBreedCodesIRN = AL.ProteinBreedCodesIRN \
LEFT JOIN {database_name_tmp}.SumatoriaAlojamiento4  SA41 ON SA41.ComplexEntityNo = AL.ComplexEntityNo AND upper(SA41.ProteinBreedCodesIRN) = '143F98F4-1292-4533-AB3D-674CC21D15DF' \
LEFT JOIN {database_name_tmp}.SumatoriaAlojamiento8  SA42 ON SA42.ComplexEntityNo = AL.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.SumatoriaAlojamiento5  SA5 ON SA5.ComplexEntityNo = AL.ComplexEntityNo AND SA5.FarmNoPadre = AL.FarmNoPadre \
LEFT JOIN {database_name_tmp}.SumatoriaAlojamiento6  SA6 ON SA6.ComplexEntityNo = AL.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.SumatoriaAlojamiento7  SA7 ON SA7.ComplexEntityNoGalpon = AL.ComplexEntityNoGalpon")
...
df_Alojamiento_ip.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.alojamiento_ip")
...
df_PlantaMayorTemp = spark.sql(f"SELECT * \
                                FROM ( \
                                     ...
                                     FROM {database_name_tmp}.alojamiento_ip A \
                                     WHERE TotalAloj = (SELECT MAX(TotalAloj) FROM {database_name_tmp}.alojamiento_ip X WHERE a.ComplexEntityNo = x.ComplexEntityNo) \
...
df_PlantaMayorTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.PlantaMayor")
...
df_RazaMayorTemp = spark.sql(f"SELECT * \
                              FROM ( \
                                    ...
                                    FROM {database_name_tmp}.alojamiento_ip A \
                                    WHERE CantAlojamientoXRaza = (SELECT MAX(CantAlojamientoXRaza) FROM {database_name_tmp}.alojamiento_ip B WHERE A.ComplexEntityNo = B.ComplexEntityNo) \
...
df_RazaMayorTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.RazaMayor")
...
df_PadreMayorTemp = spark.sql(f"SELECT * \
                               FROM ( \
                                     ...
                                     FROM {database_name_tmp}.alojamiento_ip A \
                                     WHERE CantAlojamientoPadre = (SELECT MAX(CantAlojamientoPadre) FROM {database_name_tmp}.alojamiento_ip B WHERE A.ComplexEntityNo = B.ComplexEntityNo) \
...
df_PadreMayorTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.PadreMayor")
...
df_EdadPadreMayorTemp = spark.sql(f"SELECT * \
                                   FROM ( \
                                         ...
                                         FROM {database_name_tmp}.alojamiento_ip A \
                                         WHERE CantAlojamientoXEdadPadre = (SELECT MAX(CantAlojamientoXEdadPadre) FROM {database_name_tmp}.alojamiento_ip B WHERE A.ComplexEntityNo = B.ComplexEntityNo) \
...
df_EdadPadreMayorTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.EdadPadreMayor")
...
df_EdadPadreMayor2Temp = spark.sql(f"SELECT * \
                                    FROM ( \
                                          ...
                                          FROM {database_name_tmp}.alojamiento_ip A \
                                          WHERE CantAlojamientoXEdadPadre2 = (SELECT MAX(CantAlojamientoXEdadPadre2) FROM {database_name_tmp}.alojamiento_ip B WHERE A.ComplexEntityNo = B.ComplexEntityNo) \
...
df_EdadPadreMayor2Temp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.EdadPadreMayor2")
...
df_sumEdadPadreCorral =spark.sql(f"SELECT ComplexEntityNo, (SUM(EdadPadre*CantAloj*1.0) / SUM(CantAloj)) sumEdadPadreCantAloj \
                                FROM {database_name_tmp}.alojamiento_ip group by ComplexEntityNo")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_sumEdadPadreCorral.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.sumEdadPadreCorral")
...
df_sumEdadPadreCorral1 =spark.sql(f"SELECT pk_plantel,pk_lote,pk_galpon, (SUM(EdadPadre*CantAloj*1.0) / SUM(CantAloj)) sumEdadPadreCantAloj2 \
                                    FROM {database_name_tmp}.alojamiento_ip group by pk_plantel,pk_lote,pk_galpon")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_sumEdadPadreCorral1.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.sumEdadPadreCorral1")
...
df_sumEdadPadreCorral2 =spark.sql(f"SELECT pk_plantel,pk_lote,(SUM(EdadPadre*CantAloj*1.0) / SUM(CantAloj)) sumEdadPadreCantAloj3 \
                                    FROM {database_name_tmp}.alojamiento_ip group by pk_plantel,pk_lote")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_sumEdadPadreCorral2.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.sumEdadPadreCorral2")
...
df_EdadPadreTemp = spark.sql(f"SELECT IRN,a.ComplexEntityNo,ComplexEntityNoPadre,EdadPadre \
                             ...
                             ,CASE WHEN SUM(CantAloj) = 0 THEN 0 ELSE sum(d.sumEdadPadreCantAloj3) END AS EdadPadreLote \
                              FROM {database_name_tmp}.alojamiento_ip A \
                              left join {database_name_tmp}.sumEdadPadreCorral b on B.ComplexEntityNo = A.ComplexEntityNo \
                              left join {database_name_tmp}.sumEdadPadreCorral1 c on c.pk_plantel = A.pk_plantel and c.pk_lote = a.pk_lote and c.pk_galpon = a.pk_galpon \
                              left join {database_name_tmp}.sumEdadPadreCorral2 d on d.pk_plantel = A.pk_plantel and d.pk_lote = a.pk_lote \
...
,AL.NroGuia \
,AL.FechaNacimiento \
,(SELECT MIN(FechaAlojamiento) FROM {database_name_tmp}.alojamiento_ip A WHERE a.pk_lote = al.pk_lote) FechaAlojamiento \
...
,PesoHvo \
,CASE WHEN (SELECT SUM(CantAloj) FROM {database_name_tmp}.alojamiento_ip D WHERE D.ComplexEntityNo = AL.ComplexEntityNo) = 0 THEN 0 \
ELSE (SELECT SUM(PesoHvoXCantAloj) FROM {database_name_tmp}.alojamiento_ip D WHERE D.ComplexEntityNo = AL.ComplexEntityNo AND CantAloj>0) / \
(SELECT sum(CantAloj) FROM {database_name_tmp}.alojamiento_ip D WHERE D.ComplexEntityNo = AL.ComplexEntityNo AND PesoHvoXCantAloj>0) END AS PesoHvoPond \
...
,Numplaca \
,DescripIncubadora \
FROM {database_name_tmp}.alojamiento_ip AL \
LEFT JOIN {database_name_tmp}.PlantaMayor PM ON AL.ComplexEntityNo = PM.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.RazaMayor RM ON AL.ComplexEntityNo = RM.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.PadreMayor PAM ON AL.ComplexEntityNo = PAM.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.EdadPadreMayor EPM ON AL.ComplexEntityNo = EPM.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.EdadPadreMayor2 EPM2 ON AL.ComplexEntityNo = EPM2.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.EdadPadre EP ON AL.ComplexEntityNo = EP.ComplexEntityNo AND AL.ComplexEntityNoPadre = EP.ComplexEntityNoPadre AND AL.IRN = EP.IRN \
...
if datos_existentes:
    ...
    final_data.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.{name_table18}Temporal")
    
    print(f"Tabla {name_table18}Temporal Creada correctamente de la base de datos '{database_name_tmp}'.")
    ...
    final_data2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.{name_table18}")
    ...
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_alojamientoTemporal')
    print(f"Tabla {name_table18}Temporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
