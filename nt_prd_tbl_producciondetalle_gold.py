...
WITH CategoriaPorLote AS (
    ...
    FROM default.CategoriaGalponTemp
    GROUP BY pk_lote
)
SELECT clote, 
COALESCE(cg.categoria, '') AS categoria,
e.pk_plantel,f.pk_lote, 1 pk_galpon 
FROM {database_name}.si_brimentities a 
LEFT JOIN {database_name}.si_proteinentities b on a.ProteinEntitiesIRN=b.IRN 
LEFT JOIN {database_name}.si_mvbrimentities c ON c.ProteinEntitiesIRN=a.ProteinEntitiesIRN 
LEFT JOIN {database_name}.si_mtsysuserfieldcodes d ON a.U_Categoria=d.IRN 
LEFT JOIN {database_name}.lk_plantel e ON e.IRN=CAST(b.ProteinFarmsIRN AS VARCHAR(50)) 
LEFT JOIN {database_name}.lk_lote f ON f.PK_Plantel=e.PK_Plantel and f.nlote=rtrim(b.EntityNo) and f.ActiveFlag in (0,1) 
...
df_producciondetalle1 = spark.sql(f"SELECT \
                   ...
                   LEFT JOIN {database_name}.lk_tipoproducto TP ON Upper(tp.ntipoproducto) = Upper(lpr.grupoproducto) \
...
if datos_existentes:
    ...
    final_data.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name}.stg_ProduccionDetalleTemporal")
    ...
    final_data2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name}.stg_ProduccionDetalle")
    ...
    glue_client.delete_table(DatabaseName=database_name, Name='stg_ProduccionDetalleTemporal')
    print(f"Tabla stg_ProduccionDetalleTemporal eliminada correctamente de la base de datos '{database_name}'.")
