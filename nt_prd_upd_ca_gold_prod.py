...
try:
    ...
    df_lk= spark.sql(f"""SELECT
                ...
                1 AS idempresa
            FROM {database_name_si}.si_ProteinProducts PP
            LEFT JOIN {database_name_si}.si_ProteinProductsAnimals PPA ON PPA.ProteinProductsIRN = PP.IRN
            LEFT JOIN {database_name_gl}.lk_grupoconsumo GC ON PP.ProductType = GC.cgrupoconsumo and idempresa =1""")
    ...
    updated_data.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.{table_name}Temporal")
    ...
    final_data2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.{table_name}")
    ...
    glue_client.delete_table(DatabaseName=database_name_tmp, Name=f'{table_name}Temporal')
    
    print(f"Tabla {table_name}Temporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk_administrador= spark.sql(f"""SELECT TechSupervisorNo cadministrador, TechSupervisorname nadministrador_B, IRN AS IRN_B, U_Administrador abrevadministrador_B,FarmType AS tipo_B FROM {database_name_si}.si_ProteinTechSupervisors""")
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT FeedTypeNo,FeedTypeName,cast(IRN as varchar(50)) IRN,CreationDate FROM {database_name_si}.si_FmimFeedTypes""")
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT DriverNo cconductor, DriverName nconductor_B, IRN, activeflag activeflag_B FROM {database_name_si}.si_ProteinDrivers""")
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT BreedNo,BreedName,SpeciesType,IRN,ProteinBreedCodeTypesIRN,ActiveFlag activeflag_B FROM {database_name_si}.si_ProteinBreedCodes""")
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT MFF.FormulaNo,MFF.FormulaName,MFF.IRN, LA.pk_alimento,MFF.ProteinProductsIRN,ActiveFlag activeflag_B FROM {database_name_si}.si_mvFmimFeedFormulas MFF LEFT JOIN {database_name_gl}.lk_alimento LA ON LA.IRN = MFF.FmimFeedTypesIRN""")
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT HatcheryNo cincubadora,HatcheryName,HatcheryNo,IRN,Activeflag activeflag_B,ProteinCostCentersIRN FROM {database_name_si}.si_ProteinFacilityHatcheries""")
...
try:
    ...
    df_lk= spark.sql(f"""SELECT PP.productNo cproductoconsumo_B,SUBSTRING(PP.Description,1,50) nproductoconsumo_B,PP.IRN,PP.ActiveFlag activeflag_B,GC.pk_grupoconsumo pk_grupoconsumo_B,pk_subgrupoconsumo pk_subgrupoconsumo_B
                        FROM (select *,
                                ...
                                END SubGrupoConsumo from {database_name_si}.si_ProteinProducts) PP
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT StandardNo cstandard_B, StandardName nstandard_B, IRN,'1' activeflag_B, SpeciesType SpeciesType_B,CASE WHEN StandardNo like '%AD%' THEN 'Adulto' WHEN StandardNo like '%JO%' THEN 'Joven' ELSE '-' END AS lstandard_B FROM {database_name_si}.si_ProteinStandards""")
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT GrowoutNo csubzona_B, GrowoutName nzubzona_B, IRN, '1' Activeflag_B FROM {database_name_si}.si_ProteinGrowoutCodes""")
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT CountyNo czona, CountyName nzona_B, IRN,'1' activeflag_B FROM {database_name_si}.si_ProteinCounties """)
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT rtrim(VehicleNo) cvehiculo, rtrim(VehicleName) nvehiculo_B,rtrim(VehicleName) marca_B,IRN, activeflag activeflag_B FROM {database_name_si}.si_ProteinVehicles """)
...
try:
    ...
    df_lk= spark.sql(f"""SELECT  PF.FarmNo cplantel_B, PF.FarmName nplantel_B, PF.FarmNo noplantel_B, PF.IRN, PF.Address1 direccion1_B, PF.Address2 direccion2_B,COALESCE(LZ.pk_zona,1) pk_zona_B, pf.ActiveFlag, PF.ProteinCostCentersIRN, pk_subzona pk_subzona_B, UFC.Name Cluster_B, PV.VendorShortName duenho_B
                            ...
                            LEFT JOIN {database_name_gl}.Lk_zona LZ ON LZ.IRN = PF.ProteinCountiesIRN
                            LEFT JOIN {database_name_gl}.lk_subzona LS ON ls.IRN = pf.ProteinGrowoutCodesIRN
...
try:
    ...
    Lote as (SELECT distinct CONCAT(rtrim(BE.Farmno),'-',rtrim(BE.EntityNo)) clote, rtrim(BE.entityNo) nlote, rtrim(BE.farmno) noplantel,''AS IRN, '1' activeflag , COALESCE((SELECT pk_plantel FROM  lk_plantel LP WHERE LP.IRN like BE.ProteinFarmsIRN), 1) idplantel, 1 AS idempresa FROM {database_name_si}.si_mvBrimEntities BE ),
    ProteinEntities as (
    select *,EntityNo EntityNoCat from {database_name_si}.si_ProteinEntities where substring(notes,1,5) not in ('c1','c2','c3') or notes is null
    union
    select *, concat(substring(EntityNo,1,2) , substring(Notes,2,1) , substring(EntityNo,3,3)) EntityNoCat from {database_name_si}.si_ProteinEntities where substring(notes,1,5) in ('c1','c2','c3') and length(EntityNo) <> 7
    union
    select *, EntityNo EntityNoCat from {database_name_si}.si_ProteinEntities where substring(notes,1,5) in ('c1','c2','c3') and length(EntityNo) = 7 ) ,
    LotePavo AS (SELECT	DISTINCT substring(VB.ComplexEntityNo,1,length(VB.ComplexEntityNo)-6) clote
        ...
        FROM {database_name_si}.si_BrimFieldTrans AS BFT INNER JOIN
        {database_name_si}.si_mvBrimEntities AS VB ON BFT.ProteinEntitiesIRN = VB.ProteinEntitiesIRN INNER JOIN
        {database_name_si}.si_ProteinFarms AS PF ON VB.ProteinFarmsIRN = PF.IRN inner join
...
try:
    ...
    df_lk= spark.sql(f"""SELECT concat(rtrim(LP.cplantel),'-',rtrim(PH.HouseNo)) cgalpo,concat(rtrim(LP.cplantel),'-',rtrim(PH.HouseNo)) ngalpo, rtrim(PH.HouseNo) nogalpon, rtrim(LP.cplantel) noplantel_B, PH.HeadCapacity capacidad_B,PH.IRN IRN_B, PH.activeflag, LP.pk_plantel, PH.length length_B, PH.Width width_B, PH.area area_B 
                        FROM {database_name_si}.si_ProteinHouses PH LEFT JOIN {database_name_gl}.lk_plantel LP ON PH.ProteinFarmsIRN = LP.IRN """)
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT distinct COALESCE(VendorNo,'0') cproveedor, COALESCE(VendorName,'Sin Proveedor') nproveedor_B FROM {database_name_si}.si_mvBrimFarms """)
...
try:
    existing_data = spark.read.format("parquet").load(path_source)
    df_lk= spark.sql(f"""SELECT PlantNo cplanta, PlantName nplanta_B,ProteinCostCentersIRN ProteinCostCentersIRN_B, activeflag activeflag_B,IRN IRN_B FROM {database_name_si}.si_ProteinFacilityPlants """)
