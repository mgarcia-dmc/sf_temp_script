...
name_table19 = 'ft_peso_Diario'
file_name_target19 = f"{bucket_name_prdmtech}{name_table19}/"
path_target19 = f"s3://{bucket_name_target}/{file_name_target19}"
...
df_parametros = spark.sql(f"select * from {database_name_gl}.Parametros")
...
FROM {database_name_gl}.stg_ProduccionDetalle  group by  ComplexEntityNo,pk_diasvida")
...
df_MaxPesodia.write \
    ...
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.MaxPesodia")
...
left join {database_name_tmp}.MaxPesodia MT1 on MT1.ComplexEntityNo = M.ComplexEntityNo and MT1.pk_diasvida BETWEEN M.pk_diasvida AND M.pk_diasvida + 2 
left join {database_name_tmp}.MaxPesodia MT2 on MT2.ComplexEntityNo = M.ComplexEntityNo and MT2.pk_diasvida <= M.pk_diasvida 
...
df_PesoAcumuladorTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.PesoAcumulador")
...
from {database_name_tmp}.PesoAcumulador  group by ComplexEntityNo,pk_diasvida,pk_semanavida ")
...
df_SumPesoDiarioAcum.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.SumPesoDiarioAcum")
...
df_gananciaTemp1 = spark.sql(f"WITH BSD AS ( \
    ...
    FROM {database_name_tmp}.PesoAcumulador \
) \
...
df_ganancia.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ganancia")
...
df_PollosRendidosTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.PollosRendidos")
...
FROM {database_name_gl}.ft_alojamiento A \
where pk_empresa = 1 \
...
df_ft_alojamientoTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_gl}.ft_alojamientoTemp")
...
df_TablaPivotPesoSemTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.TablaPivotPesoSem")
...
df_PesoDetalleTemp1=spark.sql(f"select \
 PD.pk_tiempo \
,PD.pk_empresa \
,nvl(PD.pk_division,(select pk_division from {database_name_gl}.lk_division where cdivision=0)) as pk_division \
,nvl(PD.pk_zona,(select pk_zona from {database_name_gl}.lk_zona where czona='0')) as pk_zona \
,nvl(PD.pk_subzona,(select pk_subzona from {database_name_gl}.lk_subzona where csubzona='0')) as pk_subzona \
,nvl(PD.pk_plantel,(select pk_plantel from {database_name_gl}.lk_plantel where cplantel='0')) as pk_plantel \
,nvl(PD.pk_lote,(select pk_lote from {database_name_gl}.lk_lote where clote='0')) as pk_lote \
,nvl(PD.pk_galpon,(select pk_galpon from {database_name_gl}.lk_galpon where cgalpon='0')) as pk_galpon \
,nvl(PD.pk_sexo,(select pk_sexo from {database_name_gl}.lk_sexo where csexo=0)) pk_sexo \
,nvl(PD.pk_standard,(select pk_standard from {database_name_gl}.lk_standard where cstandard='0')) pk_standard \
,nvl(PD.pk_producto,(select pk_producto from {database_name_gl}.lk_producto where cproducto='0')) pk_producto \
,nvl(PD.pk_tipoproducto,(select pk_tipoproducto from {database_name_gl}.lk_tipoproducto where ntipoproducto='Sin Tipo Producto')) pk_tipoproducto \
,nvl(LEP.pk_especie,(select pk_especie from {database_name_gl}.lk_especie where cespecie='0'))pk_especie \
,nvl(PD.pk_estado,(select pk_estado from {database_name_gl}.lk_estado where cestado=0)) pk_estado \
,nvl(PD.pk_administrador,(select pk_administrador from {database_name_gl}.lk_administrador where cadministrador='0')) pk_administrador \
,nvl(PD.pk_proveedor,(select pk_proveedor from {database_name_gl}.lk_proveedor where cproveedor=0)) pk_proveedor \
,PD.pk_semanavida \
,PD.pk_diasvida \
,nvl(PD.pk_alimento,(select pk_alimento from {database_name_gl}.lk_alimento where calimento='0')) as pk_alimento \
,PD.ComplexEntityNo \
,PD.FechaNacimiento \
,PD.Edad as Edad \
...
,nvl((select avg(PesoSem) from {database_name_tmp}.PesoAcumulador a where a.ComplexEntityNo = PD.ComplexEntityNo and pk_diasvida = 6),0) as Peso5Dias \
...
LEFT JOIN {database_name_gl}.ft_ingresocons FIC ON FIC.ComplexEntityNo = PD.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.PollosRendidos PR ON cast(PR.ProteinFarmsIRN as varchar(50)) = cast(PD.ProteinFarmsIRN as varchar(50)) and PR.complexentityno =PD.ComplexEntityNo \
LEFT JOIN {database_name_tmp}.PesoAcumulador ACPE ON ACPE.ComplexEntityNo=PD.ComplexEntityNo AND PD.pk_diasvida= ACPE.pk_diasvida \
LEFT JOIN {database_name_tmp}.ganancia GAN ON GAN.ComplexEntityNo=PD.ComplexEntityNo AND PD.pk_diasvida= GAN.pk_diasvida \
LEFT JOIN {database_name_tmp}.TablaPivotPesoSem TPM ON PD.ComplexEntityNo = TPM.ComplexEntityNo \
LEFT JOIN {database_name_gl}.ft_alojamientoTemp ALO ON PD.complexentityno = ALO.complexentityno \
...
and PD.pk_lote not in (select pk_lote from {database_name_gl}.lk_lote where (clote like 'P186%' AND substring(clote,8,4) >= '11'))") 
#and PD.pk_lote in (select pk_lote from {database_name_gl}.lk_lote where substring(clote,8,4) in ('01','02','03','04','05','06','07','08','09','10') )")
...
df_PesoDetalleTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.PesoDetalle")
...
from {database_name_tmp}.PesoDetalle \
...
df_PesoTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Peso")
...
from {database_name_tmp}.Peso A \
left join {database_name_tmp}.Peso B on A.ComplexEntityNo = B.ComplexEntityNo and A.Orden = B.Orden + 1 \
order by A.Orden")
...
df_GananciaPesoDiaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.GananciaPesoDia")
...
from {database_name_tmp}.PesoDetalle PD \
...
FROM (
    ...
    ,'' DescripSemanaVida
    ,'' DescripDiaVida
    from {database_name_tmp}.PesoDetalle b 
    LEFT JOIN {database_name_tmp}.GananciaPesoDia GP ON GP.ComplexEntityNo = b.ComplexEntityNo AND GP.pk_diasvida = b.pk_diasvida 
    ...
    left join {database_name_tmp}.PesoDetalle c on c.ComplexEntityNo = b.ComplexEntityNo and c.pk_diasvida  = 2 
    left join {database_name_tmp}.PesoDetalle d on d.ComplexEntityNo = b.ComplexEntityNo and d.pk_diasvida  = b.pk_diasvida  - (8) 
    ...
    ,b.TipoOrigen
    ,CC.ConcatSemAnioCorral)
A  LEFT JOIN {database_name_gl}.lk_tiempo B ON A.pk_tiempo=b.pk_tiempo order by 1 """)
...
df_ft_peso_Diario.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.ft_peso_DiarioTemp")
...
if datos_existentes:
    ...
    final_data.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_peso_DiarioTemporal")
    ...
    final_data2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_peso_Diario")
    ...
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_peso_DiarioTemporal')
    print(f"Tabla ft_peso_DiarioTemporal eliminada correctamente de la base de datos {database_name_tmp}.")
...
WITH stdPesoNuevos as (
    ...
    from {database_name_gl}.ft_peso_Diario A
    inner join  (
        select 
    	A.pk_tiempo,
    	A.pk_diasvida,
    	...
    	(A.STDPEso - B.STDPEso) Diferencia 
        from {database_name_gl}.ft_peso_Diario A
        left join {database_name_gl}.ft_peso_Diario B 
        ...
        inner join (
        			select 
        				...
        				,ComplexEntityNo
        				,min(STDPeso) STDPEso 
        			from {database_name_gl}.ft_peso_Diario
        			where STDPeso = 0.0 and pk_empresa = 1 
        			group by ComplexEntityNo
        		 ) C on A.ComplexEntityNo = C.ComplexEntityNo and A.pk_diasvida = c.pk_diasvida - 1 
...
df_stdpesonuevo.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.STDPesoNuevo")
print('carga STDPesoNuevo',df_stdpesonuevo.count())
...
,a.DescripDiaVida
,a.DescripFecha
from {database_name_gl}.ft_peso_Diario  a left join (select distinct * from {database_name_tmp}.STDPesoNuevo) b on a.ComplexEntityNo = b.ComplexEntityNo 
...
,a.DescripDiaVida
,a.DescripFecha
from {database_name_gl}.ft_peso_Diario a left join 
{database_name_gl}.lk_ft_excel_categoria b on a.pk_lote = b.pk_lote
""")
