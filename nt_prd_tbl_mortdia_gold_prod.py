...
FROM {database_name_si}.si_brimfieldtrans a \
INNER JOIN {database_name_si}.si_mvbrimentities b ON a.ProteinEntitiesIRN=b.ProteinEntitiesIRN \
INNER JOIN {database_name_si}.si_proteinfarms c ON b.ProteinFarmsIRN=c.IRN \
LEFT OUTER JOIN {database_name_si}.si_mvfmimfeedformulas d ON a.FmimFeedFormulasIRN=d.IRN \
LEFT OUTER JOIN {database_name_si}.si_fmimfeedtypes e ON a.FmimFeedTypesIRN=e.IRN \
LEFT OUTER JOIN (select * from {database_name_si}.si_fmimfeedtrans where \
...
LEFT OUTER JOIN (select * from {database_name_si}.si_fmimfeedtransdetails where \
...
df_Mortdia.write \
    ...
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.mortdia")
...
df_Atipicos.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Atipicos")
