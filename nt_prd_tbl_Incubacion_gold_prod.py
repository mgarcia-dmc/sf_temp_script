...
#    .mode("overwrite") \
#    .saveAsTable(f"{database_name_tmp}.Incubacion")
...
LT.pk_tiempo \
,nvl(LD.pk_division,(select pk_division from {database_name_gl}.lk_division where cdivision=0)) pk_division \
,nvl(RGD.pk_zona,(select pk_zona from {database_name_gl}.lk_zona where czona='0')) pk_zona \
,nvl(LP.pk_plantel,(select pk_plantel from {database_name_gl}.lk_plantel where cplantel='0')) pk_plantel \
,nvl(LL.pk_lote,(select pk_lote from {database_name_gl}.lk_lote where clote='0')) pk_lote \
,nvl(LG.pk_galpon,(select pk_galpon from {database_name_gl}.lk_galpon where cgalpon='0')) pk_galpon \
,nvl(LI.pk_incubadora,(select pk_incubadora from {database_name_gl}.lk_incubadora where cincubadora='0')) pk_incubadora \
,nvl(LE.pk_especie,(select pk_especie from {database_name_gl}.lk_especie where cespecie='0')) pk_especie \
,nvl(LGE.pk_generacion,(select pk_generacion from {database_name_gl}.lk_generacion where cgeneracion=0)) pk_generacion \
,nvl(LPRO.pk_producto,(select pk_standard from {database_name_gl}.lk_standard where cstandard='0')) pk_producto \
...
FROM {database_name_tmp}.Incubacion A \
LEFT JOIN {database_name_gl}.lk_tiempo LT ON LT.fecha = nvl(cast(cast(A.xDate as timestamp) as date),cast('1899-11-30' as date)) \
LEFT JOIN {database_name_gl}.lk_plantel LP ON LP.cplantel = A.FarmNo AND LP.idempresa = 1 \
LEFT JOIN {database_name_gl}.lk_lote LL ON LL.nlote = A.EntityNo and LL.noplantel = A.FarmNo \
LEFT JOIN {database_name_gl}.lk_galpon LG ON LG.nogalpon = A.HouseNo and LG.noplantel = A.FarmNo AND LG.idempresa = 1 \
LEFT JOIN {database_name_gl}.lk_incubadora LI ON LI.IRN = A.ProteinFacilityHatcheriesIRN \
LEFT JOIN {database_name_gl}.lk_especie LE ON LE.cespecie = BreedNo \
LEFT JOIN {database_name_gl}.lk_generacion LGE ON LGE.cgeneracion = A.GenerationCode \
LEFT JOIN {database_name_gl}.lk_producto LPRO ON LPRO.cproducto = A.ProductNo AND LPRO.idempresa = 1 \
LEFT JOIN {database_name_gl}.ft_Reprod_Galpon_Diario RGD ON RGD.pk_plantel = LP.pk_plantel and RGD.pk_lote = LL.pk_lote and RGD.pk_galpon = LG.pk_galpon and \
RGD.descripFecha = cast(cast(A.ProdDate as timestamp) as date) \
LEFT JOIN {database_name_gl}.ft_Reprod_Galpon_Diario RGDC ON RGDC.pk_plantel = LP.pk_plantel and RGDC.pk_lote = LL.pk_lote and RGDC.pk_galpon = LG.pk_galpon and \
RGDC.descripFecha = cast(cast(A.SetDate as timestamp) as date) \
LEFT JOIN {database_name_si}.si_proteincostcenters PC ON PC.IRN = A.ProteinCostCentersIRN \
LEFT JOIN {database_name_gl}.lk_division LD ON LD.IRN = PC.ProteinDivisionsIRN \
LEFT JOIN {database_name_gl}.lk_maquinaincubadora LM ON LM.himsettersirn = A.himsettersirn \
LEFT JOIN {database_name_gl}.lk_maquinanacedora LN ON LN.HimHatchersIRN = A.HimHatchersIRN \
...
if datos_existentes00:
    ...
    final_data00.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Incub_PollitosNacidosTemporal")
    ...
    final_data00_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_Incub_PollitosNacidos")
    ...
    #glueContext.purge_table("{database_name_tmp}", "ft_mortalidadtemporal", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Incub_PollitosNacidosTemporal')
    print(f"Tabla ft_Incub_PollitosNacidosTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
...
df_OvoscopiaTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Ovoscopia")
...
,nvl(LI.pk_incubadora,(select pk_incubadora from {database_name_gl}.lk_incubadora where cincubadora='0')) pk_incubadora \
,nvl(LE.pk_especie,(select pk_especie from {database_name_gl}.lk_especie where cespecie='0')) pk_especie \
,nvl(LPRO.pk_producto,(select pk_producto from {database_name_gl}.lk_producto where cproducto='0')) pk_producto \
,nvl(LM.pk_maquinaincubadora,1) pk_maquinaincubadora \
,nvl(LN.pk_maquinanacedora,1) pk_maquinanacedora \
,A.ComplexEntityNo \
...
FROM {database_name_tmp}.Ovoscopia A \
LEFT JOIN {database_name_gl}.lk_tiempo LT ON LT.fecha = nvl(cast(cast(A.transdate as timestamp) as date),cast('1899-11-30' as date)) \
LEFT JOIN {database_name_gl}.lk_plantel LP ON LP.cplantel = A.FarmNo AND LP.idempresa = 1 \
LEFT JOIN {database_name_gl}.lk_lote LL ON LL.nlote = A.EntityNo and LL.noplantel = A.FarmNo \
LEFT JOIN {database_name_gl}.lk_galpon LG ON LG.nogalpon = A.HouseNo and LG.noplantel = A.FarmNo AND LG.idempresa = 1 \
LEFT JOIN {database_name_gl}.lk_incubadora LI ON LI.IRN = A.ProteinFacilityHatcheriesIRN \
LEFT JOIN {database_name_gl}.lk_especie LE ON LE.cespecie = ProgenyBreedNo \
LEFT JOIN {database_name_gl}.lk_producto LPRO ON LPRO.cproducto = A.ProgenyProductNo AND LPRO.idempresa = 1 \
LEFT JOIN {database_name_gl}.ft_Reprod_Galpon_Diario RGD ON RGD.pk_plantel = LP.pk_plantel and RGD.pk_lote = LL.pk_lote and RGD.pk_galpon = LG.pk_galpon and RGD.descripFecha = cast(cast(A.ProdDate as timestamp) as date) \
LEFT JOIN {database_name_gl}.ft_Reprod_Galpon_Diario RGDC ON RGDC.pk_plantel = LP.pk_plantel and RGDC.pk_lote = LL.pk_lote and RGDC.pk_galpon = LG.pk_galpon and RGDC.descripFecha = cast(cast(A.SetDate as timestamp) as date) \
LEFT JOIN {database_name_si}.si_proteincostcenters PC ON PC.IRN = A.ProteinCostCentersIRN \
LEFT JOIN {database_name_gl}.lk_division LD ON LD.IRN = PC.ProteinDivisionsIRN \
LEFT JOIN {database_name_gl}.lk_maquinaincubadora LM ON LM.himsettersirn = A.himsettersirn \
LEFT JOIN {database_name_gl}.lk_maquinanacedora LN ON LN.HimHatchersIRN = A.HimHatchersIRN \
...
if datos_existentes:
    ...
    final_data.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Incub_OvoscopiaTemporal")
    ...
    final_data2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_Incub_Ovoscopia")
    ...
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    #glueContext.purge_table("{database_name_tmp}", "ft_mortalidadtemporal", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Incub_OvoscopiaTemporal')
    print(f"Tabla ft_Incub_OvoscopiaTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
...
,A.pk_empresa 
FROM {database_name_gl}.ft_incub_pollitosnacidos A 
left join {database_name_gl}.ft_Incub_Ovoscopia B on A.ComplexEntityNo = B.ComplexEntityNo and A.MaquinaNacedora = B.MaquinaNacedora and A.MaquinaIncubadora = B.MaquinaIncubadora and A.pk_tiempo = B.pk_tiempo 
and A.pk_incubadora = B.pk_incubadora and B.NumMuestras <> 0 
left join {database_name_gl}.lk_especie C on A.pk_especie = C.pk_especie 
left join {database_name_gl}.lk_ft_excel_incub_std_raza D on substring(C.cespecie,1,4) = D.raza and Ceiling(A.EdadFechaCarga) = D.edad 
...
df_TablaNacDiarioTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.TablaNacDiario")
...
,pk_empresa \
,fecha \
FROM {database_name_tmp}.TablaNacDiario \
...
if datos_existentes0:
    ...
    final_data0.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Incub_Tabla_NacDiarioTemporal")
    ...
    final_data0_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_Incub_Tabla_NacDiario")
    ...
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    #glueContext.purge_table("{database_name_tmp}", "ft_mortalidadtemporal", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Incub_Tabla_NacDiarioTemporal')
    print(f"Tabla ft_Incub_Tabla_NacDiarioTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
...
,sum(Otros) Otros \
,pk_empresa \
FROM {database_name_gl}.ft_Incub_Tabla_NacDiario \
...
df_SemanalTemp.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.Semanal")
print('carga Semanal')
...
,0 hvoscontaminado \
,A.fecha \
FROM {database_name_tmp}.Semanal A \
LEFT JOIN {database_name_gl}.lk_especie C on A.pk_especie = C.pk_especie \
LEFT JOIN {database_name_gl}.lk_ft_Excel_Incub_STD_Raza D on substring(C.cespecie,1,4) = D.raza and Ceiling(A.Edad) = D.edad \
LEFT JOIN {database_name_gl}.lk_zona Z on A.pk_zona = Z.pk_zona \
LEFT JOIN {database_name_gl}.lk_especie E ON A.pk_especie = E.pk_especie \
WHERE (date_format(fecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM'))")
...
if datos_existentes01:
    ...
    final_data01.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_tmp}.ft_Incub_Tabla_NacSemanalTemporal")
    ...
    final_data01_2.write \
        ...
        .options(**additional_options) \
        .mode("overwrite") \
        .saveAsTable(f"{database_name_gl}.ft_Incub_Tabla_NacSemanal")
    ...
    glueContext.purge_s3_path(f"s3://{bucket_name_target}/{bucket_name_prdmtech}Temporal/", {"retentionPeriod": 0})
    #glueContext.purge_table("{database_name_tmp}", "ft_mortalidadtemporal", {"retentionPeriod": 0})
    glue_client.delete_table(DatabaseName=database_name_tmp, Name='ft_Incub_Tabla_NacSemanalTemporal')
    print(f"Tabla ft_Incub_Tabla_NacSemanalTemporal eliminada correctamente de la base de datos '{database_name_tmp}'.")
