...
from {database_name_si}.si_brimstandardsdata BSD
LEFT JOIN {database_name_si}.si_ProteinStandardVersions PSV ON BSD.ProteinStandardVersionsIRN = PSV.IRN
LEFT JOIN {database_name_gl}.lk_standard LST ON LST.IRN = PSV.ProteinStandardsIRN
where age <> 0
...
df_std1.write \
    ...
    .mode("overwrite") \
    .saveAsTable(f"{database_name_tmp}.std1")
...
df_UPDstd1.write \
    ...
    .format("parquet") \
    .options(**additional_options) \
    .saveAsTable(f"{database_name_tmp}.std1_new")

df_std1_nueva = spark.sql(f"""SELECT * from {database_name_tmp}.std1_new """)

# 2️⃣ Eliminar la tabla original (opcional, si se permite)
spark.sql(f"DROP TABLE IF EXISTS {database_name_tmp}.std1")
...
spark.sql(f"DROP TABLE IF EXISTS {database_name_tmp}.std1_new")
...
df_ft_dinuts_total.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name_gl}.ft_dinuts_total")
