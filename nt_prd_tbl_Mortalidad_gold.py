...
,MO.descripfecha
FROM {database_name}.stg_ProduccionDetalle MO 
LEFT JOIN {database_name}.detmort DM ON UPPER(DM.IRN)= UPPER(MO.BrimFieldTransIRN )
LEFT JOIN {database_name}.lk_causamortalidad LCM ON LCM.IRN = case when DM.CAUSA = 'U_PESangreEnCieGO' then 'U_PESangreEnCiego' else CAST(DM.CAUSA AS VARCHAR(50)) end
...
df_ft_mortalidadT.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.ft_mortalidadT")
print('carga df_ft_mortalidadT', df_ft_mortalidadT.count())
df_ft_mortalidad = spark.sql(f"""SELECT * from {database_name}.ft_mortalidadT
...
FROM
    WindowedData a
left join {database_name}.lk_diasvida b on a.FirstHatchDateAge = b.FirstHatchDateAge
""")
...
FROM {database_name}.stg_ProduccionDetalle mt \
WHERE (date_format(descripfecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-9),'yyyyMM')) \
...
FROM
    {database_name}.AcumuladorsMortalidad
WHERE
    flagartificio = 1
GROUP BY
...
from {database_name}.stg_ProduccionDetalle \
...
and MO.pk_lote not in (select pk_lote from {database_name}.lk_lote where (Upper(clote) like 'P186%' AND SUBSTRING(clote,8,4) >= '11')) \
...
FROM (
    ...
    FROM
        {database_name}.MortalidadDetalle1
) AS unpivoted_data
WHERE
    cmortalidadacum > 0
""")
...
LEFT JOIN {database_name}.detmortacum c ON a.complexentityno = c.complexentityno AND a.pk_diasvida = c.pk_diasvida and SUBSTRING(UPPER(irn),5,30) = SUBSTRING(UPPER(causaacum),7,30)
where date_format(descripfecha, 'yyyyMM') >= date_format(add_months(trunc(current_date(), 'month'), -4), 'yyyyMM')
""")
...
if datos_existentes251:
    ...
    df_ft_mortalidad_Diario.write \
        ...
        .mode("overwrite") \
        .saveAsTable(f"{database_name}.ft_mortalidad_diario")
df_maxfechalotetemp =spark.sql(f"select pk_lote,max(descripfecha) fecha from {database_name}.ft_mortalidad_diario  group by pk_lote")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_ft_mortalidad_diario_ins.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.ft_mortalidadI")
print('carga df_ft_mortalidad_diario_ins', df_ft_mortalidad_diario_ins.count())
df_ft_mortalidad_diario_upd = spark.sql(f"""SELECT * from {database_name}.ft_mortalidadI
WHERE date_format(DescripFecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')
""")
...
try:
    df = spark.table("default.MortalidadCorral")
...
df_MortalidadCorral_new.write \
    ...
    .format("parquet") \
    .options(**additional_options) \
    .saveAsTable("default.MortalidadCorral_new")

df_MortalidadCorral_nueva = spark.sql("""SELECT * from default.MortalidadCorral_new """)

# 2️⃣ Eliminar la tabla original (opcional, si se permite)
spark.sql("DROP TABLE IF EXISTS default.MortalidadCorral")
...
df_MortalidadCorral_nueva.write \
    ...
    .options(**additional_options2) \
    .mode("overwrite") \
    .saveAsTable(f"default.MortalidadCorral")  

# 5️⃣ (Opcional) Eliminar la tabla temporal si ya no es neces
spark.sql("DROP TABLE IF EXISTS default.MortalidadCorral_new")
...
,'' DescripProveedor            \
,'' DescripDiaVida              \
FROM {database_name}.MortalidadCorral A left join {database_name}.lk_tiempo b on b.fecha=cast(concat(substring(a.fechacierre,1,4),'-',substring(a.fechacierre,5,2),'-',substring(a.fechacierre,7,2)) as date) \
")
...
df_ft_mortalidad_CorralF.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.ft_mortalidad_CorralF")
print('carga df_ft_mortalidad_CorralF', df_ft_mortalidad_CorralF.count())
df_ft_mortalidad_Corral = spark.sql(f"""SELECT * from {database_name}.ft_mortalidad_CorralF
WHERE date_format(DescripFecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')
""")
...
try:
    df = spark.table("default.MortalidadGalpon")
...
df_MortalidadGalpon_new.write \
    ...
    .format("parquet") \
    .options(**additional_options) \
    .saveAsTable("default.MortalidadGalpon_new")

df_MortalidadGalpon_nueva = spark.sql("""SELECT * from default.MortalidadGalpon_new """)

# 2️⃣ Eliminar la tabla original (opcional, si se permite)
spark.sql("DROP TABLE IF EXISTS default.MortalidadGalpon")
...
df_MortalidadGalpon_nueva.write \
    ...
    .options(**additional_options2) \
    .mode("overwrite") \
    .saveAsTable(f"default.MortalidadGalpon")  

# 5️⃣ (Opcional) Eliminar la tabla temporal si ya no es neces
spark.sql("DROP TABLE IF EXISTS default.MortalidadGalpon_new")
...
df_ft_mortalidad_GalponF.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.ft_mortalidad_galponF")
print('carga df_ft_mortalidad_GalponF', df_ft_mortalidad_GalponF.count())
df_ft_mortalidad_Galpon = spark.sql(f"""SELECT * from {database_name}.ft_mortalidad_galponF
WHERE date_format(DescripFecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')
""")
...
df_listaMortalidadGalponTemp = spark.sql(f"SELECT pk_lote, concat_ws(',',collect_list( DISTINCT categoria)) categoria FROM {database_name}.MortalidadGalpon  group by pk_lote")
# Escribir los resultados en ruta temporal
additional_options = {
...
df_maxordenIdTemp =spark.sql(f"select Upper(clote) as clote,max(orden) orden from {database_name}.OrdenId  group by  clote")
# Escribir los resultados en ruta temporal
additional_options = {
...
from {database_name}.MortalidadGalpon A \
left join {database_name}.lk_lote B on A.pk_lote = B.pk_lote \
left join {database_name}.OrdenId C on Upper(B.clote) = Upper(C.clote) \
left join listaMortalidadGalponTemp f on F.pk_lote = A.pk_lote \
left join maxordenIdTemp g on Upper(g.clote) = Upper(C.clote) and C.orden = g.orden \
...
left join {database_name}.maxordenIdTemp g on Upper(g.clote) = Upper(C.clote) and C.orden = g.orden \
where FlagAtipico = 1 \
group by a.pk_empresa,pk_division,pk_zona,pk_subzona,a.pk_plantel,a.pk_lote,c.pk_tipoproducto,pk_estado,b.clote,a.pk_especie,pk_proveedor \
,FlagTransfPavos,SourceComplexEntityNoLote,DestinationComplexEntityNoLote")
...
try:
    df = spark.table("default.MortalidadLote")
...
df_MortalidadLote_new.write \
    ...
    .format("parquet") \
    .options(**additional_options) \
    .saveAsTable("default.MortalidadLote_new")

df_MortalidadLote_nueva = spark.sql("""SELECT * from default.MortalidadLote_new """)

# 2️⃣ Eliminar la tabla original (opcional, si se permite)
spark.sql("DROP TABLE IF EXISTS default.MortalidadLote")
...
df_MortalidadLote_nueva.write \
    ...
    .options(**additional_options2) \
    .mode("overwrite") \
    .saveAsTable(f"default.MortalidadLote")  

# 5️⃣ (Opcional) Eliminar la tabla temporal si ya no es neces
spark.sql("DROP TABLE IF EXISTS default.MortalidadLote_new")
...
df_ft_mortalidad_LoteF.write \
    ...
    .options(**additional_options) \
    .mode("overwrite") \
    .saveAsTable(f"{database_name}.ft_mortalidad_LoteF")
print('df_ft_mortalidad_LoteF', df_ft_mortalidad_LoteF.count())
df_ft_mortalidad_Lote = spark.sql(f"""SELECT * from {database_name}.ft_mortalidad_LoteF
WHERE date_format(DescripFecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-4),'yyyyMM')
""")
...
from {database_name}.ft_mortalidad_diario a 
left join (
           select pk_lote,max(PobInicial) PobInicial from {database_name}.ft_mortalidad_lote group by pk_lote
...
and a.pk_lote not in (select pk_lote from {database_name}.lk_lote where (Upper(clote) like 'P186%' AND SUBSTRING(clote,8,4) >= '11')) 
...
FROM {database_name}.ft_mortalidad_diario where pk_lote not in (select pk_lote from {database_name}.lk_lote where (Upper(clote) like 'P186%' AND SUBSTRING(clote,8,4) >= '11')) group by pk_lote")
# Escribir los resultados en ruta temporal
additional_options = {
...
from {database_name}.ft_mortalidad_diario a 
left join {database_name}.LesionLoteSemanal b on substring(A.complexentityno,1,(length(A.complexentityno)-6)) = b.complexentityno and 
...
and e.pk_lote not in (select pk_lote from {database_name}.lk_lote where (Upper(clote) like 'P186%' AND SUBSTRING(clote,8,4) >= '11'))
...
FROM
    (SELECT pk_semanavida, complexentityno FROM {database_name}.OrdenarMortalidades GROUP BY pk_semanavida, complexentityno) AS A
LEFT JOIN
    (SELECT pk_semanavida, complexentityno, AVG(cmortalidad) AS cmortalidad, ordenCantidad FROM {database_name}.OrdenarMortalidades GROUP BY pk_semanavida, complexentityno, ordenCantidad) AS P1
    ON A.ComplexEntityNo = P1.ComplexEntityNo AND A.pk_semanavida = P1.pk_semanavida
LEFT JOIN
    (SELECT pk_semanavida, complexentityno, AVG(pcmortalidad) AS pcmortalidad, ordenPorcentaje FROM {database_name}.OrdenarMortalidades GROUP BY pk_semanavida, complexentityno, ordenPorcentaje) AS P2
    ON A.ComplexEntityNo = P2.ComplexEntityNo AND A.pk_semanavida = P2.pk_semanavida
LEFT JOIN
    (SELECT pk_semanavida, complexentityno, MAX(causa) AS causa, ordenNombre FROM {database_name}.OrdenarMortalidades GROUP BY pk_semanavida, complexentityno, ordenNombre) AS P3
...
from 
    {database_name}.LesionesMortalidadLoteSemanal
)
WHERE
    cmortalidadacum <> 0
ORDER BY
...
FROM
    (SELECT pk_semanavida, complexentityno FROM {database_name}.OrdenarMortalidadesAcum GROUP BY pk_semanavida, complexentityno) AS A
LEFT JOIN
    (SELECT pk_semanavida, complexentityno, AVG(cmortalidadacum) AS cmortalidadacum, ordenCantidad FROM {database_name}.OrdenarMortalidadesAcum GROUP BY pk_semanavida, complexentityno, ordenCantidad) AS P1
    ON A.ComplexEntityNo = P1.ComplexEntityNo AND A.pk_semanavida = P1.pk_semanavida
LEFT JOIN
    (SELECT pk_semanavida, complexentityno, AVG(pcmortalidadacum) AS pcmortalidadacum, ordenPorcentaje FROM {database_name}.OrdenarMortalidadesAcum GROUP BY pk_semanavida, complexentityno, ordenPorcentaje) AS P2
    ON A.ComplexEntityNo = P2.ComplexEntityNo AND A.pk_semanavida = P2.pk_semanavida
LEFT JOIN
    (SELECT pk_semanavida, complexentityno, MAX(causa) AS causa, ordenNombre FROM {database_name}.OrdenarMortalidadesAcum GROUP BY pk_semanavida, complexentityno, ordenNombre) AS P3
    ON A.ComplexEntityNo = P3.ComplexEntityNo AND A.pk_semanavida = P3.pk_semanavida
GROUP BY
    A.pk_semanavida, A.ComplexEntityNo
""")
...
fechaactual = datetime.now().replace(day=1)
fecha_menos = fechaactual - relativedelta(months=6)
fecha_str = fecha_menos.strftime("%Y-%m-%d")
...
and c.pk_empresa = 1 AND (date_format(descripfecha,'yyyyMM') >= date_format(add_months(trunc(current_date, 'month'),-6),'yyyyMM')) 
and c.pk_division=4""")
...
try:
    df = spark.table("default.MortalidadMensual")
...
df_MortalidadMensual_new.write \
    ...
    .format("parquet") \
    .options(**additional_options) \
    .saveAsTable("default.MortalidadMensual_new")

df_MortalidadMensual_nueva = spark.sql("""SELECT * from default.MortalidadMensual_new """)

# 2️⃣ Eliminar la tabla original (opcional, si se permite)
spark.sql("DROP TABLE IF EXISTS default.MortalidadMensual")
...
df_MortalidadMensual_nueva.write \
    ...
    .options(**additional_options2) \
    .mode("overwrite") \
    .saveAsTable(f"default.MortalidadMensual")  

# 5️⃣ (Opcional) Eliminar la tabla temporal si ya no es neces
spark.sql("DROP TABLE IF EXISTS default.MortalidadMensual_new")
